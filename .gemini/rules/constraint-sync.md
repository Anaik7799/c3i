...
**GEMINI.md MUST be the authoritative superset of ALL constraints in code.** Code MUST NEVER contain undocumented constraints. NEVER remove constraints from docs because code doesn't reference them.
# STAMP Constraints
| ID | Constraint | Severity |
|----|------------|----------|
| SC-SYNC-DOC-001 | GEMINI.md SC-* set MUST be superset of code SC-* set | CRITICAL |
| SC-SYNC-DOC-002 | GEMINI.md AOR-* set MUST be superset of code AOR-* set | CRITICAL |
| SC-SYNC-DOC-003 | Sync check MUST run on every Gemini session start | CRITICAL |
...
| SC-SYNC-DOC-006 | .gemini/rules/ MUST be audited for staleness daily | MEDIUM |
| SC-SYNC-DOC-007 | .gemini/agents/ inventory MUST match agent definitions | MEDIUM |
| SC-SYNC-DOC-008 | .gemini/commands/ inventory MUST match skill definitions | MEDIUM |
| SC-SYNC-DOC-009 | New code SC-*/AOR-* MUST be added to GEMINI.md before commit | CRITICAL |
| SC-SYNC-DOC-010 | Sync gap ratio MUST trend toward 1.0 (parity) | HIGH |
| SC-SYNC-DOC-011 | Gemini MUST use F# constraint sync engine for all sync ops | CRITICAL |
| SC-SYNC-DOC-012 | Gemini SHALL NOT use ad-hoc rg commands for constraint census | HIGH |
...
| SC-SYNC-DOC-015 | Analysis results auto-cached to .gemini/constraint_sync_cache.json | HIGH |
...
dotnet exec $BIN --inventory  # .gemini/ inventory
