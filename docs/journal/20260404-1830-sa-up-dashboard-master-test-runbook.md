...
A comprehensive mission simulation that transitions through three phases to verify the entire system stack, featuring real-time OODA telemetry logging for AI Agent visibility.
...
*   **Zenoh OTel Telemetry**: A live right-hand pane tracks all `Observe`, `Orient`, `Decide`, and `Act` OTel spans generated by the system state and manual interactions.
*   **AI Agent Logging**: Upon exit, all telemetry is written to `zenoh_otel_trace.jsonl` to ensure full visibility for OODA-based AI monitoring and improvement.
*   **Interaction**:
