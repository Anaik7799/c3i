...
# SOPv5.1 ENHANCED SCRIPT - focused_complexity_refactor.exs
...
**Agent**: Script Enhancement System with systematic SOPv5.1 integration
  ...
  defp count_nested_statements(lines) do
    Enum.count(lines, fn line ->
      # Look for deeply nested __statements
  ...
  defp apply_case_statement_optimization(content) do
    # Optimize complex case __statements by extracting common patterns
    content
    |> String.replace(~r/case\s+([^d][^o].*?)\s+do\s*(.*?)end/s, fn match ->
      if String.length(match) > 200 do
        # This is a complex case __statement, apply optimization patterns
  ...
  defp optimize_complex_case(case__statement) do
  ...
  defp extract_common_guards(case__statement) do
    # Extract common guard patterns into helper functions
    case_statement
  end

  defp consolidate_similar_patterns(case__statement) do
    # Consolidate similar match patterns
    case_statement
  end

  defp add_default_case_if_needed(case__statement) do
  ...
  defp apply_conditional_simplification(content) do
    # Simplify complex conditional __statements
    content
  ...
  defp extract_condition(if__statement) do
  ...
  defp apply_nested_statement_reduction(content) do
    # Reduce deeply nested __statements using early returns and guards
    content
  ...
  defp replace_nested_ifs_with_cond(content) do
    # Replace nested if __statements with cond for better readability
    content
  end
  ...
  defp save_refactoring_results(results) do
    ...
    results_filename = "__data/tmp/claude_focused_refactoring_#{timestamp}.json"
    ...
    summary_filename = "__data/tmp/claude_focused_refactoring_summary_#{timestamp}.log"
