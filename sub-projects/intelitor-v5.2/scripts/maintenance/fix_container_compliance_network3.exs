...
# SOPv5.1 ENHANCED SCRIPT - fix_container_compliance_network3.exs
...
@spec fix_file() :: any()
  def fix_file do
    ...
    case File.read(file_path) do
      {:ok, content} ->
        # Read lines and fix specific issues
        ...
        fixed_lines =
          ...
          |> Enum.map(fn {line, idx} ->
            cond do
              # Remove duplicate Redis line
              idx == 320 && String.contains?(line, "Redis") ->
                nil

              # Fix the failed_connections line
              idx == 322 ->
                "    failed_connections = Enum.reject(test_ports, fn {_name, host, port} ->"

              # Remove duplicate app.js line
              idx == 359 && String.contains?(line, "app.js") ->
                prev_line = Enum.at(lines, idx - 2, "")

                if String.contains?(prev_line, "app.js") do
                  nil
                else
                  line
                end

              # Add missing line for missing_files before the if __statement
              idx == 361 && String.trim(line) == "" ->
