#═══════════════════════════════════════════════════════════════════════════════
# SOPv5.1 ENHANCED ENVIRONMENT CONFIGURATION - gleam@dynamic@decode.erl
#═══════════════════════════════════════════════════════════════════════════════
#
...
-opaque decoder(BXS) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BXS, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BYA)) -> {ok, BYA} |
...
-spec map(decoder(CBY), fun((CBY) -> CCA)) -> decoder(CCA).
...
-spec then(decoder(CCK), fun((CCK) -> decoder(CCM))) -> decoder(CCM).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CCP), list(decoder(CCP))) -> decoder(CCP).
...
-spec recursive(fun(() -> decoder(CDF))) -> decoder(CDF).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(CBU)) -> decoder(gleam@option:option(CBU)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BZV} | {error, BZV})
) -> {BZV, list(decode_error())}.
...
-spec collapse_errors(decoder(CCH), binary()) -> decoder(CCH).
...
-spec failure(CCZ, binary()) -> decoder(CCZ).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CDB} | {error, CDB})
) -> decoder(CDB).
...
-spec fold_dict(
    {gleam@dict:dict(CBG, CBH), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CBG, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {CBH, list(decode_error())})
) -> {gleam@dict:dict(CBG, CBH), list(decode_error())}.
...
-spec dict(decoder(CAZ), decoder(CBB)) -> decoder(gleam@dict:dict(CAZ, CBB)).
dict(Key, Value) ->
...
-spec list(decoder(CAN)) -> decoder(list(CAN)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BYW, list(decode_error())}, list(any())) -> {BYW,
    list(decode_error())}.
...
-spec index(
    list(BYK),
    list(BYK),
    fun((gleam@dynamic:dynamic_()) -> {BYN, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BYK)) -> {BYN, list(decode_error())})
) -> {BYN, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BXV), fun((BXV) -> decoder(BXX))) -> decoder(BXX).
...
-spec at(list(any()), decoder(BYH)) -> decoder(BYH).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BZF), fun((BZF) -> decoder(BZH))) -> decoder(BZH).
...
-spec optional_field(any(), BZL, decoder(BZL), fun((BZL) -> decoder(BZN))) -> decoder(BZN).
...
-spec optionally_at(list(any()), BZS, decoder(BZS)) -> decoder(BZS).
