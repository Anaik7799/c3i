defmodule Intelitor.Integrations.ApiConnection do
  @moduledoc """
  ...
  relationships do
    ...
    has_many :data_mappings, Intelitor.Integrations.DataMapping do
      public? true
    end

    has_many :sync_jobs, Intelitor.Integrations.SyncJob do
      public? true
    end
  end
  ...
  validations do
    validate fn changeset, _context ->
      ...
      if connection_type in [:rest_api, :soap, :graphql, :onvif] && is_nil(base_url) do
        {:error, field: :base_url, message: "required for #{connection_type} connections"}
      else
        {:ok, changeset}
      end
    end
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:status]
      index [:enabled?], name: "api_connections_enabled_index", where: "enabled? = true"
    end
  end
end
