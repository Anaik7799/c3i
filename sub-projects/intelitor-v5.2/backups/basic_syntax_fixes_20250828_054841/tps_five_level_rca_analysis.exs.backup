...
defmodule TPSFiveLevelRCAAnalysis do
  ...
  def main(_args) do
    ...
    IO.puts("Principle: Stop, Fix, and Prevent Recurrence")
    IO.puts("")

    # Apply 5-Level RCA to each issue
    rca_results = Enum.map(@identified_issues, fn issue ->
  ...
  defp analyze_issue_with_five_levels(issue) do
    ...
    # Level 5: Why did the system fail to prevent it?
    ...
    %{
      ...
      prevention: generate_prevention_measures(issue, level5)
    }
  end
  ...
  defp analyze_level_5(issue, _level4) do
    IO.puts("  📍 Level 5: Why did the system fail to prevent it?")
  ...
  @spec generate_prevention_measures(term(), term()) :: term()
  defp generate_prevention_measures(issue, root_cause) do
    %{
      process_improvement: "Update development workflow to prevent #{root_cause}"
      automation: "Implement automated checks for early detection",
      training: "Team training on #{issue.domain} best practices",
      documentation: "Update guidelines with prevention strategies"
    }
  end
  ...
  defp display_tps_summary(rca_results) do
    ...
    IO.puts("  Prevention Measures: Comprehensive")
    IO.puts("")
