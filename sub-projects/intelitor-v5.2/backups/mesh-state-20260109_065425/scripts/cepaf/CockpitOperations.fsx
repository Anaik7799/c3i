...
module Operations =
    let deploy () =
        Console.printHeader "DEPLOYING FRACTAL MESH ENVIRONMENT"

        let clusterFile = Path.Combine(Config.ArtifactsDir, "podman-compose-fractal-cluster.yml")
        let targetFile = if File.Exists(clusterFile) then clusterFile else Path.Combine(Config.ArtifactsDir, "podman-compose-prod-standalone.yml")
    ...
    let status () =
        ...
        Console.printSection "Health Checks (SC-CLU-002: Fractal-Cluster)"
        // Fractal-cluster container names per podman-compose-fractal-cluster.yml
        let containers = ["db-primary"; "indrajaal-obs"; "indrajaal-app-1"; "indrajaal-app-2"; "indrajaal-app-3"]
    ...
    let monitor () =
        ...
        for (name, url) in endpoints |> List.take 3 do
            let (code, _, _) = Shell.exec "curl" $"-s -o /dev/null -w '%%{{http_code}}' {url}"
            let status = if code = 0 then "✅ UP" else "❌ DOWN"
    ...
    let cleanup () =
        ...
        if File.Exists(targetFile) then
            ...
            // Fallback: stop individual fractal-cluster containers (SC-CLU-002)
            let containers = ["indrajaal-app-1"; "indrajaal-app-2"; "indrajaal-app-3";
                             "db-primary"; "indrajaal-obs"]
