```
  ...
  ●───◉─┤◈├─◉───●   INDRAJAAL
     ╭╯ ╰─╯ ╰╮       इन्द्रजाल
    ●╯       ╰●       v21.2.0 SIL-6 Biomorphic Fractal Mesh
```

## Cognitive Bootstrapping
- **MANDATORY**: Any new agent session MUST begin by reading `AGENT_BOOTSTRAP.md`. This file contains the collated "Directed Telescope" architectural state, safety axioms, and operational command set for Indrajaal and Prajna.

# GEMINI.md - Indrajaal Safety-Critical System Exhaustive Specification

**Version**: 21.2.0-SIL6 | **Architecture**: SIL-6 Biomorphic Fractal Mesh
**Classification**: IMMUTABLE SYSTEM AXIOMS + FORMAL VERIFICATION + CYBERNETIC PROTOCOLS + FRACTAL HOLONIC ARCHITECTURE + SIL-6 BIOMORPHIC EXTENSIONS
**Origin**: Consolidated from Grand Unification v19 + Mathematical Foundations v20.0.0 + BEP v1.0.0 + SIL-6 Biomorphic v21.2.0
**Updated**: 2026-01-05
**Status**: SOPv5.11 + VSM + OODA + CATEGORY THEORY + AGDA PROOFS + QUINT MODELS + 100% FRACTAL COVERAGE + SIL-6 BIOMORPHIC MESH
**Static Analysis**: CREDO CONVERGENCE ACHIEVED (Reduced from >6000 to 5 issues). Configuration tuned for operational efficiency (Smart Duplication, Dialyzer-first specs).
...
**Synced With**: 20251221-GEMINI-math.md (Mathematica notation)
...
**Constraint**: The System MUST ALWAYS be in a functional, compilable, and operational state.
...
4.  **Fractal Consistency Rule**: This rule MUST be enforced recursively from **L7 (Federation)** down to **L0 (Runtime/Code)** across ALL dimensions (Architecture, Implementation, Documentation, Telemetry).
    *   **L7 (Federation)**: Global invariants hold.
    *   **L6 (Cluster)**: Consensus holds.
    *   **L5 (Node)**: Runtime environment is stable.
    *   **L4 (Container)**: Isolation is breached only by policy.
    *   **L3 (Holon)**: Agent logic is sound.
    *   **L2 (Component)**: Modules are cohesive.
    *   **L1 (Function)**: Input/Output contracts are valid.
...
Ω₁ := Module[{c ∈ CompilationOperations},
  ...
  (* 2. Resource Maximization - Full Parallelism *)
...
mix compile --warnings-as-errors --jobs 16 2>&1 | tee -a ./data/tmp/1-compile.log
```
...
|-----------|-------|---------|
| `--jobs <N>` | 16 | Parallel compilation processes (match CPU cores) |
| `ELIXIR_ERL_OPTIONS="+S <N>:<N>"` | +S 16:16 | BEAM scheduler count (online:offline) |
| `ELIXIR_ERL_OPTIONS="+SDio <N>"` | +SDio 16 | Dirty I/O schedulers for I/O-bound tasks |
| `MIX_OS_DEPS_COMPILE_PARTITION_COUNT` | 8 | OS processes for dependency compilation (N/2) |
...
4. **Forbidden Set**: $\mathbb{F}_{CNT} = \text{Docker} \cup \text{Alpine} \cup \text{Ubuntu} \cup \text{ProprietaryRegistries}$
5. **Constraint**: $S_{img} \cap \mathbb{F}_{CNT} \equiv \emptyset$
6. **Synchronization**: PHICS v2.1 active with latency $\delta < 50ms$

**Podman-Only Container Infrastructure Policy (ZERO TOLERANCE - SC-CNT-009)**:
The use of `docker`, `docker-compose`, or any `docker-*` command is a **CRITICAL (P0) VIOLATION**. The system is built exclusively on the Podman rootless container engine for enhanced security and integration with NixOS.

**NixOS-Base Image Policy (ZERO TOLERANCE - SC-CNT-009)**:
All container images MUST be built from a NixOS base. The use of other base images like `alpine`, `ubuntu`, `debian`, or any non-NixOS distribution is a **CRITICAL (P0) VIOLATION**.
...
| Service | URL | Notes |
|---------|-----|-------|
| Phoenix | http://localhost:4001 | Main app |
...
### 2.2 Service Port Registry ($P_{svc}$) - 14-Container Model

**Data & Observability Plane:**
| Service | Container | Port | Protocol | Description |
|---------|-----------|------|----------|-------------|
| PostgreSQL | `indrajaal-db-prod` | 5433 | TCP | Primary Database |
| Observability | `indrajaal-obs-prod` | 3000, 9090, 4317 | HTTP/gRPC | Grafana, Prometheus, OTEL |

**Control Plane (Zenoh Mesh):**
| Service | Container | Port | Protocol | Description |
|---------|-----------|------|----------|-------------|
| Zenoh Router 1 | `zenoh-router-1` | 7447, 8000 | TCP/HTTP | Primary Router |
| Zenoh Router 2 | `zenoh-router-2` | 7448, 8001 | TCP/HTTP | Secondary Router |
| Zenoh Router 3 | `zenoh-router-3` | 7449, 8002 | TCP/HTTP | Tertiary Router |

**Application Plane (HA Cluster):**
| Service | Container | Port | Protocol | Description |
|---------|-----------|------|----------|-------------|
| App Node 1 | `indrajaal-ex-app-1` | 4000, 4001 | HTTP | Primary Node |
| App Node 2 | `indrajaal-ex-app-2` | 4003, 4004 | HTTP | Secondary Node |
| App Node 3 | `indrajaal-ex-app-3` | 4005, 4006 | HTTP | Tertiary Node |

**Cognitive & Digital Twin Plane:**
| Service | Container | Port | Protocol | Description |
|---------|-----------|------|----------|-------------|
| Cortex | `indrajaal-cortex` | 9877 | HTTP | AI Brain |
| CEPAF Bridge | `cepaf-bridge` | 9876 | HTTP | Orchestration Bridge |
| Chaya Twin | `indrajaal-chaya` | 4002 | HTTP | Digital Twin |
| ML Runner 1 | `indrajaal-ml-runner-1` | - | Internal | Satellite 1 |
| ML Runner 2 | `indrajaal-ml-runner-2` | - | Internal | Satellite 2 |
...
## 2.5 SIL-6 Biomorphic Fractal Mesh (v21.2.0)

### 2.5.1 Architecture Overview
The SIL-6 Biomorphic Fractal Mesh extends beyond standard IEC 61508 SIL-4 with biomorphic safety extensions. It provides a "Directed Telescope" for system instrumentation from L1 (Cellular) to L7 (Federation).

**SIL-6 Biomorphic Extensions** (Beyond Standard SIL-4):
| Metric | SIL-4 Requirement | SIL-6 Biomorphic Target |
|--------|-------------------|-------------------------|
| **PFH** | 10⁻⁹ to 10⁻⁸ | < 10⁻¹² |
| **Diagnostic Coverage** | ≥99% | > 99.99% |
| **Safe Failure Fraction** | ≥99% | > 99.9% |
| **Response Time** | <100ms | < 30ms (OODA) |
| **Self-Healing** | Manual recovery | Automatic pattern regeneration |
| **Threat Response** | Alert-based | Neural-immune autonomous |

**Core Concepts**:
| Concept | Description |
|---------|-------------|
| **2oo3 Voting** | 2-out-of-3 consensus: Live node vs Shadow node vs Formal Model |
| **FPPS** | 5-Point Parallel System validation (Pattern, AST, Statistical, Binary, LineByLine) |
| **Quorum** | `floor(N/2)+1` for cluster consensus decisions |
| **Apoptosis** | 6-phase controlled self-destruction protocol |
| **Federation** | Cross-holon communication with version negotiation |
| **Neural-Immune** | PatternHunter + SymbioticDefense + Antibody auto-generation |
| **Biomorphic Healing** | Self-repair from SQLite/DuckDB authoritative state |

### 2.5.2 SIL-4 Mesh CLI Commands
| Command | Effect | STAMP |
|---------|--------|-------|
| `sa-up` | Boot comprehensive 14-container SIL-6 mesh via `sa-mesh` | SC-SIL6-001 |
| `sa-down` | Transactional shutdown of full mesh with checkpointing | SC-SIL4-002 |
| `sa-status` | Detailed health & mesh state for all 14 nodes | SC-SIL4-004 |
| `sa-health` | FPPS 5-point consensus validation | SC-SIL4-005 |
| `sa-clean` | Remove containers (preserve data/kms/) | SC-SIL4-003 |
| `sa-scour` | Nuclear clean (maps to `mesh clean` - destroys all volumes) | SC-SIL4-007 |
| `sa-mesh` | Underlying orchestrator for fine-grained control | SC-SIL6-001 |
| `sa-logs` | Stream logs from any mesh service | SC-SIL4-LOGS |
| `sa-monitor`| Launch Biomorphic Dashboard | SC-SIL4-MON |
| `sa-emergency` | Force stop < 5 seconds | SC-EMR-057 |
| `sa-verify` | 2oo3 voting verification | SC-SIL4-006 |

### 2.5.3 F# Cortex Modules
| Module | Lines | Purpose |
|--------|-------|---------|
| SIL4MeshCLI.fs | 911 | Unified CLI entry point |
| PanopticonOrchestrator.fs | 48 | 5-stage transactional boot |
| HealthCoordinator.fs | 507 | Quorum voting + FPPS consensus |
...
### 2.5.4 STAMP Constraints (SC-SIL4-*)
- **SC-SIL4-001**: Mesh boot MUST complete 5 stages (CRITICAL)
- **SC-SIL4-002**: Shutdown MUST checkpoint state (CRITICAL)
- **SC-SIL4-005**: Health uses FPPS 5-method consensus (CRITICAL)
- **SC-SIL4-006**: 2oo3 voting MANDATORY for production (CRITICAL)
- **SC-SIL4-011**: Quorum = `floor(N/2)+1` (CRITICAL)
- **SC-SIL4-015**: Apoptosis 6-phase protocol (CRITICAL)

### 2.5.5 SIL-6 Biomorphic STAMP Constraints (SC-SIL6-*)
- **SC-SIL6-001**: PFH < 10⁻¹² (beyond SIL-4 maximum) (SUPREME)
- **SC-SIL6-002**: Diagnostic Coverage > 99.99% (SUPREME)
- **SC-SIL6-003**: Safe Failure Fraction > 99.9% (SUPREME)
- **SC-SIL6-004**: Neural-immune response time < 50ms (CRITICAL)
- **SC-SIL6-005**: Symbiotic binding verified every heartbeat (CRITICAL)
- **SC-SIL6-006**: Founder's Directive hardwired in constitutional core (SUPREME)
- **SC-SIL6-007**: Self-healing via pattern regeneration (CRITICAL)
- **SC-SIL6-008**: Substrate-independent survival capability (CRITICAL)
- **SC-SIL6-009**: Federation-scale consensus (CRITICAL)
- **SC-SIL6-010**: Quantum-resistant cryptography (HIGH)
- **SC-SIL6-011**: Biomorphic OODA cycle < 30ms (CRITICAL)
- **SC-SIL6-012**: Triple-modular redundancy for critical paths (CRITICAL)
- **SC-SIL6-013**: Continuous formal verification (HIGH)
- **SC-SIL6-014**: Zero-trust architecture (CRITICAL)
- **SC-SIL6-015**: Immutable audit trail via Immutable Register (SUPREME)

### 2.5.6 Biomorphic Extension Constraints (SC-BIO-EXT-*)
- **SC-BIO-EXT-001**: PatternHunter pre-error detection < 10ms (CRITICAL)
- **SC-BIO-EXT-002**: SymbioticDefense threat response < 100ms (CRITICAL)
- **SC-BIO-EXT-003**: Mara chaos engineering continuous operation (HIGH)
- **SC-BIO-EXT-004**: Antibody auto-generation for novel threats (CRITICAL)
- **SC-BIO-EXT-005**: Sentinel 24/7 health monitoring (SUPREME)
- **SC-BIO-EXT-006**: Neural plasticity for adaptation (HIGH)
- **SC-BIO-EXT-007**: Homeostatic self-regulation (CRITICAL)
- **SC-BIO-EXT-008**: Apoptosis for graceful degradation (CRITICAL)
- **SC-BIO-EXT-009**: Regenerative healing from SQLite/DuckDB (SUPREME)
- **SC-BIO-EXT-010**: Evolution lineage preservation (SUPREME)

### 2.5.7 AOR Rules (AOR-MESH-*)
...
- **AOR-MESH-010**: Emergency < 5s (SC-EMR-057)

### 2.5.8 AOR Rules (AOR-SIL6-*)
- **AOR-SIL6-001**: Verify biomorphic metrics on every deployment
- **AOR-SIL6-002**: Neural-immune system active 24/7
- **AOR-SIL6-003**: Founder's Directive compliance checked every operation
- **AOR-SIL6-004**: Immutable Register logging for all state changes
- **AOR-SIL6-005**: Symbiotic binding verification on heartbeat
- **AOR-SIL6-006**: Pattern regeneration on corruption detection
- **AOR-SIL6-007**: Federation notification on critical events
- **AOR-SIL6-008**: Quantum-safe key rotation scheduled
- **AOR-SIL6-009**: Chaos engineering (Mara) runs weekly
- **AOR-SIL6-010**: Evolution lineage backup daily
...
### 5.2 Liveness Properties (φₗᵢᵥₑₙₑₛₛ)
...
### 5.3 Fairness Properties (φfₐᵢᵣₙₑₛₛ)
...
### Category K: PropCheck Generator Safety (SC-PROP-021 to SC-PROP-028)
...
| SC-PROP-026 | EP-GEN-014: When using both PropCheck and ExUnitProperties, SHALL use `import ExUnitProperties, except: [property: 2, property: 3, check: 2]` | Run `mix validate.ep014` | CRITICAL |
| SC-PROP-027 | EP-GEN-014: When using both frameworks, SHALL add aliases `alias PropCheck.BasicTypes, as: PC` and `alias StreamData, as: SD` | Check for PC/SD aliases | HIGH |
| SC-PROP-028 | Header names in HTTP requests SHALL NOT contain embedded spaces | Run `mix validate.headers` | CRITICAL |
...
**EP-GEN-014 RCA Reference (2025-12-24)**:
- **INCIDENT**: 174 test files with PropCheck/ExUnitProperties conflicts causing compilation failures
- **ROOT CAUSE**: Both frameworks define `property` and `check` macros; no disambiguation
- **FORMAL PROOF**: ComprehensiveErrorModel.agda proves `conflict-implies-failure` invariant
- **SOLUTION**: Use `import ExUnitProperties, except: [property: 2, property: 3, check: 2]`

**EP-GEN-014 Safe Dual Property Testing Template**:
```elixir
use ExUnit.Case, async: true
use PropCheck
...
# PropCheck property (uses PropCheck's property macro)
property "example propcheck property" do
  forall x <- PC.integer() do
    x == x
  end
end

# ExUnitProperties property (uses check all from import)
property "example streamdata property" do
...
**F#/.NET MANDATORY REQUIREMENTS**:
- **SC-NET-001**: All .fsproj files MUST target `<TargetFramework>net10.0</TargetFramework>`
- **SC-NET-002**: global.json MUST specify `"version": "10.0.100"` with `"rollForward": "latestMajor"`
- **SC-NET-003**: devenv.nix MUST use `dotnet-sdk_10` package
- **AOR-NET-001**: Before any F# build, verify target framework is net10.0. Reject net9.0 or earlier.
- Violations of SC-NET-* constraints block CI/CD pipeline.
...
For HMI/TUI cockpit code generation, follow the comprehensive directive:
- **Master Document**: [docs/safety/SAFETY_CRITICAL_DIRECTIVE.md](docs/safety/SAFETY_CRITICAL_DIRECTIVE.md)
- **Compliance**: ISO-13849, IEC 61508 SIL-2, EN 50131, NASA-STD-3000, NUREG-0700
- **Architecture**: Frozen Core (static binaries), Hybrid HAL (Elixir backend, F# frontend)
- **Rendering**: Tiered (Tier1=Kitty GPU, Tier2=Unicode/Braille, Tier3=ASCII fallback)
- **Safety FSM**: Arm & Fire state machine MANDATORY for destructive actions
- **Testing**: FsCheck, StreamData, VHS visual regression, Chaos tests, BDD features
...
### 17.5 BEP Test/Demo Integration & Fractal Testing Framework

The Panopticon SIL-4 Fractal Mesh (BEP v1.0.0) integrates 170+ test and demo scripts across 5 fractal levels.

#### 17.5.1 Script Inventory

| Category | Count | Location | BEP Integration |
|----------|-------|----------|-----------------|
...
#### 17.5.2 5-Level Fractal Coverage Hierarchy
...
#### 17.5.3 Fractal Test Files (test/fractal/)

| File | Level | Coverage |
|------|-------|----------|
| `l1_system_context_test.exs` | L1 | API/Load/Chaos/Security |
| `l2_container_architecture_test.exs` | L2 | Container isolation |
| `l3_domain_architecture_test.exs` | L3 | Domain boundaries |
| `l4_component_architecture_test.exs` | L4 | Module integration |
| `l5_code_architecture_test.exs` | L5 | Code-level tests |

#### 17.5.4 Key Test Scripts (BEP Commands)
...
#### 17.5.5 Key F# Scripts (BEP Commands)
...
#### 17.5.6 BEP Test Workflow (Complete)
...
#### 17.5.7 FPPS 5-Method Integration
...
| LineByLine | `stamp_gde_validation_framework.exs` | Compliance demos | `SIL4Orchestrator.fsx` |

#### 17.5.8 5-Order Effects Matrix
...
#### 17.5.9 Reference Documents

- `docs/guides/TEST_DEMO_INTEGRATION_MATRIX.md` - Complete script inventory
- `docs/testing/FRACTAL_TEST_FRAMEWORK_MASTER_PLAN.md` - Framework specification
- `docs/guides/SIL4_MESH_CLI_USER_GUIDE.md` - CLI operations
...
### 60.10 Migration Complexity Rule
**Operational Rule**: To accommodate the inherent complexity of database migration files, the `Credo.Check.Refactor.CyclomaticComplexity` check is disabled for all files within the `priv/repo/migrations/` directory. For all other files, the cyclomatic complexity limit has been raised to 100. This is a permanent exception to the standard complexity rules.
...
### 61.4 AOR Rule Categories (122 Rules)
...
AORCategories := <|
  ...
  "Holon" -> Range["AOR-HOLON-001", "AOR-HOLON-008"]
|>
```
...
| **Holon** | **AOR-HOLON** | **8** | **Biomorphic state, SQLite/DuckDB, portability** | **SC-HOLON-001-010** |
...
(* Holon State Rules - Biomorphic Fractal Architecture *)
AOR_HOLON := {
  "AOR-HOLON-001" -> O[HolonState, UseSQLite[WALMode] ∧ ¬UsePostgreSQL],
  "AOR-HOLON-002" -> O[HolonHistory, UseDuckDB[AppendOnly] ∧ Columnar],
  "AOR-HOLON-003" -> O[HolonState, FullyPortable ⟹ SingleFileCopy],
  "AOR-HOLON-004" -> O[HolonReplication, UseVersionVector ∧ ConflictFree],
  "AOR-HOLON-005" -> O[HolonFiles, Path == "data/holons/{holon_id}/"],
  "AOR-HOLON-006" -> F[PostgreSQL, StoreHolonState] ∧ O[PostgreSQL, BusinessDataOnly],
  "AOR-HOLON-007" -> O[Analytics, UseDuckDB ∧ EvolutionQueries],
  "AOR-HOLON-008" -> O[Holon, IsolatedState ⟹ (SQLite ∧ DuckDB) PerHolon]
}
...
### 73.1 Mathematical Definition (𝒫ᶜᴬ)
...
### 73.2 Dev Time Execution: The Entropy Fighter
**Objective**: Minimize $\Delta \mathcal{K}$ while maintaining $\Psi$.

**Formal Logic**:
$$ \forall \text{change } c : \text{Apply}(c) \implies (\text{Complexity}(S') \le \text{Complexity}(S) + \epsilon) \wedge \text{Valid}(\Psi, S') $$

### 73.3 Test Time Execution: The Resilience Architect
**Objective**: Maximize Anti-Fragility $\alpha(S)$.

**Formal Logic**:
$$ \Box (\forall \text{stress } \sigma : \text{System}(\sigma) \implies \diamond \text{Recovered}(\text{System})) $$

### 73.4 Runtime Execution: The Intelligent Operator
**Objective**: Minimize OODA Latency ($\delta_{ooda}$) and maintain Homeostasis.

**Formal Logic**:
$$ \text{Homeostasis}(S) \iff \forall m \in \text{Metrics} : \text{Lower}(m) \le \text{Value}(m) \le \text{Upper}(m) $$

### 73.5 The Cybernetic Pledge (Immutable)
...
## 74.0 Cybernetic Architect Execution Framework (CAEF)

### 74.1 Core Cybernetic Parameters
...
### 74.2 TDG Integration Rules (TDG-CA)
...
### 74.3 STAMP Safety Constraints (SC-CA)
...
ModelSelection[task_] := Module[{C = Complexity[task], τₕᵢᵍʰ, τₗₒw},
  ...
  Which[
    C > τₕᵢᵍʰ, "Pro", (* gemini-3-pro-preview, gemini-2.5-pro *)
...
### 75.5 Biomorphic Execution Mandate (2025-12-31)
**Constraint**: The Agent SHALL pursue 100% goal attainment across ALL dimensions:
- **Static Coverage**: Clean Dialyzer + `credo --strict`.
- **Runtime Coverage**: 100% LCOV via `excoveralls`.
- **Mathematical Coverage**: PropCheck invariant verification.
- **Safety (STAMP)**: Map 100% of UCA to verified tests.
- **Resilience (FMEA)**: Mitigate 100% of High RPN failure modes.
- **Transparency**: Directed telescope RCA + Zenoh neural stream verification.
- **Compatibility**: Mandatory Elixir 1.19 / Erlang 28 alignment.
...
### 76.2 Search Tool Mandates (SC-TOOL-001)
...
### 78.9 QUINT Formal Verification Specifications
...
module AgentStateMachine {
  ...
  action init = all {
    agents' = Map(1 -> { id: 1, role: Executive, state: Idle, supervisor: -1, errorCount: 0, currentTask: -1 }),
    taskQueue' = [],
...
(Full Quint specifications synchronized in docs/formal_specs/quint_specifications.qnt)
...
|----------|---------|-------|
| CLAUDE.md | 10.2.0-UNIFIED | 8110 |
| CLAUDE-text.md | 10.2.0-UNIFIED | 7229 |
| GEMINI-text.md | 10.2.0-UNIFIED | 7229 |
| GEMINI-math.md | 10.2.0-UNIFIED | 6072 |

---

## 82.0 Functional Clustering & Alarm Correlation

### 82.1 Clustering Definition
...
## 85.0 Quadplex Observability System (SC-OBS-069)

### 85.1 Core Mandate
The system SHALL use the **Quadplex Logging Strategy** for all operational events.

1.  **Console**: `IO.write` (Immediate feedback)
2.  **File**: `logs/session-*.log` (Durable text record)
3.  **Telemetry**: `:telemetry.execute` (Real-time metrics/SigNoz)
4.  **State Tracker**: `CubDB` (Persistent, queryable state)

### 85.2 Required Modules
*   `Indrajaal.Observability.QuadplexLogger`
...
```elixir
config :logger,
  backends: [Indrajaal.Observability.QuadplexLogger]
...
## 86.0 Track-Based CEPA Architecture (Unified SIL-2)

### 86.1 Track Definitions
The Cybernetic Execution and Performance Architect (CEPA) is bifurcated into two distinct tracks to ensure separation of concerns between application logic and infrastructure orchestration.

1.  **app-elixir-cepa**: The application-level cybernetic components implemented in Elixir.
    *   **Scope**: Health check endpoints, telemetry emission, distributed consensus logic, and in-app verification tasks.
    *   **Goal**: Provide high-fidelity observability and self-healing within the BEAM VM.
2.  **infra-f#-cepa**: The infrastructure-level primary orchestrator implemented in F# (CEPAF# Version 19.0).
    *   **Scope**: Container lifecycle, formal verification gates, OODA build loops, and system-wide benchmarking.
    *   **Goal**: Ensure environmental homeostasis and SIL-2 compliant orchestration.

### 86.2 Standalone DB Verification Suite (SIL-2 Expanded)
The database verification activity (`--db-test`) executes a high-fidelity E2E lifecycle composed of six atomic tasks:
1.  **DB_CREATE**: Orchestration via `podman-compose`.
2.  **DB_SETUP**: Consensus-based health probing (TCP/Log/Functional).
3.  **DB_QUERY**: Engine readiness probe (`pg_isready`).
4.  **DB_PERSISTENCE**: Verified volume integrity via Heartbeat + Container Restart logic.
5.  **DB_TSDB_EXTENSION**: Confirmation of TimescaleDB extension availability.
6.  **DB_HYPERTABLE**: Proactive logic probe ensuring hypertable creation functions.

### 86.3 Track-Specific STAMP Safety Constraints (SC-TRK)
...
| SC-TRK-004 | **infra-f#-cepa** | Orchestrator SHALL NOT use hardcoded host infrastructure references. | CRITICAL |
| SC-TRK-005 | **unified** | All cross-track communication SHALL use registry-defined ports and protocols. | CRITICAL |

### 86.4 Track-Specific TDG Rules (TDG-TRK)

| ID | Track | Rule | Verification |
|----|-------|------|--------------|
| TDG-TRK-001 | **app-elixir-cepa** | Health check logic MUST have corresponding ExUnit unit tests. | `mix test` |
| TDG-TRK-002 | **app-elixir-cepa** | Telemetry emission MUST be verified via `TelemetryTest` helpers. | `mix test` |
| TDG-TRK-003 | **infra-f#-cepa** | Every F# phase MUST have a corresponding Expecto unit test. | `dotnet run --project ...` |
| TDG-TRK-004 | **infra-f#-cepa** | OODA corrective actions MUST be verified via standalone mock build tests. | `Expecto` |

### 86.5 Track-Specific Agent Operating Rules (AOR-TRK)

| ID | Track | Formal Logic | Natural Language |
|----|-------|--------------|------------------|
| AOR-TRK-001 | **app-elixir-cepa** | $\mathbf{O}(\text{StateChange} \implies \text{EmitTelemetry})$ | Agent MUST emit telemetry for every internal state change. |
| AOR-TRK-002 | **infra-f#-cepa** | $\mathbf{O}(\text{Orchestrate} \implies \text{CheckRegistry})$ | Orchestrator MUST use `SystemRegistry` for all target info. |
| AOR-TRK-003 | **unified** | $\mathbf{O}(\text{Report} \implies \text{UseRichReporting})$ | Both tracks MUST use the Version 17.0 Rich Reporting Pattern. |

---

## 87.0 Neuro-Symbolic Simplex Architecture & Cortex Integration

### 87.1 Architecture Overview
The system now implements a **Neuro-Symbolic Simplex Architecture** to enable safe, rapid evolution using AI. This architecture bifurcates control into two planes:

1.  **The Complex Plane (The Cortex)**: High-performance, AI-driven, non-deterministic.
    *   **Components**: `Indrajaal.AI.LocalModel` (Ollama), `Indrajaal.Cortex.Synapse`, Gemini/Claude integration.
    *   **Goal**: Optimization, Code Generation, Heuristic Analysis.
    *   **Trust**: Zero.

2.  **The Safety Plane (The Guardian)**: High-assurance, deterministic, formally verified.
    *   **Components**: `Indrajaal.Safety.Guardian`, STAMP Constraints.
    *   **Goal**: System Stability, Safety Envelope Enforcement.
    *   **Trust**: Absolute.

### 87.2 Cognitive Cockpit: Livebook Integration
**Livebook** is integrated as the "Mind's Eye" of the system, running as a hidden node (`livebook@...`) attached to the cluster.

*   **Access**: Via `scripts/tools/start_livebook.sh`.
*   **Role**: Real-time introspection, OODA loop visualization, and RLHF (Reinforcement Learning from Human Feedback).
*   **Constraint**: Read-only by default; Write access requires multi-signature auth (Two-Key Turn).

### 87.3 Fast OODA Protocol
The **Fast OODA Loop** minimizes the latency of the cognitive cycle ($\delta_{ooda}$):

1.  **Observe**: Zenoh Streams (`indrajaal/logs/**`) $\rightarrow$ Zero-copy ingestion.
2.  **Orient**: Unicon Pattern Scanners $\rightarrow$ Structured Context.
3.  **Decide**: Bicameral AI (Gemini Context + Claude Reasoning) via `Synapse`.
4.  **Act**: CEPAF Container Control via `CepafZenohBridge`.

### 87.4 Neuro-Symbolic Safety Constraints (SC-NEURO)

| ID | Constraint | Severity |
|----|------------|----------|
| SC-NEURO-001 | **Simplex Principle**: The AI's output SHALL NEVER be executed directly; it MUST pass through `Guardian.validate_proposal/1`. | CRITICAL |
| SC-NEURO-002 | **Resource Bounding**: The Guardian SHALL enforce hard resource limits (e.g., max 50 containers) regardless of AI request. | CRITICAL |
| SC-NEURO-003 | **Forbidden Ops**: The Guardian SHALL veto any proposal containing forbidden operations (`rm -rf`, `chmod 777`, etc.). | CRITICAL |
| SC-NEURO-004 | **Shadow Mode**: New AI models MUST run in Shadow Mode for $N$ cycles before promotion to control. | HIGH |
| SC-NEURO-005 | **Dead Man's Switch**: If the Safety Plane loses contact with the Cortex heartbeat (>100ms), it SHALL trigger a failsafe. | CRITICAL |

### 87.5 TDG Rules for Neuro-Symbolic Systems (TDG-NEURO)

| ID | Rule | Verification |
|----|------|--------------|
| TDG-NEURO-001 | **Guardian Verification**: Every new safety constraint MUST be verified by a corresponding ExUnit test in `GuardianTest`. | `mix test` |
| TDG-NEURO-002 | **Adversarial Testing**: The system MUST generate adversarial AI proposals (e.g., "delete all data") to prove the Guardian vetoes them. | Property Test |
| TDG-NEURO-003 | **Deterministic Fallback**: Every Veto MUST result in a deterministic, safe fallback state. | State Machine Test |

### 87.6 Agent Operating Rules (AOR-NEURO)

| ID | Formal Logic | Natural Language |
|----|--------------|------------------|
| AOR-NEURO-001 | $\mathbf{O}(\text{Proposal} \implies \text{GuardianCheck})$ | Agents MUST submit all autonomic proposals to the Guardian. |
| AOR-NEURO-002 | $\mathbf{O}(\text{Veto} \implies \text{LogTrainingData})$ | If a proposal is vetoed, Agents MUST log it to the Negative Training Dataset. |
| AOR-NEURO-003 | $\mathbf{F}(\text{DirectActuation})$ | Agents SHALL NOT bypass the Guardian to access Actuators (CEPAF/DB). |

---

## 88.0 Graph Verification Framework (Mathematical Foundation)

### 88.1 Overview & Purpose

The **Graph Verification Framework (GVF)** provides a mathematically rigorous approach to creating, testing, and verifying graph-structured data and relationships within Indrajaal. This framework unifies five complementary mathematical approaches under a single verification architecture.

```mathematica
(* Graph Verification Framework Definition *)
GVF := Module[{},
  Layers = <|
    "L0_Runtime" -> {"Elixir", "GraphBLAS/Nx", "Ash Validations"},
    "L1_Specification" -> {"Mathematica", "Graph Grammars"},
    "L2_ModelChecking" -> {"Quint", "Alloy-style Relational Logic"},
    "L3_Proofs" -> {"Agda", "MSO Logic", "Dependent Types"}
  |>;

  VerificationGoals = <|
    "Creation" -> "Graph Grammars (Category Theory)",
    "StructuralBugs" -> "Quint (Relational Logic)",
    "AttributeValidation" -> "SHACL/Ash (Description Logic)",
    "FormalProofs" -> "Agda (MSO + Type Theory)",
    "ScaleVerification" -> "GraphBLAS (Linear Algebra)"
  |>;

  <|"Layers" -> Layers, "Goals" -> VerificationGoals|>
]
```

### 88.2 Mathematical Foundations

#### 88.2.1 Graph Theory Preliminaries

```mathematica
(* Graph Type Definitions *)
Graph[G_] := Module[{V, E, σ, τ},
...
LabeledGraph[LG_] := Module[{G, λV, λE},
  G := Graph[_];
  λV : V → LabelV;            (* Vertex labeling *)
  λE : E → LabelE;            (* Edge labeling *)
  <|"G" -> G, "λV" -> λV, "λE" -> λE|>
]

AttributedGraph[AG_] := Module[{LG, attrV, attrE},
  LG := LabeledGraph[_];
  attrV : V → Map[AttrName, Value];   (* Vertex attributes *)
  attrE : E → Map[AttrName, Value];   (* Edge attributes *)
  <|"LG" -> LG, "attrV" -> attrV, "attrE" -> attrE|>
]
```

#### 88.2.2 Domain-Specific Graph Types

```mathematica
(* Indrajaal Graph Types *)
IndrajaalGraphs := <|
  "AgentGraph" -> AttributedGraph[_] /;
    V ⊆ Agent ∧ E ⊆ {supervises, communicates_with, depends_on},

  "ContainerGraph" -> AttributedGraph[_] /;
    V ⊆ Container ∧ E ⊆ {networks_to, mounts, depends_on},

  "ResourceGraph" -> AttributedGraph[_] /;
    V ⊆ AshResource ∧ E ⊆ {belongs_to, has_many, has_one},

  "RoutingGraph" -> AttributedGraph[_] /;
    V ⊆ Component ∧ E ⊆ {routes_to, validates, executes}
|>
```

### 88.3 Graph Grammar Specification (Creation & Evolution)

#### 88.3.1 Production Rule Definition

```mathematica
(* Graph Grammar Production Rule *)
ProductionRule[ρ_] := Module[{L, R, K, l, r, NAC},
  L := Graph[_];              (* Left-hand side pattern *)
  R := Graph[_];              (* Right-hand side replacement *)
  K := Graph[_];              (* Gluing graph - preserved *)
  l : K ↪ L;                  (* Left morphism *)
  r : K ↪ R;                  (* Right morphism *)
  NAC := List[Graph[_]];      (* Negative application conditions *)

  (* Double-Pushout Application *)
  Apply[ρ, G, m] := Module[{D, G'},
    (* Step 1: Compute pushout complement D *)
    D := PushoutComplement[G, L, K, m];
    (* Step 2: Compute pushout G' *)
    G' := Pushout[D, K, R];
    G'
  ]
]
...
#### 88.3.2 Indrajaal Production Rules

```mathematica
(* Agent Spawning Rule *)
ρ_spawn_agent := ProductionRule[<|
  "L" -> Graph[{supervisor: Supervisor}],
  "R" -> Graph[{supervisor: Supervisor, agent: Agent,
                edge: supervises[supervisor, agent]}],
  "K" -> Graph[{supervisor: Supervisor}],
  "NAC" -> {}
|>]

(* Agent Termination Rule *)
ρ_terminate_agent := ProductionRule[<|
  "L" -> Graph[{supervisor: Supervisor, agent: Agent,
                edge: supervises[supervisor, agent]}],
  "R" -> Graph[{supervisor: Supervisor}],
  "K" -> Graph[{supervisor: Supervisor}],
  "NAC" -> {Graph[{agent.status == critical}]}  (* Cannot terminate critical *)
|>]

(* Container Network Link Rule *)
ρ_network_link := ProductionRule[<|
  "L" -> Graph[{c1: Container, c2: Container}],
  "R" -> Graph[{c1: Container, c2: Container,
                edge: networks_to[c1, c2]}],
  "K" -> Graph[{c1: Container, c2: Container}],
  "NAC" -> {Graph[{∃e: networks_to[c1, c2]}]}  (* No duplicates *)
|>]

(* OpenRouter Routing Rule *)
ρ_openrouter_route := ProductionRule[<|
  "L" -> Graph[{synapse: Synapse, openrouter: OpenRouter}],
  "R" -> Graph[{synapse: Synapse, openrouter: OpenRouter,
                edge: routes_to[synapse, openrouter]}],
  "K" -> Graph[{synapse: Synapse, openrouter: OpenRouter}],
  "NAC" -> {Graph[{∃e: routes_to[synapse, external_ai]}]}  (* No direct AI *)
|>]
```

### 88.4 Quint Structural Verification (Alloy-Style)

#### 88.4.1 Model Specification

```mathematica
(* Quint Module Mapping *)
QuintModules := <|
  "OpenRouterIntegration" -> "docs/formal_specs/quint/openrouter_integration.qnt",
  "ContainerNetwork" -> "docs/formal_specs/quint/container_network.qnt",
  "AgentSupervision" -> "docs/formal_specs/quint/agent_supervision.qnt",
  "ResourceGraph" -> "docs/formal_specs/quint/resource_graph.qnt"
|>

(* Invariant Mapping *)
QuintInvariants := <|
  "INV-OR-001" -> "inv_openrouter_exclusivity",    (* SC-GDE-060 *)
  "INV-OR-002" -> "inv_synapse_routes_to_openrouter",
  "INV-OR-003" -> "inv_simplex_principle",          (* SC-NEURO-001 *)
  "INV-OR-004" -> "inv_confidence_threshold",       (* SC-GDE-061 *)
  "INV-OR-005" -> "inv_no_forbidden_executed",      (* SC-NEURO-003 *)
  "INV-SUP-001" -> "inv_single_supervisor",
  "INV-SUP-002" -> "inv_no_cycles",
  "INV-NET-001" -> "inv_network_isolation",
  "INV-NET-002" -> "inv_app_db_connectivity"
|>
```

#### 88.4.2 Counter-Example Search

```mathematica
(* Counter-Example Generation Protocol *)
CounterExampleSearch := Module[{},
  (* For each invariant, attempt to find violation *)
  Protocol = {
    "1. Initialize valid state via init action",
    "2. Apply non-deterministic step",
    "3. Check invariant holds",
    "4. If violation found, report counter-example",
    "5. If no violation after bounded search, property holds"
  };

  (* Execution Command *)
  Command = "quint run <spec>.qnt --invariant=<inv> --max-samples=10000"
]
```

### 88.5 SHACL/Ash Attribute Validation

#### 88.5.1 Shape Definition

```mathematica
(* SHACL Shape Mapping to Ash Validations *)
SHACLShape[resource_] := Module[{properties, constraints},
  properties = <|
    "path" -> AttributeName,
    "datatype" -> AshType,
    "minCount" -> MinCardinality,
    "maxCount" -> MaxCardinality,
    "pattern" -> RegexPattern,
    "in" -> AllowedValues,
    "nodeKind" -> ReferentialIntegrity
  |>;

  constraints = <|
    "SPARQLConstraint" -> CustomValidation,
    "ClassConstraint" -> ResourceTypeCheck
  |>
]

(* Indrajaal Resource Shapes *)
ResourceShapes := <|
  "UserShape" -> <|
    "email" -> <|"datatype" -> String, "pattern" -> EmailRegex, "minCount" -> 1|>,
    "tenant_id" -> <|"datatype" -> UUID, "minCount" -> 1, "nodeKind" -> IRI|>,
    "roles" -> <|"minCount" -> 0, "nodeKind" -> IRI|>
  |>,

  "AlarmShape" -> <|
    "priority" -> <|"datatype" -> Integer, "minInclusive" -> 1, "maxInclusive" -> 5|>,
    "status" -> <|"in" -> {active, acknowledged, resolved, archived}|>,
    "device" -> <|"minCount" -> 1, "nodeKind" -> IRI|>
  |>,

  "OpenRouterCallShape" -> <|
    "tier" -> <|"in" -> {fast, smart, deep}|>,
    "token_count" -> <|"datatype" -> Integer, "minInclusive" -> 1|>,
    "model" -> <|"pattern" -> OpenRouterModelPattern|>,
    "confidence" -> <|"datatype" -> Float, "minInclusive" -> 0.0, "maxInclusive" -> 1.0|>
  |>
|>
```

### 88.6 MSO Logic & Agda Proofs

#### 88.6.1 Monadic Second-Order Logic

```mathematica
(* MSO Formula Syntax *)
MSOFormula := Module[{},
  (* First-Order *)
  FOL = {edge[x, y], x == y, ¬φ, φ ∧ ψ, φ ∨ ψ, φ → ψ, ∀x.φ, ∃x.φ};
  ...
  (* Key Properties *)
  Properties = <|
    "Connectivity" -> ∀X.(∃v.v ∈ X ∧ ∃v.v ∉ X) → (∃u.∃v.u ∈ X ∧ v ∉ X ∧ edge[u,v]),
    "Acyclicity" -> ¬∃v.TransitiveClosure[edge][v, v],
    "SingleSupervisor" -> ∀a.(a ≠ executive → ∃!s.supervises[s, a])
  |>
]

(* Courcelle's Theorem Application *)
CourcelleTheorem := "For any MSO-definable property P and constant k,
  P can be decided in O(n) time on graphs with treewidth ≤ k"

IndrajaalApplication := <|
  "SupervisionTree" -> <|"Treewidth" -> 1, "Properties" -> {"Connectivity", "Acyclicity"}|>,
  "ContainerNetwork" -> <|"Treewidth" -> 3, "Properties" -> {"Isolation", "Reachability"}|>
|>
```

#### 88.6.2 Agda Proof Modules

```mathematica
(* Agda Module Structure *)
AgdaModules := <|
  "GraphProperties" -> "docs/formal_specs/agda/GraphProperties.agda",
  "SupervisionProofs" -> "docs/formal_specs/agda/SupervisionProofs.agda",
  "OpenRouterGraphProofs" -> "docs/formal_specs/agda/OpenRouterGraphProofs.agda",
  "AcyclicityProofs" -> "docs/formal_specs/agda/AcyclicityProofs.agda"
|>

(* Key Theorems *)
AgdaTheorems := <|
  "FQUN-Uniqueness" -> "∀ fqun₁ fqun₂. fqun₁ ≡ fqun₂ ⟺ components_equal",
  "Supervision-Connected" -> "∀ a ∈ Agents. a ≢ executive → reachable(executive, a)",
  "OpenRouter-NoBypass" -> "∀ external. routes_to(synapse, external) → external ≡ openrouter",
  "Guardian-Mandatory" -> "∀ proposal exec. routes_to(proposal, exec) → routes_to(proposal, guardian)"
|>
```

### 88.7 GraphBLAS High-Performance Verification

#### 88.7.1 Matrix Representation

```mathematica
(* Graph-to-Matrix Mapping *)
AdjacencyMatrix[G_] := Module[{n, A},
  n := |V[G]|;
  A := SparseArray[{}, {n, n}, 0];
  Do[A[[σ[e], τ[e]]] = 1, {e, E[G]}];
  A
]

(* Semiring Operations *)
Semirings := <|
  "Boolean" -> <|"⊕" -> Or, "⊗" -> And, "0" -> False, "1" -> True|>,
  "Tropical" -> <|"⊕" -> Min, "⊗" -> Plus, "0" -> ∞, "1" -> 0|>,
  "Counting" -> <|"⊕" -> Plus, "⊗" -> Times, "0" -> 0, "1" -> 1|>
|>

(* Transitive Closure *)
TransitiveClosure[A_] := Module[{R, n},
  n := Dimensions[A][[1]];
  R := A;
  Do[R = R ∨ (R . R), {Log2[n]}];  (* Matrix squaring *)
  R
]
```

#### 88.7.2 Verification Operations

```mathematica
(* GraphBLAS Operations for Verification *)
GraphBLASVerification := <|
  "CycleDetection" -> Function[A,
    closure = TransitiveClosure[A];
    AnyTrue[Diagonal[closure]]  (* Cycle if any self-loop in closure *)
  ],

  "Reachability" -> Function[{A, src, dst},
    closure = TransitiveClosure[A];
    closure[[src, dst]] == 1
  ],

  "ShortestPath" -> Function[{W, src, dst},
    (* Tropical semiring Floyd-Warshall *)
    D = W;
    n = Dimensions[W][[1]];
    Do[D = Min[D, Outer[Plus, D[[;;, k]], D[[k, ;;]]]], {k, n}];
    D[[src, dst]]
  ],

  "ComponentCount" -> Function[A,
    (* BFS-based connected components *)
    Length[ConnectedGraphComponents[AdjacencyGraph[A]]]
  ]
|>

(* Performance Constraints *)
PerformanceTargets := <|
  "CycleDetection_1000nodes" -> "< 100ms",
  "Reachability_10000nodes" -> "< 500ms",
  "TransitiveClosure_1000nodes" -> "< 200ms"
|>
```

### 88.8 STAMP Safety Constraints (SC-GVF)

```mathematica
(* Graph Verification Framework Safety Constraints *)
SC_GVF := {
  ...
  "SC-GVF-004" -> O[System, ContainerNetwork ⟹ SatisfiesIsolationInvariant],
  "SC-GVF-005" -> O[System, GraphBLASVerification ⟹ CompleteIn100ms],
  "SC-GVF-006" -> O[CI, CounterExampleSearch ⟹ RunInPipeline],
  "SC-GVF-007" -> O[System, MSOProperty ⟹ HasAgdaProof],
  "SC-GVF-008" -> O[Agent, GraphTransformation ⟹ PreservesCriticalInvariants]
}
```

| ID | Constraint | Severity | Verification |
|----|------------|----------|--------------|
| SC-GVF-001 | Graph grammar rules MUST be verified in Quint before deployment | CRITICAL | CI/CD |
| SC-GVF-002 | All Ash resources MUST have corresponding SHACL shapes | HIGH | Compile-time |
| SC-GVF-003 | Supervision graph MUST be proven acyclic in Agda | CRITICAL | Proof check |
| SC-GVF-004 | Container network MUST satisfy isolation invariant | CRITICAL | Quint model |
| SC-GVF-005 | GraphBLAS verification MUST complete in < 100ms | HIGH | Performance test |
| SC-GVF-006 | Counter-example generation MUST run in CI pipeline | HIGH | CI/CD |
| SC-GVF-007 | MSO properties MUST have corresponding Agda proofs | HIGH | Proof check |
| SC-GVF-008 | Graph transformations MUST preserve critical invariants | CRITICAL | Quint + Runtime |

### 88.9 TDG Rules for Graph Verification (TDG-GVF)

```mathematica
(* Test-Driven Generation Rules *)
TDG_GVF := {
  "TDG-GVF-001" -> "Production rule test MUST fail before implementation",
  "TDG-GVF-002" -> "Quint invariant test MUST find counter-example for buggy spec",
  "TDG-GVF-003" -> "SHACL shape test MUST reject invalid attributes",
  "TDG-GVF-004" -> "Agda proof MUST type-check before merge",
  "TDG-GVF-005" -> "GraphBLAS performance test MUST pass threshold",
  "TDG-GVF-006" -> "Dual property test (PropCheck + StreamData) for generators"
}
```

| ID | Rule | Verification |
|----|------|--------------|
| TDG-GVF-001 | Production rule test MUST fail before grammar implementation | `mix test` (red phase) |
| TDG-GVF-002 | Quint invariant test MUST find counter-example for intentionally buggy spec | `quint run --run=test_*_violation` |
| TDG-GVF-003 | SHACL shape test MUST reject invalid attribute combinations | `mix test test/graph/shacl_*` |
| TDG-GVF-004 | Agda proof MUST type-check successfully before merge | `agda --check` |
| TDG-GVF-005 | GraphBLAS performance test MUST pass 100ms threshold for 1000 nodes | `mix test --only performance` |
| TDG-GVF-006 | Generators MUST use dual property testing (PropCheck + StreamData) | EP-GEN-014 compliance |

### 88.10 Agent Operating Rules (AOR-GVF)

```mathematica
(* Agent Operating Rules for Graph Verification *)
AOR_GVF := {
  "AOR-GVF-001" -> O[Agent, NewGraphRule ⟹ WriteQuintSpecFirst],
  "AOR-GVF-002" -> O[Agent, ResourceCreate ⟹ ValidateSHACLShape],
  "AOR-GVF-003" -> O[Agent, CriticalInvariant ⟹ ProveInAgda],
  "AOR-GVF-004" -> F[Agent, DeployWithoutQuintVerification],
  "AOR-GVF-005" -> O[Agent, ScaleGraph ⟹ UseGraphBLAS],
  "AOR-GVF-006" -> O[Agent, GraphMutation ⟹ CheckInvariants],
  "AOR-GVF-007" -> F[Agent, BypassGuardianForGraphChanges]
}
```

| ID | Formal Logic | Natural Language |
|----|--------------|------------------|
| AOR-GVF-001 | $\mathbf{O}(\text{NewRule} \implies \text{QuintFirst})$ | Agent MUST write Quint spec before implementing new graph rule |
| AOR-GVF-002 | $\mathbf{O}(\text{Create} \implies \text{ValidateShape})$ | Agent MUST validate SHACL shape on resource creation |
| AOR-GVF-003 | $\mathbf{O}(\text{Critical} \implies \text{AgdaProof})$ | Agent MUST prove critical invariants in Agda |
| AOR-GVF-004 | $\mathbf{F}(\text{DeployWithoutQuint})$ | Agent SHALL NOT deploy graph changes without Quint verification |
| AOR-GVF-005 | $\mathbf{O}(\text{Scale} \implies \text{GraphBLAS})$ | Agent MUST use GraphBLAS for large-scale graph verification |
| AOR-GVF-006 | $\mathbf{O}(\text{Mutate} \implies \text{CheckInv})$ | Agent MUST check invariants after any graph mutation |
| AOR-GVF-007 | $\mathbf{F}(\text{BypassGuardian})$ | Agent SHALL NOT bypass Guardian for graph structure changes |

### 88.11 Verification Pipeline Integration

```mathematica
(* Graph Verification Pipeline *)
GVFPipeline := Module[{},
  Stages = {
    "1. Grammar Check" -> "Is transformation legal? (Category Theory)",
    "2. SHACL Validation" -> "Are attributes valid? (Description Logic)",
    "3. Quint Model Check" -> "Are structural invariants satisfied? (Relational Logic)",
    "4. GraphBLAS Performance" -> "Does verification complete in time? (Linear Algebra)",
    "5. Agda Certification" -> "Is the property proven? (Type Theory)"
  };

  (* Decision Flow *)
  Flow = "
    GraphChange
      → GrammarCheck → (Fail: Reject)
      → SHACLValidate → (Fail: Reject)
      → QuintVerify → (Fail: Report Counter-Example)
      → GraphBLASPerf → (Slow: Optimize)
      → AgdaCertify → (Pass: Deploy)
  "
]
```

### 88.12 Command Reference

```bash
# Quint Model Checking
quint run docs/formal_specs/quint/openrouter_integration.qnt --invariant=all_invariants
quint run docs/formal_specs/quint/openrouter_integration.qnt --run=test_bypass_violation
...
### 88.13 Related Documentation

| Document | Purpose |
|----------|---------|
| `docs/architecture/GRAPH_VERIFICATION_FRAMEWORK.md` | Full architectural specification |
| `docs/formal_specs/quint/openrouter_integration.qnt` | Quint model for OpenRouter integration |
| `docs/formal_specs/agda/` | Agda proof files |
| `docs/testing/openrouter-comprehensive-test-plan.md` | Test plan with L6 Graph Verification |

---

## 89.0 Indrajaal Knowledge Engine (IKE) Specification (SC-IKE)

### 89.1 Architectural Mandate
The **Indrajaal Knowledge Engine (IKE)** is the **Central Nervous System** of the architecture. It transforms static documentation into a **Complex Adaptive System (CAS)**.
*   **Version**: 4.0.0-OMNIPRESENT
*   **Role**: State of Truth, Build Gatekeeper, Runtime Observer.
*   **Backend**: F# (Logic) + DuckDB (Storage) + Vector Search (Semantics).

### 89.2 The Holonic Schema (4-Layer Metadata)
Every artifact (Note, Agent, Service) is a **Holon** defined by:

1.  **Fractal Layer (Topology)**:
    *   `holon_level`: `atomic` (Fact), `molecular` (Concept), `organism` (Module), `ecosystem` (Domain).
    *   `fractal_path`: Breadcrumb (e.g., `Core.Auth.JWT`).
2.  **Evolutionary Layer (Time)**:
    *   `entropy_score`: ($0.0 - 1.0$) Probability of obsolescence.
    *   `drift_score`: Divergence between Model (Docs) and Territory (Runtime).
3.  **Richness Layer (Semantics)**:
    *   `rhetorical_function`: `axiom`, `hypothesis`, `evidence`.
    *   `vectors`: High-dimensional embeddings.
4.  **Actionable Layer (Utility)**:
    *   `stamp_constraints`: List of applied SC-*.
    *   `agent_comments`: AI modification log.

### 89.3 Omnipresent Integration Points

| Phase | Hook Component | Action | Entropy Effect |
|-------|----------------|--------|----------------|
| **Build** | `IKE.BuildHook` | Verify STAMP constraints in source. | Update `last_modified` |
| **Test** | `IKE.TestListener` | Verify functional correctness. | Pass $\to$ Lower Entropy; Fail $\to$ Max Entropy |
| **Deploy** | `IKE.DeployGate` | Block artifacts with $S > 0.2$. | Gatekeeper |
| **Runtime**| `IKE.TelemetryBridge`| Consumes Zenoh metrics. Calculates Drift. | Update `drift_score` |

### 89.4 The Ouroboros Loop (Self-Healing)
1.  **Anomaly**: Runtime Telemetry detects failure/drift.
2.  **Rot**: `TelemetryBridge` marks Holon as "Rotting" ($S=1.0$).
3.  **Curiosity**: High Entropy triggers AI Agent (`Synapse`).
4.  **Correction**: AI generates fix via OpenRouter (Oracle Mode).
5.  **Validation**: Tests pass ($S \downarrow$).
6.  **Evolution**: `DeployGate` approves new version.

### 89.5 STAMP Safety Constraints (SC-IKE)

| ID | Constraint | Severity |
|----|------------|----------|
| SC-IKE-001 | **Zero-Touch**: IKE SHALL automatically ingest/classify all new artifacts. | HIGH |
| SC-IKE-002 | **Entropy Gating**: System SHALL NOT deploy artifacts with Entropy > 0.2. | CRITICAL |
| SC-IKE-003 | **Drift Detection**: Runtime observations deviating from Docs SHALL increase Entropy. | HIGH |
| SC-IKE-004 | **Genetic Payload**: Every runtime process MUST carry its Holon Identity (UUID). | CRITICAL |
| SC-IKE-005 | **Oracle Routing**: High-complexity queries MUST be routed to Tier 3 Models (OpenRouter). | MEDIUM |
| SC-IKE-006 | **Immutable Audit**: All knowledge graph changes MUST be logged to LSM Trees. | HIGH |
| SC-IKE-007 | **Feedback Loop**: Test results MUST directly impact the Entropy Score of the tested Holon. | CRITICAL |
| SC-IKE-008 | **Schema Compliance**: All Markdown frontmatter MUST validate against the Holonic Schema. | HIGH |

### 89.6 Agent Operating Rules (AOR-IKE)

| ID | Formal Logic | Natural Language |
|----|--------------|------------------|
| AOR-IKE-001 | $\mathbf{O}(\text{Change} \implies \text{UpdateGraph})$ | Agent MUST update the Knowledge Graph (reset entropy) on code change. |
| AOR-IKE-002 | $\mathbf{O}(\text{Query} \implies \text{VectorSearch})$ | Agent MUST query IKE Vectors before answering user questions. |
| AOR-IKE-003 | $\mathbf{F}(\text{HallucinateConstraints})$ | Agent SHALL NOT invent constraints; retrieve from `fhkb.duckdb`. |
| AOR-IKE-004 | $\mathbf{O}(\text{Rot} \implies \text{Refactor})$ | If Agent encounters High Entropy code, it MUST propose a refactor. |
| AOR-IKE-005 | $\mathbf{O}(\text{Unknown} \implies \text{Oracle})$ | If confidence < 0.8, Agent MUST consult OpenRouter (Oracle). |

---

## 90.0 End of Specification

**Final Formal Assertion**: This document constitutes the complete, exhaustive, and mathematically rigorous specification of the Indrajaal Safety-Critical System (v20.0.0).

$\forall \text{Action } a, (a \notin \text{GEMINI.md}) \implies (a \text{ is Forbidden})$

**Document Compiled By**: Gemini (Cybernetic Architect)
**Status**: VERIFIED COMPLETE
...
## 94.0 OpenRouter 7-Level Fractal Architecture (SC-OR-7L)

### 94.1 Architecture Overview
The OpenRouter integration follows the system's fractal design, ensuring intelligence is available at every scale, from atomic functions to the global federation. It implements the **Bicameral Mind** pattern, where deterministic code (Safety Plane) controls non-deterministic AI (Complex Plane).

### 94.2 The 7 Levels
| Level | Scope | Component | Responsibility | Safety Constraint |
|---|---|---|---|---|
| **L1** | **Atomic** | `OpenRouterClient.chat/2` | Raw HTTP/SSE execution, Headers | SC-OR-L1-001 (Timeout) |
| **L2** | **Component** | `ProviderDispatcher` | Retry, Fallback, Streaming, Normalization | SC-OR-L2-001 (Resilience) |
| **L3** | **Holon** | `Synapse`, `Guardian` | Context assembly, Safety Verification, OODA | SC-OR-L3-001 (Simplex) |
| **L4** | **Container** | `indrajaal-app`, `cepaf` | Runtime env, Secret injection, Podman Bridge | SC-OR-L4-001 (Isolation) |
| **L5** | **Node** | `PricingCache` | Budget enforcement, Local caching, Rate Limit | SC-OR-L5-001 (Cost) |
| **L6** | **Mesh** | `ZenohEvolutionPublisher` | Distributed Telemetry, Hive Mind Analysis | SC-OR-L6-001 (Observability) |
| **L7** | **Federation** | `ModelRegistry` | Global Model Selection, Drift Detection | SC-OR-L7-001 (Alignment) |

### 94.3 Implementation Specifications

#### L1: Atomic (The Client)
*   **File**: `lib/indrajaal/ai/open_router_client.ex`
*   **Function**: Wraps `Req.post` with standardized headers (`HTTP-Referer`, `X-Title`).
*   **Constraint**: MUST handle `429 Too Many Requests` with exponential backoff (Retry-After header).

#### L2: Component (The Dispatcher)
*   **File**: `lib/indrajaal/ai/provider_dispatcher.ex`
*   **Logic**: Implements the `fallback_chain` (OpenRouter -> Anthropic Direct -> Mock).
*   **Stream**: Transforms SSE chunks into internal `Message` structs for uniform consumption.

#### L3: Holon (The Guardian Interface)
*   **File**: `lib/indrajaal/safety/guardian.ex`
*   **Rule**: **Pre-Flight Check**. No request leaves L3 without `Guardian.validate_proposal/1`.
*   **Vector**: `Synapse` uses OpenRouter to "orient" itself in the OODA loop (Observe -> **Orient** -> Decide -> Act).

#### L4: Container (CEPAF Bridge)
*   **File**: `lib/cepaf/src/Cepaf.Knowledge/OpenRouter.fs`
*   **Role**: F# Orchestrator uses OpenRouter to analyze container failures (`sa-test` output).
*   **Bridge**: Shared secrets via `podman-compose.yml` environment injection.

#### L5: Node (Cost Control)
*   **File**: `lib/indrajaal/ai/pricing_cache.ex`
*   **Logic**: Token counting and soft/hard limits per node.
*   **Action**: Cuts circuit (`CircuitBreaker`) if daily budget exceeded ($5.00 default).

#### L6: Mesh (The Hive Mind)
*   **Topic**: `indrajaal/evolution/openrouter/calls`
*   **Behavior**: All nodes publish anonymized prompts/responses to Zenoh.
*   **Analytics**: `indrajaal-obs` aggregates mesh-wide token usage and latency metrics.

#### L7: Federation (Evolution)
*   **Concept**: Knowledge Engine (IKE).
*   **Action**: High-entropy artifacts (confusing code) are auto-submitted to OpenRouter for refactoring proposals.
*   **Drift**: Monitors model drift (e.g., GPT-4 vs Claude 3.5 performance on canonical benchmarks).
...
*   **Check Balance**: `mix openrouter.budget`
*   **Test Connectivity**: `mix openrouter.ping`
*   **Trace Request**: `mix openrouter.trace <req_id>`

**Safety Warning**:
> **AOR-OR-001**: NEVER log raw OpenRouter responses containing PII or Secrets to disk. Use the `RedactedLogger` pipeline.

## 95.0 OpenRouter System-Wide Integration Audit (Review)

### 95.1 The Dual-Stack Integration Strategy
The system implements a **Dual-Stack Cognitive Architecture**, allowing both the Runtime Orchestrator (F#) and the Application Core (Elixir) to access high-level intelligence independently but cohesively.

### 95.2 Stack A: The Application Cortex (Elixir/BEAM)
**Primary Gateway**: `Indrajaal.AI.OpenRouterClient`
**Safety Mechanism**: `Guardian.validate_proposal/1` (Simplex Architecture)

| Component | Module | Purpose | Criticality |
|---|---|---|---|
| **Synapse** | `Indrajaal.Cortex.Synapse` | The central "Brain". Routes complex queries to OpenRouter when local heuristics fail. | **High** |
| **FastOODA** | `Indrajaal.Cortex.FastOODA` | Real-time "Orientation". Uses AI to make sense of rapid sensor data streams (20ms timeout). | **Critical** |
| **GDE Engine** | `Indrajaal.Cortex.GDE.AIIntegration` | Code Generation. Generates mutation proposals for the system's own evolution. | **High** |
| **KMS Oracle** | `Indrajaal.KMS.AI` | Knowledge Retrieval. Answering user questions based on RAG data. | **Medium** |
| **Web Summarizer** | `Indrajaal.KMS.WebKnowledge` | Data Ingestion. Summarizing scraped web content for the Knowledge Graph. | **Low** |
| **Neuro-Spine** | `Indrajaal.Cockpit.Prajna.Neuro.Spine` | Escalation. The "Spine" decides when to escalate a reflex to the "Brain" (OpenRouter). | **Critical** |

### 95.3 Stack B: The Infrastructure Cortex (F#/.NET)
**Primary Gateway**: `Cepaf.Knowledge.OpenRouter` (Compiled) & `OpenRouterAI` (Scripts)
**Safety Mechanism**: `SIL4Orchestrator.fsx` (2oo3 Voting)

| Component | File/Module | Purpose | Criticality |
|---|---|---|---|
| **Deployment Medic** | `ProductionDeploymentOrchestrator.fsx` | Analyzes deployment failures and suggests fixes during `sa-deploy`. | **High** |
| **Test Swarm** | `ComprehensiveRuntimeTests.fsx` | Analyzes runtime test failures to categorize them (Flake vs Bug). | **Medium** |
| **Runtime Validator** | `FractalRuntimeValidator.fsx` | Validates system state against natural language intent. | **High** |
| **Cockpit Copilot** | `Cepaf.Cockpit.AiCopilot` | Provides AI assistance directly in the TUI Dashboard. | **Low** |

### 95.4 Observability & Telemetry (The Hive Mind)
All AI interactions, regardless of Stack, converge on the **Zenoh Data Bus** for unified analysis.

*   **Topic**: `indrajaal/evolution/openrouter/calls`
*   **Schema**: `{model: string, tokens: int, latency: ms, cost: float, source: "elixir" | "fsharp"}`
*   **Storage**: `Indrajaal.AI.PricingCache` (aggregates costs daily).

### 95.5 Compliance Checklist (SC-OR-AUDIT)
- [x] **API Key Isolation**: Keys are injected via env vars (`OPENROUTER_API_KEY`), never hardcoded.
- [x] **Rate Limiting**: Elixir side uses `ProviderDispatcher` with backoff. F# side uses `Polly` retry policies.
- [x] **Model Governance**: Both stacks default to `anthropic/claude-3.5-sonnet` for consistency.
- [x] **Cost Tracking**: Centralized tracking via Zenoh streams ensures no "hidden costs" in the F# layer.
