# SIL-4 Mesh CLI User Guide
**Version**: 21.2.0-SIL6
**Date**: 2026-01-11
**Compliance**: SC-SIL4-*, AOR-MESH-*

---

## 1.0 Overview

The SIL-4 Mesh CLI provides unified command interface for the Panopticon SIL-4 Fractal Mesh architecture. All mesh operations follow the **Biomorphic Evolutionary Plan (BEP)** and implement 5-stage transactional boot with FPPS consensus validation.
...
#### sa-up - Boot Mesh
Starts the **SIL-6 Biomorphic Fractal Mesh** (14 Containers) via the comprehensive orchestrator.
...
Shows container health and mesh state for all 14 nodes.
...
| `SIL4Orchestrator.fsx` | SIL-4 compliance testing |
...
|----|------------|---------|
| SC-SIL4-001 | 5-stage boot | `sa-up` |
| SC-SIL4-002 | Checkpoint shutdown | `sa-down` |
| SC-SIL4-003 | Preserve data/kms/ | `sa-clean` |
| SC-SIL4-004 | Status < 30s | `sa-status` |
| SC-SIL4-005 | FPPS consensus | `sa-health` |
| SC-SIL4-006 | 2oo3 voting | `sa-verify` |
| SC-SIL4-007 | Nuclear clean | `sa-scour` |
| SC-SIL4-011 | Quorum N/2+1 | (automatic) |
| SC-SIL4-015 | Apoptosis 6-phase | (automatic) |
...
The SIL-4 Mesh CLI integrates with the Prajna C3I Cockpit:
