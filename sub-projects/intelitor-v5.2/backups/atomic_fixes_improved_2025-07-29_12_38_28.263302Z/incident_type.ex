defmodule Intelitor.Alarms.IncidentType do
  @moduledoc """
  Defines types of security incidents and their response protocols.

  IncidentTypes provide a categorization system for alarm events, defining
  severity levels, response procedures, and notification requirements for
  ...
  attributes do
    ...
    attribute :requires_verification?, :boolean do
      allow_nil? false
      public? true
      default true
    end
  ...
  relationships do
    has_many :alarm_events, Intelitor.Alarms.AlarmEvent
    has_many :workflow_templates, Intelitor.Alarms.WorkflowTemplate
  end
  ...
  actions do
    ...
    update :remove_sia_code do
      ...
      change fn changeset, _context ->
        ...
        updated_codes = Enum.reject(current_codes, &(&1 == code_to_remove))
  ...
  calculations do
    ...
    calculate :requires_immediate_response?, :boolean do
      calculation fn records, _context ->
        values =
          Enum.map(records, fn incident_type ->
