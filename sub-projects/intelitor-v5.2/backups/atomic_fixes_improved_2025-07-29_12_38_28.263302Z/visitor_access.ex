defmodule Intelitor.VisitorManagement.VisitorAccess do
  @moduledoc """
  ...
  relationships do
    ...
    belongs_to :visitor_pass, Intelitor.VisitorManagement.VisitorPass do
      attribute_writable? true
    end

    belongs_to :authorizing_escort, Intelitor.Accounts.User do
      attribute_writable? true
    end

    belongs_to :location_from_ref, Intelitor.Sites.Location do
      attribute_writable? true
    end

    belongs_to :location_to_ref, Intelitor.Sites.Location do
      attribute_writable? true
    end
  end
  ...
  calculations do
    calculate :duration_at_location, :integer do
      calculation fn records, _ ->
        # This would typically be calculated based on consecutive access events
        # For now, we'll return nil as it requires complex logic
  ...
  actions do
    ...
    create :log_access_event do
      ...
      change set_attribute(:access_event_type, arg(:event_type))
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:tenant_id, :security_alert_triggered], where: "security_alert_triggered = true"
      index [:tenant_id, :alert_level]
      index [:tenant_id, :incident_reported], where: "incident_reported = true"
      index [:tenant_id, :follow_up_required], where: "follow_up_required = true"
