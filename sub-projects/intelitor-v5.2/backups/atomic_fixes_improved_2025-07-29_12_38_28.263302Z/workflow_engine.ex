defmodule Intelitor.Alarms.WorkflowEngine do
  @moduledoc """
  ...
  defp execute_human_decision_step(step, instance, alarm) do
  ...
  defp execute_action(%{type: :lock_down_area} = action, alarm, _instance) do
    ...
    :telemetry.execute(
      [:intelitor, :workflow, :area_lockdown],
      %{count: 1},
  ...
  defp execute_action(%{type: :start_video_recording} = action, alarm, _instance) do
    Logger.info("Starting video recording for alarm #{alarm.id}", %{
      alarm_id: alarm.id,
      site_id: alarm.site_id,
      camera_selection: action.params[:cameras]
    })

    # Emit telemetry for video action
    :telemetry.execute(
      [:intelitor, :workflow, :video_recording],
      %{count: 1},
      %{alarm_id: alarm.id, site_id: alarm.site_id}
    )
    ...
    #   :area_cameras -> Video.list_cameras_for_area(alarm.site_id, alarm.zone_id)
  ...
  defp execute_action(%{type: :dispatch_security} = action, alarm, _instance) do
    ...
    :telemetry.execute(
      [:intelitor, :workflow, :security_dispatch],
      %{count: 1, priority_level: priority_to_level(action.params[:priority])},
      %{alarm_id: alarm.id, site_id: alarm.site_id}
    )
  ...
  defp execute_action(%{type: :notify_stakeholders} = action, alarm, _instance) do
    ...
    :telemetry.execute(
      [:intelitor, :workflow, :stakeholder_notification],
      %{count: 1},
      %{alarm_id: alarm.id, template: action.params[:template]}
    )
  ...
  defp execute_action(%{type: :activate_sirens} = _action, alarm, _instance) do
  ...
  defp evaluate_condition(%{type: :alarm_severity} = condition, alarm, _instance) do
  ...
  defp evaluate_condition(%{type: :time_of_day} = condition, _alarm, _instance) do
    ...
    case condition.operator do
      :between -> current_hour >= condition.start_hour && current_hour <= condition.end_hour
      :outside -> current_hour < condition.start_hour || current_hour > condition.end_hour
    end
  end

  defp evaluate_condition(%{type: :location_type} = _condition, _alarm, _instance) do
          ...
          parallel_steps: [
            ...
            %{
              ...
              action: %{
                ...
                params: %{service: :fire_department}
              }
            }
          ]
        }
      ...
      steps: [
        ...
        %{
          ...
          action: %{
            ...
            params: %{template: :medical_emergency}
          }
        }
      ]
    }
        ...
        %{
          ...
          parallel_steps: [
            %{
              ...
              action: %{
                type: :dispatch_security,
                params: %{priority: :critical, silent: true}
              }
            },
            %{
              ...
              action: %{
                type: :start_video_recording,
                params: %{cameras: :area_cameras, mode: :covert}
              }
            },
            %{
              id: "alert_police",
              ...
              action: %{
                type: :call_emergency_services,
                params: %{service: :police, silent: true}
              }
            }
          ]
        }
    ...
    %{
      ...
      steps: [
        %{
          ...
          action: %{
            type: :dispatch_security,
            params: %{priority: :high, type: :investigation}
          }
        }
      ]
    }
  ...
  defp create_decision_request(step, instance, _alarm) do
    # This would create an actual decision request
    {:ok,
     %{
       id: Ecto.UUID.generate(),
       ...
       assigned_to: step.decision.assigned_to,
       timeout_at: DateTime.add(DateTime.utc_now(), step.decision.timeout, :second)
     }}
  end
  ...
  # defp get_stakeholders(_alarm) do
  ...
  defp update_loop_state(instance, loop_id, loop_state) do
    %{instance | context: Map.put(instance.context, "loop_#{loop_id}", loop_state)}
  end
