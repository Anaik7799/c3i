defmodule Intelitor.Dispatch.Assignment do
  @moduledoc """
  ...
  relationships do
    ...
    belongs_to :officer, Intelitor.Dispatch.Officer do
      attribute_public? true
    end
    ...
    belongs_to :route, Intelitor.Dispatch.Route do
      attribute_public? true
    end
  ...
  actions do
    ...
    update :en_route do
      ...
      change fn changeset, _context ->
        ...
        |> Ash.Changeset.force_change_attribute(:en_route_at, DateTime.utc_now())
      end
    end
    ...
    update :transfer do
      ...
      change fn changeset, _context ->
        ...
        if new_team_id = changeset.arguments.new_team_id do
          changeset = Ash.Changeset.force_change_attribute(changeset, :team_id, new_team_id)
        end

        if new_officer_id = changeset.arguments.new_officer_id do
          changeset = Ash.Changeset.force_change_attribute(changeset, :officer_id, new_officer_id)
        end
    ...
    update :add_communication do
      ...
      change fn changeset, _context ->
        ...
        changeset
        |> Ash.Changeset.force_change_attribute(:communication_log, [new_entry | log])
        |> Ash.Changeset.force_change_attribute(:last_communication, DateTime.utc_now())
      end
    end
    ...
    update :calculate_cost do
      ...
      change fn changeset, _context ->
        ...
        duration = Ash.Changeset.get_attribute(changeset, :on_scene_duration_seconds)
  ...
  calculations do
          ...
          Enum.map(
            ...
            fn assignment ->
              assignment.estimated_arrival_time &&
                DateTime.compare(DateTime.utc_now(), assignment.estimated_arrival_time) == :gt &&
    ...
    calculate :requires_supervisor_review?, :boolean do
      calculation fn records, _context ->
        values =
          Enum.map(
            records,
            fn assignment ->
              assignment.status == :completed &&
                ...
                ((assignment.performance_rating && assignment.performance_rating < 7) ||
                   (assignment.customer_satisfaction && assignment.customer_satisfaction < 3))
            end
          )
  ...
  defp generate_assignment_number(changeset) do
    # Generate assignment number like ASG-20251206-001
