defmodule Intelitor.Maintenance.Schedule do
  @moduledoc """
  Represents recurring maintenance schedules for equipment and facilities.

  Schedules define preventive maintenance programs with recurring patterns,
  ...
  relationships do
    ...
    belongs_to :preferred_technician, Intelitor.Accounts.User do
      attribute_public? true
    end

    has_many :work_orders, Intelitor.Maintenance.WorkOrder do
      source_attribute :id
  ...
  actions do
    ...
    create :create do
      ...
      change fn changeset, _context ->
        start_date = Ash.Changeset.get_argument_or_attribute(changeset, :start_date)
    ...
    update :record_completion do
      ...
      change fn changeset, _context ->
        ...
        completed = Ash.Changeset.get_attribute(changeset, :completed_work_orders)
        ...
        changeset
        |> Ash.Changeset.force_change_attribute(:last_completed_date, completion_date)
        |> Ash.Changeset.force_change_attribute(:next_due_date, next_due)
        |> Ash.Changeset.force_change_attribute(:completed_work_orders, completed + 1)
    ...
    update :update_usage do
      ...
      change fn changeset, _context ->
        ...
        trigger_value = Ash.Changeset.get_attribute(changeset, :usage_trigger_value)
    ...
    update :add_blackout_date do
      ...
      change fn changeset, _context ->
        blackouts = Ash.Changeset.get_attribute(changeset, :blackout_dates) || []
          ...
          Enum.map(
            ...
            fn schedule ->
              if schedule.schedule_type == :usage_based && schedule.usage_trigger_value do
                schedule.current_usage_value / schedule.usage_trigger_value * 100.0
              else
                nil
              end
            end
          ...
          Enum.map(
            ...
            fn schedule ->
              if schedule.total_work_orders > 0 do
                schedule.completed_work_orders / schedule.total_work_orders * 100.0
              else
                nil
              end
            end
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:preferred_technician_id], where: "preferred_technician_id IS NOT NULL"
  ...
  defp calculate_next_due_date(base_date, changeset) do
    pattern = Ash.Changeset.get_argument_or_attribute(changeset, :recurrence_pattern)
    interval = Ash.Changeset.get_argument_or_attribute(changeset, :recurrence_interval) || 1
