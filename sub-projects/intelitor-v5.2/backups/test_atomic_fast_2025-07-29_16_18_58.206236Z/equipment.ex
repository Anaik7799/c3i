defmodule Intelitor.Maintenance.Equipment do
  @moduledoc """
  ...
  history, performance metrics, and lifecycle management. They support preventive
  ...
  attributes do
    ...
    attribute :maintenance_frequency, :atom do
      public? true
    ...
    attribute :inspection_frequency_months, :integer do
      public? true
    ...
    attribute :data_collection_frequency, :atom do
      public? true
  ...
  relationships do
    ...
    belongs_to :successor_equipment, Intelitor.Maintenance.Equipment do
      attribute_public? true
    end

    has_many :work_orders, Intelitor.Maintenance.WorkOrder
    has_many :schedules, Intelitor.Maintenance.Schedule
  ...
  actions do
    ...
    create :create do
      ...
      change fn changeset, _context ->
        commissioning_date =
          Ash.Changeset.get_argument_or_attribute(changeset, :commissioning_date)

        service_interval =
          Ash.Changeset.get_argument_or_attribute(changeset, :service_interval_days)
        ...
        changeset =
          ...
          |> Ash.Changeset.force_change_attribute(:operational_status, :operational)
    ...
    update :update_usage do
      ...
      change fn changeset, _context ->
        ...
        current_hours_total = Ash.Changeset.get_attribute(changeset, :operating_hours_total)
        current_hours_ytd = Ash.Changeset.get_attribute(changeset, :operating_hours_ytd)
        current_cycles_total = Ash.Changeset.get_attribute(changeset, :cycles_total)
        ...
        |> Ash.Changeset.force_change_attribute(:last_usage_reading, DateTime.utc_now())
      end
    end
    ...
    update :record_service do
      ...
      change fn changeset, _context ->
        ...
        service_interval = Ash.Changeset.get_attribute(changeset, :service_interval_days)
        ...
        changeset =
          changeset
          |> Ash.Changeset.force_change_attribute(:last_service_date, service_date)
          |> Ash.Changeset.force_change_attribute(
    ...
    update :record_failure do
      ...
      change fn changeset, _context ->
        ...
        current_failures = Ash.Changeset.get_attribute(changeset, :failure_count_ytd)
        current_downtime = Ash.Changeset.get_attribute(changeset, :downtime_hours_ytd)
        ...
        changeset
        |> Ash.Changeset.force_change_attribute(:failure_count_ytd, current_failures + 1)
        |> Ash.Changeset.force_change_attribute(
          :downtime_hours_ytd,
    ...
    update :update_costs do
      ...
      change fn changeset, _context ->
        ...
        current_maintenance_cost = Ash.Changeset.get_attribute(changeset, :maintenance_cost_ytd)
    ...
    update :schedule_inspection do
      ...
      change fn changeset, _context ->
        ...
        frequency = Ash.Changeset.get_attribute(changeset, :inspection_frequency_months)

        next_inspection =
          if frequency do
            # Approximate
            Date.add(inspection_date, frequency * 30)
          else
            nil
          end
    ...
    update :add_spare_part do
      ...
      change fn changeset, _context ->
        parts = Ash.Changeset.get_attribute(changeset, :critical_spare_parts) || []
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:successor_equipment_id], where: "successor_equipment_id IS NOT NULL"
      ...
      index [:service_contract_expires], where: "service_contract_expires IS NOT NULL"
      ...
      index [:expected_retirement_date], where: "expected_retirement_date IS NOT NULL"
    end
  end
end
