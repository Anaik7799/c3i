defmodule Indrajaal.Alarms.ProcessingEngine do
  ...
  """
  @spec handle_api_event(any(), any()) :: any()
  ...
  def handle_call({:process_alarm, device_event}, _from, state) do
    ...
    result =
      with {:ok, alarm} <- create_alarm_event(device_event),
           ...
           :ok <- EscalationEngine.trigger_for_alarm(alarm) do
  ...
  def handle_call({:handle_sia_event, binary_data}, _from, state) do
    result =
      with {:ok, parsed} <- parse_sia_event(binary_data),
           ...
           # Future: {:ok, device} <- Devices.get_device_by_account_number(pars
        ...
        device_event = %{
          ...
          _location_id: device._location_id,
  ...
  @spec create_alarm_event(term()) :: term()
  defp create_alarm_event(device_event) do
    attrs = %{
      ...
      site_id: get_site_from_location(device_event[:_location_id]),
      zone_id: get_zone_from_location(device_event[:_location_id]),
  ...
  @spec generate_event_code(term()) :: term()
  ...
  @spec calculate_initial_priority(term()) :: term()
  ...
  @spec generate_description(term()) :: term()
  defp generate_description(device_event) do
    "#{String.capitalize(to_string(device_event.event_type))} alarm from
      device #{device_event.device_id}"
  end

  @spec check_storm_conditions(term(), term()) :: term()
  defp check_storm_conditions(tenant_id, state) do
    ...
    if DateTime.diff(DateTime.utc_now(), state.last_storm_check) > 30 do
      _result = StormDetection.detect_storm(tenant_id)
    end

    :ok
  end

  @spec record_processing_metrics(term(), term()) :: term()
  ...
  @spec parse_sia_event(term()) :: term()
  defp parse_sia_event(_binarydata) do
  ...
  @spec normalize_sia_event(term(), term()) :: term()
  ...
  @spec map_sia_to_incident_type(term()) :: term()
  ...
  @spec validate_and_get_device(term(), term()) :: term()
  defp validate_and_get_device(device_id, tenant_id) do
    ...
    device = %{
      ...
      tenant_id: tenant_id,
      status: :online,
      _location_id: Ecto.UUID.generate()
    }
  ...
  @spec get_incident_type(term()) :: term()
  ...
  @spec get_site_from_location(term()) :: term()
  ...
  @spec get_zone_from_location(term()) :: term()
  ...
  @spec get_device_by_account_number(term(), term()) :: term()
