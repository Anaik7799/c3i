defmodule Indrajaal.VisitorManagement.VisitRequest do
  @moduledoc """
  Visit _requests with scheduling, approval workflows, and access _requirements.
  """
  ...
  attributes do
    ...
    attribute :_request_id, :string do
      allow_nil? false
    ...
    attribute :_requested_areas, {:array, :uuid} do
      default []
    end
    ...
    attribute :_request_status, :atom do
      constraints one_of: [
                    :submitted,
                    :under_review,
                    :approved,
  ...
  relationships do
    ...
    belongs_to :_requesting_employee, Indrajaal.Accounts.User do
      allow_nil? false
      attribute_writable? true
    end
    ...
    has_many :approvals, Indrajaal.VisitorManagement.VisitApproval do
      destination_attribute :visit_request_id
    end

    has_one :visitor_pass, Indrajaal.VisitorManagement.VisitorPass do
      destination_attribute :visit_request_id
    end
    ...
    calculate :is_overdue_for_approval, :boolean do
      calculation fn records, _ ->
        ...
        Enum.map(
          records,
          fn record ->
            case {record._request_status, record.approval_deadline} do
              {status, deadline}
    ...
    calculate :is_visit_active, :boolean do
      calculation fn records, _ ->
        ...
        Enum.map(
          records,
          fn record ->
            case record._request_status do
              :approved ->
                arrival_passed =
                  DateTime.compare(
                    now,
  ...
  actions do
    ...
    create :submit_request do
      argument :_request_id, :string do
        allow_nil? false
      end
      ...
      argument :_requesting_employee_id, :uuid do
        allow_nil? false
      end
      ...
      change set_attribute(:_request_id, arg(:_request_id))
      ...
      change set_attribute(
               :_requesting_employee_id,
               arg(:_requesting_employee_id)
             )
    ...
    update :update_status do
      ...
      change set_attribute(:_request_status, arg(:new_status))
    end

    update :approve_request do
      require_atomic? false
      change set_attribute(:_request_status, :approved)
    end
    ...
    update :reject_request do
      ...
      change set_attribute(:_request_status, :rejected)
    ...
    update :cancel_request do
      ...
      change set_attribute(:_request_status, :cancelled)
    ...
    update :record_departure do
      ...
      change set_attribute(:_request_status, :completed)
    end
    ...
    update :set_compliance_requirements do
      ...
      argument :_requirements, {:array, :string} do
        allow_nil? false
      end

      change set_attribute(:compliance_requirements, arg(:_requirements))
    end

    update :_require_security_briefing do
      require_atomic? false
  ...
  code_interface do
    ...
    define :_require_security_briefing
  end
  ...
  postgres do
    ...
    custom_indexes do
      index [:tenant_id, :_request_id], unique: true
      index [:tenant_id, :visitor_id]
      index [:tenant_id, :_requesting_employee_id]
      index [:tenant_id, :site_id]
      index [:tenant_id, :_request_status]
      ...
      index [:tenant_id, :approval_deadline],
