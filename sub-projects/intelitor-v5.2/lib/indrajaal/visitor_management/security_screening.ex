defmodule Indrajaal.VisitorManagement.SecurityScreening do
  @moduledoc """
  ...
  attributes do
    ...
    attribute :_requested_date, :date do
      allow_nil? false
  ...
  relationships do
    ...
    belongs_to :_requested_by, Indrajaal.Accounts.User do
      allow_nil? false
      attribute_writable? true
    end

    belongs_to :conducted_by, Indrajaal.Accounts.User do
      attribute_writable? true
    end
  ...
  calculations do
    ...
    calculate :screening_duration_days, :integer do
      calculation fn records, _ ->
        Enum.map(records, fn record ->
          case {record.completed_date, record._requested_date} do
            {completed, _requested} when not is_nil(completed) ->
              Date.diff(completed, _requested)
  ...
  actions do
    ...
    create :initiate_screening do
      ...
      argument :_requested_by_id, :uuid do
        allow_nil? false
      end
      ...
      change set_attribute(:_requested_by_id, arg(:_requested_by_id))
    ...
    update :escalate_screening do
      ...
      change set_attribute(:screening_status, :_requires_escalation)
...
\n\nCLEARANCE EXTENDED: #{reason}"
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:tenant_id, :_requested_date]
      ...
      index [:tenant_id, :clearance_expiry_date],
        where: "clearance_expiry_date IS NOT NULL"

      index [:tenant_id, :risk_assessment_score],
        where: "risk_assessment_score IS NOT NULL"

      index [:tenant_id, :appeals_process_initiated],
        where: "appeals_process_initiated = true"
    end
  end
end
