defmodule Indrajaal.VisitorManagement.ContractorManagement do
  @moduledoc """
  Extended contractor management with project tracking and certification
    _requirements.
  """
  ...
  relationships do
    ...
    belongs_to :project_manager, Indrajaal.Accounts.User do
      attribute_writable? true
    end

    belongs_to :safety_officer, Indrajaal.Accounts.User do
      attribute_writable? true
    end
  end
  ...
  actions do
    ...
    update :update_certifications do
      ...
      argument :_required_certifications, {:array, :string} do
        allow_nil? false
      end
...
\n\nSAFETY INCIDENT: #{incident_details}"
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:tenant_id, :project_start_date],
        where: "project_start_date IS NOT NULL"

      index [:tenant_id, :project_end_date],
      ...
      index [:tenant_id, :performance_rating],
