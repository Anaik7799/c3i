defmodule Indrajaal.Billing.Invoice do
  @moduledoc """
          ...
          Enum.map(
            ...
            fn invoice ->
              if Decimal.compare(
                ...
                ratio = Decimal.div(invoice.amount_paid, invoice.total_amount)
                Decimal.mult(ratio, 100)
              else
                Decimal.new(0)
              end
            end
  ...
  # Helper functions
  @spec generate_invoice_number(term()) :: term()
  defp generate_invoice_number(changeset) do
    # Generate invoice number like INV - 20_251_206 - 001
    today = Date.utc_today()
    date_str = today |> Date.to_string() |> String.replace("-", "")

    random_value = :rand.uniform(999)
  ...
  defp calculate_totals(changeset) do
    ...
    total_amount =
      subtotal |> Decimal.sub(discount_amount) |> Decimal.add(tax_amount)
