defmodule Indrajaal.Core.AuditLog do
  ...
  attributes do
    ...
    attribute :metadata, :map do
      default %{}
      description "Additional __context (IP, user agent, etc.)"
    end

    attribute :ip_address, :string do
      # IPv6 max length
      constraints max_length: 45
      description "IP address of the __request"
    end
    ...
    attribute :__request_id, :uuid do
      description "Correlation ID for the __request"
    end
  ...
  calculations do
    ...
    calculate :duration_ms, :integer do
      calculation fn records, _ ->
        # Would calculate from metadata if __request timing is stored
        Enum.map(records, fn record ->
  ...
  defmodule Logger do
    ...
    @spec log(term(), term(), binary() | integer(), term(), keyword() | map()) :: term()
    def log(actor, action, resource_type, resource_id, opts \\ []) do
      params = %{
        actorid: actor[:id],
      ...
      Indrajaal.Core.AuditLog.log(params, actor: %{is_system: true})
    end
  end
end
