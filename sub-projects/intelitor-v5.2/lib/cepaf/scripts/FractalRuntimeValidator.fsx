...
module TestCases =
    ...
    let c1_redis_embedded : FractalTestCase = {
        ...
        Execute = fun () -> async {
            let sw = Stopwatch.StartNew()
            match! Shell.exec "podman" "exec indrajaal-ex-app-1 redis-cli ping" with
    ...
    let c2_flame_enabled : FractalTestCase = {
        ...
        Execute = fun () -> async {
            let sw = Stopwatch.StartNew()
            match! Shell.exec "podman" "exec indrajaal-ex-app-1 printenv FLAME_ENABLED" with
    ...
    let c2_clustering_enabled : FractalTestCase = {
        ...
        Execute = fun () -> async {
            let sw = Stopwatch.StartNew()
            match! Shell.exec "podman" "exec indrajaal-ex-app-1 printenv CLUSTERING_ENABLED" with
            | Ok output when output.Trim() = "true" ->
                sw.Stop()
    ...
    let c3_dark_mode : FractalTestCase = {
        ...
        Execute = fun () -> async {
            let sw = Stopwatch.StartNew()
            match! Shell.exec "podman" "exec indrajaal-ex-app-1 printenv PRAJNA_DARK_MODE" with
            | Ok output when output.Trim() = "true" ->
                sw.Stop()
...
let printBanner () =
    ...
    printfn "╠══════════════════════════════════════════════════════════════════════════╣"
    printfn "║  Architecture: indrajaal-db-prod | indrajaal-obs-prod | indrajaal-ex-app-1║"
