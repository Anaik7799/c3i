...
// Cockpit project: FFI integration pending (uses log-only mode currently).
// =============================================================================
...
module ZenohNativeConfig =
    ...
    /// Native mode is always active. Cockpit will use FFI when available.
    let useNative () = true
...
type SafeSession private (sessionId: string, config: SessionConfig, isSimulated: bool) =
    ...
    /// Native-only: Cockpit project uses log-only mode pending FFI integration.
    static member OpenAsync(config: SessionConfig) : Task<ZenohResult<SafeSession>> =
        task {
            try
                // Cockpit: FFI bridge integration pending — uses log-only session
...
and [<Sealed>] SafePublisher private (session: SafeSession, keyExpr: string) =
    ...
    /// Publish bytes — native-only mode (Cockpit: log-only pending FFI).
    member this.PutAsync(payload: byte[]) : Task<ZenohResult<unit>> =
        task {
            if disposed then
                ...
                try
                    // Cockpit: FFI pending — increment counter (ZenohPublish logs)
