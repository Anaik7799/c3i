...
type BoundedBuffer(config: BoundedBufferConfig) =
    ...
    /// Publish buffer telemetry using dual-write: log fallback + best-effort Zenoh (AOR-FFI-006, SC-ZTEST-008)
    /// Cockpit project uses log-only mode pending FFI integration (see ZenohNative.fs).
    let publishTelemetry (m: BufferMetrics) =
        if config.EnableTelemetry then
            try
                let payload =
                    sprintf """{"name":"%s","count":%d,"capacity":%d,"dropped":%d,"backpressure":%b,"status":"%s","timestamp":"%s"}"""
                        config.Name
                        m.CurrentCount
                        config.MaxCapacity
                        (int m.RejectedBufferFullCount)
                        m.Status.ShouldApplyBackpressure
                        (m.Status.ToString())
                        (DateTimeOffset.UtcNow.ToString("o"))
                // SC-ZTEST-008: Always write log fallback first
                eprintfn "[ZTEST-CHECKPOINT] topic=indrajaal/cockpit/buffer/metrics payload=%s" payload
            with _ ->
                () // Telemetry failures MUST NOT affect buffer operation (SC-CIRCUIT-001)
