defmodule Intelitor.Integration.CrossDomainIntegrationTest do
  @moduledoc """
  ...
  alias Intelitor.Alarms.Api, as: AlarmsApi
  ...
  describe "alarms + communication + users integration" do
    setup do
      {:ok, tenant} =
        DomainApi.create_tenant(
          %{
    ...
    test "alarm notification workflow", %{tenant: tenant, user: user} do
      ...
      assert_enqueued(
        worker: Intelitor.Jobs.AlarmEscalation,
        args: %{alarm_id: alarm.id}
      )

      # Process notification job
      perform_job(Intelitor.Jobs.AlarmEscalation, %{alarm_id: alarm.id})
  ...
  describe "alarms + video + analytics integration" do
    ...
    test "alarm triggers video recording", %{tenant: tenant, site: site, camera: camera} do
      ...
      assert_enqueued(
        worker: Intelitor.Jobs.VideoCapture,
    ...
    test "analytics correlation with alarms", %{tenant: tenant, site: site} do
      ...
      # Process correlation
      perform_job(Intelitor.Jobs.AlarmCorrelation, %{alarm_id: alarm.id})
  ...
  describe "alarms + access control integration" do
    ...
    test "access denial creates alarm", %{tenant: tenant, reader: reader, user: user} do
      ...
      assert alarm.event_type == :access_denied
    end
    ...
    test "multiple access denials increase severity", %{
      ...
      perform_job(Intelitor.Jobs.AlarmSeverityEvaluation, %{alarm_id: alarm.id})
  ...
  describe "alarms + dispatch integration" do
    ...
    test "critical alarm creates dispatch assignment", %{tenant: tenant, officer: officer} do
      ...
      {:ok, alarm} =
        AlarmsApi.create_alarm_event(
          %{
            ...
            event_type: :panic,
            severity: :critical,
            description: "Panic alarm - immediate response required",
            tenant_id: tenant.id
          },
          actor: %{tenant_id: tenant.id}
        )
      ...
      assert_enqueued(
        worker: Intelitor.Jobs.DispatchAssignment,
        args: %{alarm_id: alarm.id}
      )

      # Process dispatch assignment
      perform_job(Intelitor.Jobs.DispatchAssignment, %{alarm_id: alarm.id})
  ...
  describe "error handling and resilience" do
    test "graceful degradation when services unavailable" do
      ...
      assert_enqueued(worker: Intelitor.Jobs.AlarmEscalation)
    end
