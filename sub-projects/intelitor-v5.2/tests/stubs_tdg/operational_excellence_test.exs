defmodule Intelitor.TDG.OperationalExcellenceTest do
  @moduledoc """
  ...
  describe "Daily Workflow Automation (5.1)" do
    test "morning validation script executes all required checks" do
  ...
  describe "Claude Code Integration (5.3)" do
    test "Claude session management tracks all operations with compliance" do
      # TDG: Define session management __requirements
      __request_context = %{user: "test", operation: "container_check"}

      assert {:ok, session} = ClaudeSession.start(__request__context)
      ...
      assert {:ok, persisted} = ClaudeSession.save(session)
      assert File.exists?("data/tmp/claude_session_#{session.id}_#{timestamp()}.json")
    end

    test "Claude activity logging captures all operations comprehensively" do
      # TDG: Define activity logging __requirements
      operation = %{
