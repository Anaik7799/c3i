defmodule Intelitor.Load.PerformanceLoadTest do
  @moduledoc """
  Comprehensive load testing for the Intelitor Security Platform.

  Tests system performance under various load conditions:
  - High user concurrency
  - Large dataset operations
  - Bulk data processing
  - Real - time __event handling
  ...
  # import Intelitor.PerformanceHelpers  # Module doesn't exist
      ...
      bulk_operation = fn count ->
        ...
        results =
          Enum.map(user_params_list, fn params ->
            Intelitor.Accounts.create_user(params, %{tenant_id: tenant.id})
          end)
          ...
          Task.async(fn ->
            ...
            case :rand.uniform(4) do
              1 ->
                ...
                Intelitor.Devices.list_devices(%{type: type}, %{tenant_id: tenant.id})

              2 ->
                # Filter by status
                status = Enum.random([:online, :offline, :maintenance])
                Intelitor.Devices.list_devices(%{status: status}, %{tenant_id: tenant.id})

              3 ->
                # Get random device
                device = Enum.random(devices)
                Intelitor.Devices.get_device(device.id, %{tenant_id: tenant.id})

              4 ->
                # Search devices
                Intelitor.Devices.search_devices(%{name: "Camera"}, %{tenant_id: tenant.id})
            end
          end)
        end)
  ...
  describe "Real - time Event Processing Load Tests" do
    test "handles high - f_requency alarm events" do
      tenant = insert(:tenant)
      devices = Intelitor.Factory.insert_list(50, :device, tenant: tenant)

      # Generate high - f_requency alarm events
      event_count = 500
        ...
        |> Enum.each(fn {batch, _index} ->
          ...
          batch_tasks =
            Enum.map(batch, fn event_params ->
              Task.async(fn ->
                Intelitor.Alarms.create_alarm_event(event_params, %{tenant_id: tenant.id})
              end)
            end)
      ...
      {:ok, stored_events} = Intelitor.Alarms.list_alarm_events(%{}, %{tenant_id: tenant.id})
    ...
    test "maintains system responsiveness under event load" do
      ...
      # Test system responsiveness during __event load
      responsiveness_tests =
        Enum.map(1..10, fn _i ->
          ...
          operations = [
            fn -> Intelitor.Accounts.get_user(user.id, %{tenant_id: tenant.id}) end,
            fn -> Intelitor.Devices.list_devices(%{}, %{tenant_id: tenant.id}) end,
            fn -> Intelitor.Alarms.list_alarm_events(%{limit: 10}, %{tenant_id: tenant.id}) end
          ]
  ...
  describe "Database Performance Load Tests" do
    test "maintains query performance with large datasets" do
      ...
      query_patterns = [
        {:simple_filter,
         fn ->
           Intelitor.Accounts.list_users(%{role: :operator}, %{tenant_id: tenant.id})
         end},
        {:complex_filter,
         fn ->
           Intelitor.Accounts.search_users(
             %{
               role: :operator,
               active: true,
        ...
        {:join_query,
         fn ->
           Intelitor.Accounts.get_user_teams(
        ...
        {:aggregation,
         fn ->
           Intelitor.Alarms.get_alarm_statistics(
             %{
  ...
  defp benchmark_concurrent_operations(_tenant, operation, opts) do
    concurrency = Keyword.get(opts, :concurrency, 10)
    ops_per_process = Keyword.get(opts, :operations_per_process, 10)
    ...
    results =
      ...
      |> Task.async_stream(
        fn process_id ->
          operations =
            Enum.map(1..ops_per_process, fn op_id ->
              op_start = System.monotonic_time(:microsecond)
              result = operation.(process_id, op_id)
              op_end = System.monotonic_time(:microsecond)
              %{result: result, duration: op_end - op_start}
            end)

          %{process_id: process_id, operations: operations}
        end,
    ...
    total_operations = concurrency * ops_per_process

    all_durations = Enum.flat_map(results, fn r -> Enum.map(r.operations, & &1.duration) end)
    avg_duration = if length(all_durations) > 0, do: Enum.sum(all_durations) / length(all_durations), else: 0

    %{
      results: results,
      total_operations: total_operations,
      throughput_ops_per_second: total_operations * 1_000_000 / total_duration,
      performance_stats: %{
        avg_operation_time_us: avg_duration
      }
    }
  end

  # Create large test dataset
  defp create_large_test_dataset(tenant, opts) do
    base_count = Keyword.get(opts, :base_count, 10)
    multiplier = Keyword.get(opts, :multiplier, 1)
    count = base_count * multiplier

    users = Enum.map(1..count, fn i ->
      insert(:user, tenant: tenant, email: "dataset_user_#{i}@test.com", active: true)
    end)

    %{users: users, tenant: tenant}
  end
end
