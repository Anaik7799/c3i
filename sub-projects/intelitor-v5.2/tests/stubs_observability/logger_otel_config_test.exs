defmodule Intelitor.Observability.LoggerOtelConfigTest do
  @moduledoc """
  ...
  STAMP Safety: SC1 (Pr_event data loss), SC2 (Ensure proper initialization)
  """
  use ExUnit.Case
  ...
  describe "OTEL configuration" do
    ...
    test "ensures OTEL resource attributes are properly set" do
      ...
      assert resource[:service][:name] == "intelitor"
    ...
    test "ensures trace __context propagation is configured" do
      ...
      propagators = Application.get_env(:opentelemetry, :propagators, [:trace_context])
    ...
    test "ensures sampling configuration is set" do
      ...
      valid_sampler? =
        case sampler do
          nil -> true
          {:always_on, _opts} -> true
          {:always_off, _opts} -> true
          {:probability, _opts} -> true
          _ -> false
        end

      assert valid_sampler?, "Invalid sampler configuration: #{inspect(sampler)}"
    end
  end
  ...
  describe "Phoenix instrumentation" do
    test "ensures OpenTelemetry Phoenix is configured" do
      ...
      assert config == [:intelitor, :endpoint]
  ...
  describe "Ash telemetry integration" do
    test "ensures custom Ash telemetry is configured" do
      ash_config = Application.get_env(:intelitor, :ash_telemetry)

      assert ash_config[:enabled] == true
      assert ash_config[:trace_all_actions] == true
      assert ash_config[:include_metadata] == true
      ...
      if env_sampler do
        ...
        case env_sampler do
          ...
          "probability" ->
            ...
            if env_ratio do
              {ratio, _rest} = Float.parse(env_ratio)
              assert elem(sampler_config, 1)[:probability] == ratio
            end
        end
      end
    end
  ...
  describe "STAMP safety validation" do
    test "ensures telemetry doesn't lose data on shutdown" do
      # SC1: Pr_event data loss
    ...
    test "ensures all instrumentation libraries initialize properly" do
      ...
      assert Application.get_env(:intelitor, :ash_telemetry) != nil
    end
  end
end
