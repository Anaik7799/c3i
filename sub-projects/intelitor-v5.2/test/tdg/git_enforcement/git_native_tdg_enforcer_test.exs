defmodule GitNativeTdgEnforcerTest do
  ...
  describe "git integration" do
    test "validates git repository state" do
      # Test git repository validation
      {output, _exit_code} = System.cmd("git", ["status", "--porcelain"])
    ...
    test "checks for staged files" do
      ...
      {output, _exit_code} = System.cmd("git", ["diff", "--cached", "--name-only"])
      assert is_binary(output)
    end
  end

  # Helper function to test main execution
  defp assert_received_main(_args) do
