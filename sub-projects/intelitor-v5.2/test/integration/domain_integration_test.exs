defmodule Indrajaal.DomainIntegrationTest do
  ...
  alias Indrajaal.Accounts.{User, Team}
  alias Indrajaal.Policy.{Role, Permission, UserRole}
  alias Indrajaal.Sites.{Site, Building, Zone}
  alias Indrajaal.Devices.{Device, Camera}
  alias Indrajaal.Alarms.AlarmEvent
  alias Indrajaal.Integrations.{Webhook, ApiConnection}
  ...
  describe "cross-domain integration" do
    ...
    test "complete security monitoring workflow", %{
      ...
      # These queries should only return __data for the correct tenant
    ...
    test "multi-tenant __data isolation", %{tenant: tenant, organization: organization} do
      # Create __data for first tenant
      site1 = insert(:site, tenant: tenant, organization: organization)
      device1 = insert(:device, tenant: tenant, site: site1)

      # Create second tenant with its own __data
      ...
      # Verify tenant1 can only see its own __data
      ...
      # Verify tenant2 can only see its own __data
