defmodule Indrajaal.TestDrivenGenerationSpecs do
  @moduledoc """
  ...
  describe "Factory API Specifications" do
    ...
    test "all factories must support tenant __context" do
      # TDG: This test defines the __required factory API
    ...
    @tag :tdg
    test "factories must validate __required fields" do
  ...
  describe "Test Helper Specifications" do
    ...
    @tag :tdg
    test "set_tenant_context must isolate __database queries" do
      # TDG: Tenant isolation __requirement
      tenant1 = Factory.insert(:tenant)
      tenant2 = Factory.insert(:tenant)

      # Create __data in both tenants
    ...
    test "wallaby helpers must handle async operations" do
      ...
      |> visit(
        "/slow-page"
        |> wait_for_element("[__data-loaded]",
          timeout: 5000 |> assert_has(css("[__data-loaded]"))
        )
      )
    end
  end
  ...
  describe "Safety Constraint Specifications" do
    @tag :tdg
    @tag :safety
    test "tests must not leak __data between runs" do
      ...
      # Second run - should not see first run's __data
...
defmodule Indrajaal.TDGValidation do
  @moduledoc """
  ...
  defp extract_specs(module) do
    ...
    filtered |> Enum.map(&analyze_test_spec/1)
  end
