...
-opaque decoder(BVX) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BVX, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BWF)) -> {ok, BWF} |
...
-spec map(decoder(CAD), fun((CAD) -> CAF)) -> decoder(CAF).
...
-spec then(decoder(CAP), fun((CAP) -> decoder(CAR))) -> decoder(CAR).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CAU), list(decoder(CAU))) -> decoder(CAU).
...
-spec recursive(fun(() -> decoder(CBK))) -> decoder(CBK).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(BZZ)) -> decoder(gleam@option:option(BZZ)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BYA} | {error, BYA})
) -> {BYA, list(decode_error())}.
...
-spec collapse_errors(decoder(CAM), binary()) -> decoder(CAM).
...
-spec failure(CBE, binary()) -> decoder(CBE).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CBG} | {error, CBG})
) -> decoder(CBG).
...
-spec fold_dict(
    {gleam@dict:dict(BZL, BZM), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {BZL, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {BZM, list(decode_error())})
) -> {gleam@dict:dict(BZL, BZM), list(decode_error())}.
...
-spec dict(decoder(BZE), decoder(BZG)) -> decoder(gleam@dict:dict(BZE, BZG)).
dict(Key, Value) ->
...
-spec list(decoder(BYS)) -> decoder(list(BYS)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BXB, list(decode_error())}, list(any())) -> {BXB,
    list(decode_error())}.
...
-spec index(
    list(BWP),
    list(BWP),
    fun((gleam@dynamic:dynamic_()) -> {BWS, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BWP)) -> {BWS, list(decode_error())})
) -> {BWS, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BWA), fun((BWA) -> decoder(BWC))) -> decoder(BWC).
...
-spec at(list(any()), decoder(BWM)) -> decoder(BWM).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BXK), fun((BXK) -> decoder(BXM))) -> decoder(BXM).
...
-spec optional_field(any(), BXQ, decoder(BXQ), fun((BXQ) -> decoder(BXS))) -> decoder(BXS).
...
-spec optionally_at(list(any()), BXX, decoder(BXX)) -> decoder(BXX).
