...
-spec be_ok({ok, FMQ} | {error, any()}) -> FMQ.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FMV}) -> FMV.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FMY)) -> FMY.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
