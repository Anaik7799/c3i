...
-type response(EVT) :: {response, integer(), list({binary(), binary()}), EVT}.
...
-spec set_header(response(EWI), binary(), binary()) -> response(EWI).
...
-spec prepend_header(response(EWL), binary(), binary()) -> response(EWL).
...
-spec set_body(response(any()), EWQ) -> response(EWQ).
...
-spec try_map(response(EVU), fun((EVU) -> {ok, EVW} | {error, EVX})) -> {ok,
        response(EVW)} |
    {error, EVX}.
...
-spec map(response(EWS), fun((EWS) -> EWU)) -> response(EWU).
...
-spec expire_cookie(response(EXD), binary(), gleam@http@cookie:attributes()) -> response(EXD).
