...
-spec map_response_body(
    fun((EZP) -> gleam@http@response:response(EZQ)),
    fun((EZQ) -> EZS)
) -> fun((EZP) -> gleam@http@response:response(EZS)).
...
-spec prepend_response_header(
    fun((EZU) -> gleam@http@response:response(EZV)),
    binary(),
    binary()
) -> fun((EZU) -> gleam@http@response:response(EZV)).
...
-spec ensure_post(gleam@http@request:request(EZY)) -> {ok,
        gleam@http@request:request(EZY)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(FAH)) -> FAJ)) -> fun((gleam@http@request:request(FAH)) -> FAJ).
