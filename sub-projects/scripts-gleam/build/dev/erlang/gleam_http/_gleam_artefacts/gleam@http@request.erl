...
-spec set_header(request(ELZ), binary(), binary()) -> request(ELZ).
...
-spec prepend_header(request(EMC), binary(), binary()) -> request(EMC).
...
-spec set_body(request(any()), EMH) -> request(EMH).
set_body(Req, Body) ->
    {request,
...
-spec map(request(EMJ), fun((EMJ) -> EML)) -> request(EML).
...
-spec set_query(request(EMV), list({binary(), binary()})) -> request(EMV).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(EMZ), gleam@http:method()) -> request(EMZ).
...
-spec set_scheme(request(ENG), gleam@http:scheme()) -> request(ENG).
...
-spec set_host(request(ENJ), binary()) -> request(ENJ).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(ENM), integer()) -> request(ENM).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(ENP), binary()) -> request(ENP).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(ENS), binary(), binary()) -> request(ENS).
...
-spec remove_cookie(request(ENY), binary()) -> request(ENY).
