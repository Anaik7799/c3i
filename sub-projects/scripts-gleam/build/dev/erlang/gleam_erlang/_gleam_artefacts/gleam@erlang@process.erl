...
-opaque subject(DPX) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DPX)}.

-type name(DPY) :: any() | {gleam_phantom, DPY}.

-type do_not_leak() :: any().

-type selector(DPZ) :: any() | {gleam_phantom, DPZ}.
...
-spec 'receive'(subject(DQX), integer()) -> {ok, DQX} | {error, nil}.
...
-spec receive_forever(subject(DRF)) -> DRF.
...
-spec selector_receive(selector(DRJ), integer()) -> {ok, DRJ} | {error, nil}.
...
-spec selector_receive_forever(selector(DRN)) -> DRN.
...
-spec map_selector(selector(DRP), fun((DRP) -> DRR)) -> selector(DRR).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRT), selector(DRT)) -> selector(DRT).
merge_selector(A, B) ->
...
-spec select_map(selector(DSE), subject(DSG), fun((DSG) -> DSE)) -> selector(DSE).
...
-spec select(selector(DSA), subject(DSA)) -> selector(DSA).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSO)
) -> selector(DSO).
...
-spec select_other(selector(DSS), fun((gleam@dynamic:dynamic_()) -> DSS)) -> selector(DSS).
...
-spec deselect(selector(DSJ), subject(any())) -> selector(DSJ).
...
-spec select_specific_monitor(selector(DTE), monitor(), fun((down()) -> DTE)) -> selector(DTE).
...
-spec select_monitors(selector(DTH), fun((down()) -> DTH)) -> selector(DTH).
...
-spec select_trapped_exits(selector(DRX), fun((exit_message()) -> DRX)) -> selector(DRX).
...
-spec deselect_specific_monitor(selector(DTK), monitor()) -> selector(DTK).
...
-spec send_after(subject(DUG), integer(), DUG) -> timer().
...
-spec perform_call(
    subject(DTN),
    fun((subject(DTP)) -> DTN),
    fun((selector(DTP)) -> {ok, DTP} | {error, nil})
) -> DTP.
...
-spec call(subject(DTU), integer(), fun((subject(DTW)) -> DTU)) -> DTW.
...
-spec call_forever(subject(DTY), fun((subject(DUA)) -> DTY)) -> DUA.
