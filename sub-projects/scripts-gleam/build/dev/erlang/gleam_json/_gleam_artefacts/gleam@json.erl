...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FGS)) -> {ok, FGS} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FGM)) -> {ok, FGM} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FGI)) -> {ok, FGI} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FGY), fun((FGY) -> json())) -> json().
...
-spec array(list(FHC), fun((FHC) -> json())) -> json().
