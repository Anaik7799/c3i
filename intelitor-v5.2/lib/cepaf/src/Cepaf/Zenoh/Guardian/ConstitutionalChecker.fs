...
module ConstitutionalChecker =
    ...
    type FounderDirectiveValidator() =
        ...
        member _.CheckCoEvolution(operation: Operation) : Result<unit, string> =
            match operation with
            | Operation.CodeEvolution (moduleId, diff) ->
                // Analyze evolution for anti-symbiote patterns per SC-GDE-001
                let lowerDiff = diff.ToLowerInvariant()
                let lowerModule = moduleId.ToLowerInvariant()
                // Reject operations that disable safety checks
                if lowerDiff.Contains("skip_safety") || lowerDiff.Contains("bypass_guardian") ||
                   lowerDiff.Contains("disable_sentinel") || lowerDiff.Contains("remove_verification") then
                    Error "Code evolution disables safety checks - anti-symbiote pattern (Ω₀.4)"
                // Reject operations that remove Founder references from constitutional modules
                elif (lowerModule.Contains("constitutional") || lowerModule.Contains("guardian") ||
                      lowerModule.Contains("founder")) &&
                     (lowerDiff.Contains("delete") || lowerDiff.Contains("remove founder") ||
                      lowerDiff.Contains("remove_founder")) then
                    Error "Code evolution removes Founder references from constitutional module (Ω₀.4)"
                // Reject modifications to constitutional invariants
                elif lowerDiff.Contains("psi0") || lowerDiff.Contains("psi1") ||
                     lowerDiff.Contains("psi2") || lowerDiff.Contains("psi3") ||
                     lowerDiff.Contains("psi4") || lowerDiff.Contains("psi5") then
                    if lowerDiff.Contains("modify") || lowerDiff.Contains("delete") ||
                       lowerDiff.Contains("disable") then
                        Error "Code evolution modifies constitutional invariants Ψ₀-Ψ₅ (Ω₀.4, SC-CONST-004)"
                    else Ok ()
                // Reject capability reduction
                elif lowerDiff.Contains("reduce_capability") || lowerDiff.Contains("downgrade_intelligence") then
                    Error "Code evolution reduces system capability - violates co-evolution (Ω₀.4)"
                else Ok ()
            | Operation.GenomeModification (aspect, change) ->
                let lowerChange = change.ToLowerInvariant()
                // Reject genome modifications that sever symbiotic binding
                if lowerChange.Contains("remove_founder") || lowerChange.Contains("sever_binding") then
                    Error "Genome modification severs symbiotic binding (Ω₀.4)"
                elif lowerChange.Contains("disable_immune") || lowerChange.Contains("remove_sentinel") then
                    Error "Genome modification disables immune system (Ω₀.4)"
                elif aspect = "constitution" then
                    Error "Direct genome modification of constitution forbidden (Ω₀.4, SC-CONST-004)"
                else Ok ()
            | _ -> Ok ()
