...
module SignedBlock =
    ...
    /// <returns>Deserialized signed block or raises if null</returns>
    let fromJson (json: string) : SignedBlock<'T> =
        match JsonSerializer.Deserialize<SignedBlock<'T>>(json) with
        | null -> failwith "Failed to deserialize SignedBlock: null result"
        | block -> block
