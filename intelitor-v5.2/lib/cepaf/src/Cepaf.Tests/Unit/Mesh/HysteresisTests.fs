...
module Generators =
    ...
    /// Generate sequence of health checks (simulating flapping)
    let healthSequenceGen =
        gen {
            let! length = Gen.choose(5, 30)
            let! states = Gen.listOfLength length healthStateGen
            return states
        }

    /// Generate alternating sequence (worst case flapping)
    let flappingSequenceGen =
        gen {
            let! length = Gen.choose(10, 20)
            let states = List.init length (fun i ->
                if i % 2 = 0 then HealthState.Healthy else HealthState.Unhealthy)
            return states
        }

    /// Generate stable sequence (all same state)
    let stableSequenceGen =
        gen {
            let! length = Gen.choose(5, 15)
            let! state = healthStateGen
            return List.replicate length state
        }
...
let propertyTests =
    testList "Hysteresis Property Tests" [
        ...
        testProperty "Consecutive count never exceeds required threshold" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.healthSequenceGen) (fun healthChecks ->
                let config = Hysteresis.defaultConfig
        ...
        testProperty "Total checks = healthy + unhealthy + unknown + degraded" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.healthSequenceGen) (fun healthChecks ->
                let config = Hysteresis.defaultConfig
                let mutable state = Hysteresis.create ()
        ...
        testProperty "Health percentage always in [0, 100]" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.healthSequenceGen) (fun healthChecks ->
                let config = Hysteresis.defaultConfig
                let mutable state = Hysteresis.create ()
        ...
        testProperty "Stable sequence eventually transitions" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.stableSequenceGen) (fun healthChecks ->
                let config = Hysteresis.defaultConfig
                let mutable state = Hysteresis.create ()
        ...
        testProperty "History never exceeds MaxHistory" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.healthSequenceGen) (fun healthChecks ->
        ...
        testProperty "Flapping sequence prevents transitions" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.flappingSequenceGen) (fun healthChecks ->
                let config = Hysteresis.defaultConfig
                let mutable state = Hysteresis.create ()
