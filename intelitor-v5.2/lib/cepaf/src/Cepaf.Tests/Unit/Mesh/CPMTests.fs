...
let propertyTests =
    testList "CPM Property Tests" [
        ...
        testProperty "CPM invariant: ES + Duration = EF for all tasks" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis ->
                    analysis.Tasks
        ...
        testProperty "CPM invariant: LS + Duration = LF for all tasks" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis ->
                    analysis.Tasks
        ...
        testProperty "CPM invariant: TotalFloat = LS - ES for all tasks" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis ->
                    analysis.Tasks
        ...
        testProperty "CPM invariant: All slack values >= 0" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis ->
        ...
        testProperty "CPM invariant: Critical path has zero slack" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis ->
        ...
        testProperty "CPM invariant: Total duration = max(EF) across all tasks" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
        ...
        testProperty "CPM invariant: Critical path duration <= total work" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis when not analysis.Tasks.IsEmpty ->
        ...
        testProperty "CPM invariant: Parallelization potential in [0, 1]" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.validDagGen) (fun nodes ->
                match CPM.calculate nodes with
                | Ok analysis ->
