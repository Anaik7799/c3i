...
module Generators =
    ...
    let dagNodeGen =
        gen {
            let! id = Arb.generate<string> |> Gen.filter (not << System.String.IsNullOrWhiteSpace)
            let! duration = Gen.choose(100, 5000)
            let! wave = Gen.choose(0, 10)
            return DAG.createNode id id [] duration wave Criticality.P0_Critical
        }

    /// Generate acyclic DAG (valid dependencies only)
    let acyclicDagGen =
        gen {
            let! nodeCount = Gen.choose(2, 8)
            ...
            let! nodes =
                ...
                |> List.mapi (fun i id ->
                    gen {
                        let! duration = Gen.choose(100, 2000)
                        // Only depend on earlier nodes to ensure acyclicity
                        let possibleDeps = nodeIds |> List.take i
                        let! depCount = Gen.choose(0, min 2 (List.length possibleDeps))
                        let! deps =
                            if depCount = 0 then Gen.constant []
                            else Gen.shuffle possibleDeps |> Gen.map (fun arr -> arr |> Array.take depCount |> Array.toList)
                        return DAG.createNode id id deps duration i Criticality.P0_Critical
                    })
                |> Gen.sequence
    ...
    let cyclicDagGen =
        gen {
            let! nodeCount = Gen.choose(2, 5)
            let nodeIds = List.init nodeCount (fun i -> sprintf "node-%d" i)

            // Create a cycle: last node depends on first
            let nodes =
                nodeIds
                |> List.mapi (fun i id ->
                    let deps =
                        if i = nodeCount - 1 then [nodeIds.[0]]  // Create cycle
                        elif i > 0 then [nodeIds.[i-1]]
                        else []
...
let propertyTests =
    testList "DAG Property Tests" [
        ...
        testProperty "Acyclic DAG always produces valid topological sort" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
        ...
        testProperty "Topological sort preserves all nodes" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
                match DAG.topologicalSort nodes with
                | Sorted sorted ->
        ...
        testProperty "Cyclic DAG always detects cycle" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.cyclicDagGen) (fun nodes ->
                match DAG.topologicalSort nodes with
                | CycleDetected cycle -> cycle.Length > 0
                | Sorted _ -> false  // Should detect cycle
            )

        testProperty "groupByWave preserves all nodes" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
        ...
        testProperty "getDownstream + getUpstream are consistent" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
        ...
        testProperty "verifyDependencies succeeds when all deps exist" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
        ...
        testProperty "Critical path duration >= max single task duration" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
        ...
        testProperty "Critical path duration <= sum of all durations" <| fun () ->
            Prop.forAll (Arb.fromGen Generators.acyclicDagGen) (fun nodes ->
                if nodes.IsEmpty then true
                else
                    let cpDuration = DAG.estimateCriticalPathDuration nodes
