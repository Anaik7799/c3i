...
module Generators =
    ...
    let stateGen : Gen<ContainerState> =
        Gen.elements [
    ...
    let signalGen : Gen<ContainerSignal> =
        Gen.elements [
    ...
    /// Generate sequence of signals (FsCheck 3.x style)
    let signalSequenceGen : Gen<ContainerSignal list> =
        Gen.choose(1, 20) |> Gen.bind (fun length ->
            Gen.listOfLength length signalGen
        )
...
let propertyTests =
    testList "FSM Property Tests" [
        ...
        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "FSM is deterministic - same state + signal = same result" <|
            fun () ->
                let arb = Arb.fromGen (Gen.zip Generators.stateGen Generators.signalGen)
                Prop.forAll arb (fun (state, signal) ->
                    let result1 = FSM.transition state signal
                    let result2 = FSM.transition state signal
                    result1 = result2
                )

        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "Transition function is total - defined for all inputs" <|
            fun () ->
                let arb = Arb.fromGen (Gen.zip Generators.stateGen Generators.signalGen)
                Prop.forAll arb (fun (state, signal) ->
        ...
        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "Accepting state implies running" <|
            fun () ->
                let arb = Arb.fromGen Generators.stateGen
                Prop.forAll arb (fun state ->
                    if FSM.isAccepting state then FSM.isRunning state
                    else true
                )

        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "Terminal states are not running" <|
            fun () ->
                let arb = Arb.fromGen Generators.stateGen
                Prop.forAll arb (fun state ->
        ...
        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "History length equals number of applied signals" <|
            fun () ->
                let arb = Arb.fromGen Generators.signalSequenceGen
                Prop.forAll arb (fun signals ->
        ...
        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "Health check count equals HealthOk + HealthFail signals" <|
            fun () ->
                let arb = Arb.fromGen Generators.signalSequenceGen
                Prop.forAll arb (fun signals ->
                    let mutable fsm = FSM.create "test" "test"
        ...
        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "HealthOk always resets consecutive fails to 0" <|
            fun () ->
                let arb = Arb.fromGen Generators.signalSequenceGen
                Prop.forAll arb (fun signals ->
                    let mutable fsm = FSM.create "test" "test"
                    ...
                    for signal in signals do
                        fsm <- FSM.applySignal fsm signal None
                        if signal = ContainerSignal.HealthOk && fsm.ConsecutiveHealthFails <> 0 then
                            allValid <- false
                    allValid
                )

        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "Consecutive health fails never exceeds total health checks" <|
            fun () ->
                let arb = Arb.fromGen Generators.signalSequenceGen
                Prop.forAll arb (fun signals ->
                    let mutable fsm = FSM.create "test" "test"
        ...
        testPropertyWithConfig { FsCheckConfig.defaultConfig with maxTest = 100 } "State never spontaneously changes without signal" <|
            fun () ->
                let arb = Arb.fromGen Generators.stateGen
                Prop.forAll arb (fun state ->
