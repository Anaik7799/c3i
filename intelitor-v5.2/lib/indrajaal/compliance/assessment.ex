defmodule Indrajaal.Compliance.Assessment do
  @moduledoc """
  Represents compliance assessments and audits.

  Assessments evaluate compliance status against frameworks and _requirements,
  ...
  relationships do
    ...
    belongs_to :lead_assessor, Indrajaal.Accounts.User do
      attribute_public? true
    end

    belongs_to :peer_reviewer, Indrajaal.Accounts.User do
      attribute_public? true
    end
  ...
  actions do
      ...
      change fn changeset, _context ->
        ...
        current_count =
          case finding_type do
            ...
            :high ->
              Ash.Changeset.get_attribute(changeset, :high_findings)

            :medium ->
              Ash.Changeset.get_attribute(changeset, :medium_findings)

            :low ->
              Ash.Changeset.get_attribute(changeset, :low_findings)
    ...
    update :calculate_compliance_score do
      ...
      change fn changeset, _context ->
        _total = Ash.Changeset.get_attribute(changeset, :total_findings)
    ...
    update :approve_report do
      ...
      change fn changeset, _context ->
        ...
        |> apply_management_response(changeset.arguments.management_response)
      end
    end
  ...
  # Helper functions
  @spec apply_management_response(term(), String.t() | nil) :: term()
  defp apply_management_response(changeset, response) do
    if response do
      Ash.Changeset.force_change_attribute(changeset, :management_response, response)
    else
      changeset
    end
  end

  @spec generate_assessment_number(term()) :: term()
