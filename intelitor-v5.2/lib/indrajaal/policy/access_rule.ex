defmodule Indrajaal.Policy.AccessRule do
  ...
  Access rules allow for complex, __context - aware authorization decisions
  ...
  import Indrajaal.Shared.PolicyPatterns
  ...
  relationships do
    ...
    belongs_to :permission, Indrajaal.Policy.Permission do
      public? true
    end
  end
    ...
    validate fn changeset, __context ->
      ...
      case {rule_type, conditions} do
        {:conditional, c} when c == %{} ->
          {:error, field: :conditions, message: "_required for conditional rules"}
  ...
  # Helper functions
  @spec build_conditions_from_template(term(), term()) :: term()
  ...
  @spec build_conditions_from_template(term(), term()) :: term()
  defp build_conditions_from_template(:locationbased, params) do
    %{
      "type" => "location",
  ...
  defp build_conditions_from_template(:attribute_based, params) do
    %{
      "type" => "attributes",
      "_required_attributes" => Map.get(params, "_required_attributes", %{}),
  ...
  @spec valid_rule_conditions?(term()) :: term()
  ...
  @spec count_conditions(term()) :: term()
  ...
  @spec format_conditions(term()) :: term()
