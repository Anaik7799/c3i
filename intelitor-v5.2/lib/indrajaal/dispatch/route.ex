defmodule Indrajaal.Dispatch.Route do
  @moduledoc """
  ...
  relationships do
    belongs_to :assignment, Indrajaal.Dispatch.Assignment do
  ...
  calculations do
    ...
    calculate :is_delayed?, :boolean do
      calculation fn records, __context ->
        map_calculation(records, &calculate_is_delayed/1)
      end
    end

    calculate :estimated_arrival_time, :utc_datetime_usec do
      calculation fn records, __context ->
        map_calculation(records, &calculate_estimated_arrival/1)
      end
    end
  ...
  # Helper functions
  @spec calculate_route_metrics(term()) :: term()
  ...
  @doc false
  @spec map_calculation(list(), function()) :: {:ok, list()}
  defp map_calculation(records, mapper_fn) do
    {:ok, Enum.map(records, mapper_fn)}
  end

  @doc false
  @spec route_active?(map()) :: boolean()
  defp route_active?(route) do
    route.started_at && route.status == :active
  end

  @doc false
  @spec calculate_is_delayed(map()) :: boolean()
  defp calculate_is_delayed(route) do
    if route_active?(route) do
  ...
  @doc false
  @spec calculate_estimated_arrival(map()) :: DateTime.t() | nil
  defp calculate_estimated_arrival(route) do
    if route_active?(route) do
      remaining_percent = 100 - route.current_progress_percent

      remaining_duration =
        (route.estimated_duration_minutes * remaining_percent / 100.0)
        |> round()
