defmodule Indrajaal.Alarms.AlarmEvent do
  ...
  relationships do
    ...
    belongs_to :incident_type, Indrajaal.Alarms.IncidentType do
      attribute_public? true
    end

    belongs_to :acknowledged_user, Indrajaal.Accounts.User do
      attribute_writable? false
      source_attribute :acknowledged_by
      attribute_public? true
    end

    belongs_to :investigating_user, Indrajaal.Accounts.User do
      attribute_writable? false
      source_attribute :investigating_by
      attribute_public? true
    end

    belongs_to :resolved_user, Indrajaal.Accounts.User do
      attribute_writable? false
      source_attribute :resolved_by
      attribute_public? true
    end

    belongs_to :workflow_template, Indrajaal.Alarms.WorkflowTemplate do
      attribute_public? true
    end
    ...
    has_many :dispatch_logs, Indrajaal.Alarms.DispatchLog
  end
      ...
      after_action(fn changeset, result, __context ->
        ...
        :telemetry.execute(
          ...
          %{
            ...
            tenant_id: Indrajaal.Tracing.extract_tenant_id(__context[:actor])
          }
        )
          ...
          Map.merge(
            ...
            %{
              ...
              "severity_update_count" =>
                Map.get(
                  Ash.Changeset.get_attribute(changeset, :meta_data) || %{},
                  "severity_update_count",
                  0
                ) + 1
            }
  ...
  calculations do
    ...
    calculate :__requires_dispatch?, :boolean do
      calculation fn records, __context ->
        values =
          Enum.map(
            records,
      ...
      calculation fn records, __context ->
        values =
          Enum.map(
            records,
            fn event ->
              # Calculate SLA status based on event type, severity, and response
  ...
  @spec calculate_resolution_time(term()) :: term()
  ...
  @spec ensure_acknowledged(term()) :: term()
  ...
  @spec set_priority_from_type(term()) :: term()
  ...
  # Fix #196: Add missing acknowledge_alarm/4 function
  @doc """
  Acknowledge an alarm event with user ID, timestamp, notes, and context.
  """
  @spec acknowledge_alarm(term(), term(), term(), term()) :: {:ok, term()} | {:error, term()}
  def acknowledge_alarm(alarm_id, user_id, timestamp, notes \\ "") do
    case by_id(alarm_id) do
      {:ok, alarm} ->
        acknowledge(alarm, %{
  ...
  # Fix #197: Add missing update_alarm/2 function
  @doc """
  Update an alarm event with provided attributes.
  """
  @spec update_alarm(term(), map(), Keyword.t()) :: {:ok, term()} | {:error, term()}
  def update_alarm(alarm, attributes, opts \\ []) do
    update(alarm, attributes, opts)
  end

  # Calculate SLA status based on event characteristics and response times
  @spec calculate_sla_status(term()) :: term()
