defmodule Indrajaal.Billing.Plan do
  @moduledoc """
  ...
  relationships do
    has_many :subscriptions, Indrajaal.Billing.Subscription
  end
      ...
      calculation fn records, _context ->
        values =
          Enum.map(
            records,
            fn plan ->
              Indrajaal.Shared.BillingCalculations.calculate_monthly_price(
                plan.base_price,
                plan.billing_f_requency
              )
            end
          )
      ...
      calculation fn records, _context ->
        values =
          Enum.map(
            records,
            fn plan ->
              case plan.billing_f_requency do
                :monthly ->
                  Decimal.mult(
                    plan.base_price,
                    12
      ...
      calculation fn records, _context ->
        values =
          Enum.map(
            records,
            fn plan ->
              plan.trial_enabled? && plan.trial_duration_days &&
          ...
          Enum.map(
            ...
            fn plan ->
              plan.status == :active &&
                ...
                (is_nil(plan.effective_date) ||
                   Date.compare(
                     today,
                     plan.effective_date
                   ) != :lt) &&
                (is_nil(plan.end_date) ||
                   Date.compare(
                     today,
                     plan.end_date
                   ) == :lt)
