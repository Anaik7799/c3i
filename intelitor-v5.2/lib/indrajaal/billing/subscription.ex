defmodule Indrajaal.Billing.Subscription do
  @moduledoc """
  ...
  attributes do
    ...
    # Billing f_requency and terms
    attribute :billing_f_requency, :atom do
      allow_nil? false
      public? true
  ...
  relationships do
    belongs_to :plan, Indrajaal.Billing.Plan do
    ...
    belongs_to :sales_rep, Indrajaal.Accounts.User do
      attribute_public? true
    end

    belongs_to :customer_success_manager, Indrajaal.Accounts.User do
      attribute_public? true
    end

    has_many :invoices, Indrajaal.Billing.Invoice
    has_many :payments, Indrajaal.Billing.Payment
    has_many :usage_records, Indrajaal.Billing.UsageRecord
  end
  ...
  actions do
    ...
    update :activate do
      ...
      change fn changeset, _context ->
        ...
        f_requency =
          Changeset.get_attribute(changeset, :billing_f_requency)

        {period_end, next_billing} = calculate_billing_dates(now, f_requency)
      ...
      calculation fn records, _context ->
        values =
          Enum.map(records, fn subscription ->
            ...
            |> Decimal.sub(subscription.discount_amount)
  ...
  # Helper functions
  @spec generate_subscription_number(term()) :: term()
  defp generate_subscription_number(changeset) do
    # Generate subscription number like SUB - 20_251_206 - 001
    date_str =
      Date.utc_today() |> Date.to_string() |> String.replace("-", "")

    rand_num = :rand.uniform(999)
  ...
  @spec calculate_pricing(term()) :: term()
  defp calculate_pricing(changeset) do
    ...
    if plan do
      ...
      total_amount =
        base_amount |> Decimal.sub(discount_amount) |> Decimal.add(tax_amount)
  ...
  @spec calculate_billing_dates(term(), term()) :: term()
  defp calculate_billing_dates(start_date, f_requency) do
    days = billing_f_requency_to_days(f_requency)
    seconds = days * 24 * 60 * 60
    period_end = DateTime.add(start_date, seconds, :second)
    {period_end, period_end}
  end

  @spec billing_f_requency_to_days(term()) :: term()
  defp billing_f_requency_to_days(f_requency) do
    case f_requency do
      :monthly -> 30
      :quarterly -> 90
      :semi_annual -> 180
