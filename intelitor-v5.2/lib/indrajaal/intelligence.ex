defmodule Indrajaal.Intelligence do
  ...
  @moduledoc """
  Context module for intelligence management.

  Provides business logic for intelligence operations with:
  - Multi - tenant __data isolation
  - Authentication and authorization
  - STAMP safety validation
  - Comprehensive error handling

  Generated by worker - 6 following SOPv5.1 methodology.
  """
  ...
  @doc """
  Lists intelligence with pagination and filtering.

  Enforces tenant isolation and access control.
  """
  @spec list_intelligence(any()) :: any()
  def list_intelligence(opts \\ []) do
    ...
    with :ok <- validate_query_params(page, page_size),
         :ok <- validate_user_access(user, :list, Intelligence) do
      base_query =
        Alert
  ...
  Enforces tenant isolation and access control.
  """
  @spec get_alert(any(), any()) :: any()
  def get_alert(id, opts \\ []) do
    tenant_id = Keyword.get(opts, :tenant_id)
    user = Keyword.get(opts, :user)

    with :ok <- validate_user_access(user, :read, Intelligence),
         {:ok, item} <- fetch_alert(id, tenant_id),
  ...
  def create_alert(attrs, opts \\ []) do
    ...
    with :ok <- validate_user_access(user, :create, Intelligence),
         ...
         {:ok, item} <- do_create_alert(attrs, tenant_id, user) do
  ...
  """
  @spec update_alert(term(), term(), term()) :: term()
  def update_alert(item, attrs, opts \\ []) do
    user = Keyword.get(opts, :user)

    with :ok <- validate_user_access(user, :update, item),
         :ok <- validate_update_attrs(attrs, item),
         {:ok, updated} <- do_update_alert(item, attrs, user) do
  ...
  """
  @spec delete_alert(any(), any()) :: any()
  def delete_alert(item, opts \\ []) do
    user = Keyword.get(opts, :user)
    ...
    with :ok <- validate_user_access(user, :delete, item),
         ...
         {:ok, deleted} <- do_delete_alert(item, user) do
  ...
  @spec fetch_alert(term(), term()) :: term()
  defp fetch_alert(id, tenant_id) do
    case Repo.get_by(Alert, id: id, tenant_id: tenant_id) do
      nil -> {:error, :not_found}
      item -> {:ok, item}
    end
  end

  defp do_create_alert(attrs, tenant_id, user) do
    %Alert{}
  ...
  defp do_update_alert(item, attrs, user) do
    item
    |> Alert.changeset(attrs)
  ...
  @spec do_delete_alert(term(), term()) :: term()
  defp do_delete_alert(item, _user) do
    Repo.delete(item)
  end
  ...
  defp analyze_validation_errors(changeset) do
    EnhancedErrorHelpers.analyze_validation_errors(:intelligence, changeset)
  end
end
