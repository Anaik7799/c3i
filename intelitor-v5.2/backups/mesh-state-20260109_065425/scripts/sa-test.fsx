...
module Tests =
    ...
    let testUp () =
        runTest "V-BOOT-001: Cold Start" (fun () ->
            // Use the mesh tool boot command
            let (code, o, _) = Shell.exec "dotnet" "run --project lib/cepaf/src/Cepaf/Cepaf.fsproj -- mesh up"
            code = 0 && (o.Contains("INDRAJAAL MESH STABILIZED") || o.Contains("SIL-6 CERTIFIED"))
        )
    ...
    let testStatus () =
        runTest "V-STATUS-001: KPI Dashboard" (fun () ->
            let (code, o, _) = Shell.exec "dotnet" "run --project lib/cepaf/src/Cepaf/Cepaf.fsproj -- mesh status"
            code = 0 && o.Contains("INDRAJAAL SIL-6 MESH STATUS")
        )
    ...
    let testDown () =
        runTest "V-DOWN-001: Surgical Shutdown" (fun () ->
            let (code, o, _) = Shell.exec "dotnet" "run --project lib/cepaf/src/Cepaf/Cepaf.fsproj -- mesh down"
            code = 0 && o.Contains("SUBSTRATE RETURNED TO STATIC STATE")
        )
    ...
    // -------------------------------------------------------------------------
    let testSla () =
        runTest "V-SIL6-001: Boot Latency < 10s" (fun () ->
            let sw = Stopwatch.StartNew()
            let (code, _, _) = Shell.exec "dotnet" "run --project lib/cepaf/src/Cepaf/Cepaf.fsproj -- mesh up"
            sw.Stop()
            code = 0 && sw.ElapsedMilliseconds < 15000L // Allowing 15s for F# JIT/Startup overhead
        )
