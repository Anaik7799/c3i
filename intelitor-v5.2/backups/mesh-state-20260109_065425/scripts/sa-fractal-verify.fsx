...
// --- CONFIGURATION ---
let appNode = "indrajaal-app-1"
let obsNode = "indrajaal-obs"
let dbNode = "indrajaal-db1"
...
let checkL2 () =
    ...
    if code = 0 && out.Contains("name app1") then
...
let checkL3 () =
    ...
    try
        ...
        let response = client.GetAsync("http://localhost:4000/health").Result
...
let checkL7 () =
    ...
    let (code, out) = execShell "podman network inspect intelitor-v52_fractal-mesh"
    if code = 0 then
