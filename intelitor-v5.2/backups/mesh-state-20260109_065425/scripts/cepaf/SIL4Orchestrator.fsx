...
module OBP =
    ...
    let bootNode serviceName =
        ...
        let code = Shell.execVerbose "podman-compose" (sprintf "-f lib/cepaf/artifacts/podman-compose-fractal-cluster.yml up -d %s" serviceName)
        sw.Stop()
        if code = 0 then 
...
let runShutdown () =
    ...
    Telemetry.log "LAMEDUCK" "RUN" "Broadcasting shutdown signals..."
    Shell.execVerbose "podman" "kill indrajaal-app-1 indrajaal-app-2 indrajaal-app-3" |> ignore
    Shell.execVerbose "podman" "kill -s SIGINT indrajaal-db-1" |> ignore
    Shell.execVerbose "podman-compose" "-f lib/cepaf/artifacts/podman-compose-fractal-cluster.yml down -v" |> ignore
    Telemetry.log "FINAL" "OK" "Substrate returned to static state"
