...
// ARCHITECTURE: indrajaal-db-prod, indrajaal-obs-prod (OTEL+Prometheus+Grafana+Loki), indrajaal-app-prod (Phoenix+Redis)
...
module Shell =
    ...
    let checkPodmanContainer (name: string) : Async<bool> = async {
        match! exec "podman" $"inspect {name} --format '{{{{.State.Status}}}}'" with
        | Ok status -> return status.Trim().Trim('\'') = "running"
        | Error _ -> return false
    }
    ...
    let scenarios : TestScenario list = [
            ...
            Execute = fun () -> async {
                ...
                | Error msg ->
                    ...
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = $"Database connection failed: {msg}"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // INF-002: Redis connectivity (embedded in app container)
        {
            ...
            Execute = fun () -> async {
                let sw = Stopwatch.StartNew()
                let startTime = DateTime.UtcNow
                // Redis is now embedded in app container
                ...
                | Ok output when output.Trim() = "PONG" ->
                    ...
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Redis responding with PONG"
                ...
                | Ok output ->
                    ...
                    return {
                        ScenarioId = "INF-REDIS-001"
                        Status = Failed $"Unexpected response: {output}"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Redis responded but not with PONG"
                        Metrics = Map.empty
                        AIValidation = None
                    }
                | Error msg ->
                    ...
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = $"Redis connection failed: {msg}"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // INF-008: Container network
        {
            ...
            Execute = fun () -> async {
                ...
                | Ok output when output.Contains("indrajaal-mesh") ->
                    sw.Stop()
                    return {
                        ScenarioId = "INF-NET-001"
                        Status = Passed
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Mesh network is configured"
                        Metrics = Map.empty
                        AIValidation = None
                    }
                | Ok _ | Error _ ->
                    ...
                    return {
                        ScenarioId = "INF-NET-001"
                        Status = Failed "Mesh network not found"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Container mesh network is not configured"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // INF-009: FLAME pool
        {
            Id = "INF-FLAME-001"
            Domain = Infrastructure
            Name = "FLAME Pool Status"
            Description = "Verify FLAME distributed processing is enabled"
            Priority = High
            ...
            Execute = fun () -> async {
                ...
                | Ok _ | Error _ ->
                    ...
                    return {
                        ScenarioId = "INF-FLAME-001"
                        Status = Failed "FLAME not enabled"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "FLAME distributed processing is not enabled"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // INF-010: Clustering
        {
            ...
            Description = "Verify Elixir clustering is configured"
            Priority = High
            Dependencies = ["INF-PHX-001"]
            Timeout = TimeSpan.FromSeconds(15.0)
            Execute = fun () -> async {
                ...
                | Ok output when output.Trim() = "true" ->
                    sw.Stop()
                    return {
                        ScenarioId = "INF-CLUS-001"
                        Status = Passed
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Clustering is enabled"
                        Metrics = Map.empty
                        AIValidation = None
                    }
                | Ok _ | Error _ ->
                    ...
                    return {
                        ScenarioId = "INF-CLUS-001"
                        Status = Failed "Clustering not enabled"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Elixir clustering is not enabled"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
...
// =============================================================================
// TEST SCENARIOS - DATAFLOW (15 scenarios)
// =============================================================================

module DataflowTests =
    let scenarios : TestScenario list = [
        // DF-001: API endpoint availability
        {
            Id = "DF-API-001"
            Domain = Dataflow
            Name = "API Endpoints"
            ...
            Execute = fun () -> async {
                let sw = Stopwatch.StartNew()
                let startTime = DateTime.UtcNow
                let endpoints = ["/api/health"; "/api/status"]
                let! results =
                    endpoints
                    |> List.map (fun ep -> Http.get $"{Config.PhoenixUrl}{ep}")
                    |> Async.Parallel

                let passed = results |> Array.filter (function Ok _ -> true | _ -> false) |> Array.length
                sw.Stop()

                if passed = endpoints.Length then
                    return {
                        ScenarioId = "DF-API-001"
                        Status = Passed
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = $"All {endpoints.Length} API endpoints responding"
                        Metrics = Map.ofList [("endpoints_checked", float endpoints.Length); ("response_time_ms", sw.Elapsed.TotalMilliseconds)]
                        AIValidation = None
                    }
                else
                    return {
                        ScenarioId = "DF-API-001"
                        Status = Failed $"{endpoints.Length - passed} endpoints failed"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Some API endpoints not responding"
                        Metrics = Map.ofList [("passed", float passed); ("failed", float (endpoints.Length - passed))]
                        AIValidation = None
                    }
            }
        }

        // DF-002: Database read operations
        {
            Id = "DF-DB-READ-001"
            Domain = Dataflow
            Name = "Database Read"
            Description = "Verify database read operations"
            Priority = Critical
            Dependencies = ["INF-DB-001"]
            Timeout = TimeSpan.FromSeconds(30.0)
            Execute = fun () -> async {
                let sw = Stopwatch.StartNew()
                let startTime = DateTime.UtcNow
                match! Shell.exec "podman" "exec indrajaal-db-prod psql -U postgres -d indrajaal_prod -c 'SELECT 1'" with
                | Ok _ ->
                    sw.Stop()
                    return {
        ...
        // DF-003: Redis cache operations
        {
            ...
            Execute = fun () -> async {
                ...
                match! Shell.exec "podman" $"exec indrajaal-app-prod redis-cli SET {testKey} 'test_value'" with
                | Ok _ ->
                    // Get
                    match! Shell.exec "podman" $"exec indrajaal-app-prod redis-cli GET {testKey}" with
                    | Ok value when value.Trim() = "test_value" ->
                        // Cleanup
                        let! _ = Shell.exec "podman" $"exec indrajaal-app-prod redis-cli DEL {testKey}"
                        sw.Stop()
                        return {
                            ScenarioId = "DF-CACHE-001"
                            Status = Passed
                            Duration = sw.Elapsed
                            StartTime = startTime
                            EndTime = DateTime.UtcNow
                            Details = "Cache read/write successful"
                            Metrics = Map.ofList [("operation_time_ms", sw.Elapsed.TotalMilliseconds)]
                            AIValidation = None
                        }
                    | Ok v ->
                        sw.Stop()
                        return {
                            ScenarioId = "DF-CACHE-001"
                            Status = Failed $"Unexpected value: {v}"
                            Duration = sw.Elapsed
                            StartTime = startTime
                            EndTime = DateTime.UtcNow
                            Details = "Cache returned wrong value"
                            Metrics = Map.empty
                            AIValidation = None
                        }
                    | Error msg ->
                ...
                | Error msg ->
                    ...
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = $"Cache write failed: {msg}"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // DF-004: Prajna dashboard data
        {
            Id = "DF-PRAJNA-001"
            Domain = Dataflow
            Name = "Prajna Data Flow"
            Description = "Verify Prajna cockpit receives data"
            Priority = High
            Dependencies = ["INF-PHX-001"]
            Timeout = TimeSpan.FromSeconds(30.0)
            Execute = fun () -> async {
                ...
                | Ok response when response.Length > 100 ->
                    sw.Stop()
                    return {
                        ScenarioId = "DF-PRAJNA-001"
                        Status = Passed
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Prajna dashboard accessible"
                        Metrics = Map.ofList [("response_size", float response.Length)]
                        AIValidation = None
                    }
                | Ok response ->
                    sw.Stop()
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = $"Prajna returned minimal data: {response.Length} bytes"
                        Metrics = Map.empty
                        AIValidation = None
                    }
                | Error msg ->
                    ...
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = $"Prajna access failed: {msg}"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // DF-005 through DF-015: Additional dataflow scenarios
        {
            Id = "DF-TELEMETRY-001"
            Domain = Dataflow
            Name = "Telemetry Pipeline"
            Description = "Verify OTEL telemetry is flowing"
            Priority = High
            Dependencies = ["INF-OTEL-001"]
            Timeout = TimeSpan.FromSeconds(20.0)
            Execute = fun () -> async {
                let sw = Stopwatch.StartNew()
                let startTime = DateTime.UtcNow
                match! Http.get "http://localhost:8888/metrics" with
                | Ok response when response.Contains("otel") || response.Contains("receiver") ->
                    sw.Stop()
                    return {
                        ...
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Telemetry pipeline active"
                        Metrics = Map.ofList [("metrics_size", float response.Length)]
                        AIValidation = None
                    }
                | Ok _ ->
                    sw.Stop()
...
module CockpitTests =
    let scenarios : TestScenario list = [
        ...
        // CP-003: Dark mode
        {
            ...
            Execute = fun () -> async {
                ...
                | Ok output when output.Trim() = "true" ->
                    sw.Stop()
                    return {
                        ScenarioId = "CP-DARK-001"
                        Status = Passed
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Dark mode is enabled"
                        Metrics = Map.empty
                        AIValidation = None
                    }
                | Ok _ | Error _ ->
                    ...
                    return {
                        ScenarioId = "CP-DARK-001"
                        Status = Failed "Dark mode not enabled"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "PRAJNA_DARK_MODE not set to true"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
        ...
        // CP-004 through CP-025: Additional cockpit scenarios
        {
            Id = "CP-NAV-001"
            Domain = Cockpit
            ...
            Execute = fun () -> async {
                ...
                | Ok response when response.Length > 500 ->
                    sw.Stop()
                    return {
                        ScenarioId = "CP-NAV-001"
                        Status = Passed
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Navigation menu present"
                        Metrics = Map.ofList [("page_size", float response.Length)]
                        AIValidation = None
                    }
                | Ok _ | Error _ ->
                    ...
                    return {
                        ScenarioId = "CP-NAV-001"
                        Status = Failed "Page content insufficient"
                        Duration = sw.Elapsed
                        StartTime = startTime
                        EndTime = DateTime.UtcNow
                        Details = "Navigation menu may be missing"
                        Metrics = Map.empty
                        AIValidation = None
                    }
            }
...

        {
            ...
            Domain = Cockpit
            Name = "Response Time"
            Description = "Verify cockpit response time < 50ms"
            Priority = Critical
            Dependencies = ["INF-PHX-001"]
            Timeout = TimeSpan.FromSeconds(15.0)
            Execute = fun () -> async {
            ...
            Execute = fun () -> async {
                ...
                let! results =
                    ...
                    |> List.map (fun v -> async {
                        match! Shell.exec "podman" $"exec indrajaal-app-prod printenv {v}" with
                        | Ok output when not (String.IsNullOrWhiteSpace output) -> return Some v
                        | _ -> return None
                    })
                    |> Async.Parallel
...
module SwarmExecutor =
    ...
    let private mutable currentState = {
        ActiveWorkers = 0
        CompletedTests = 0
        FailedTests = 0
        TotalTests = 0
    ...
    let run (mode: string) : Async<TestReport> = async {
        ...
        printfn $"   Mode: {mode}"
        printfn $"   AI Validation: {if Config.AIValidationEnabled && Config.OpenRouterApiKey.IsSome then "Enabled" else "Disabled"}"
        printfn ""
...
module ReportGenerator =
    let generateMarkdown (report: TestReport) : string =
        ...
        sb.AppendLine($"**Generated**: {report.Timestamp:yyyy-MM-dd HH:mm:ss} UTC") |> ignore
        sb.AppendLine($"**Duration**: {report.Duration.TotalSeconds:F1}s") |> ignore
        sb.AppendLine() |> ignore

        sb.AppendLine("## Summary") |> ignore
        sb.AppendLine($"- **Total Tests**: {report.TotalTests}") |> ignore
        sb.AppendLine($"- **Passed**: {report.Passed} ({float report.Passed / float report.TotalTests * 100.0:F1}%)") |> ignore
    ...
    let saveReport (report: TestReport) : unit =
        let filename = $"reports/runtime_test_{DateTime.UtcNow:yyyy-MM-dd_HHmmss}.md"
...
let main args =
    ...
    printfn $"  - OpenRouter: {if Config.OpenRouterApiKey.IsSome then "Configured" else "Not configured"}"
