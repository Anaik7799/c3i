...
defmodule ContainerRegistryOptimizationSimple do
  ...
  defp get_current_containers do
    ...
    case System.cmd("podman", ["images", "--format", "table"], stderr_to_stdout: true) do
      {output, 0} ->
        ...
        containers
      {_, _} ->
        IO.puts("  ⚠️ Using mock data for container analysis")
  ...
  defp parse_size(size_str) do
    ...
    cond do
      String.contains?(size_str, "GB") ->
        {float_val, _} = Float.parse(String.replace(size_str, "GB", ""))
        round(float_val * 1_000_000_000)
      String.contains?(size_str, "MB") ->
        {float_val, _} = Float.parse(String.replace(size_str, "MB", ""))
  ...
  defp create_mock_containers do
    [
      %{repository: "localhost/intelitor-app", tag: "latest", id: "abc123", size: 1_500_000_000},
      %{repository: "localhost/intelitor-db", tag: "latest", id: "def456", size: 800_000_000},
      %{repository: "localhost/intelitor-base", tag: "nixos", id: "ghi789", size: 2_200_000_000},
  ...
  defp identify_critical_containers do
    ...
    critical_containers = [
      ...
      "localhost/intelitor-db:latest",
      "localhost/intelitor-base:nixos",
      "localhost/intelitor-redis:latest",
      "localhost/intelitor-registry.nixos.org/nixos/nginx:latest",
      "localhost/intelitor-monitoring:latest",
      "localhost/intelitor-backup:latest",
      "localhost/intelitor-security:latest"
    ]

    IO.puts("  ✅ Critical containers identified: #{length(critical_containers)}")
    IO.puts("  ✅ Production requirements defined")
    IO.puts("")
