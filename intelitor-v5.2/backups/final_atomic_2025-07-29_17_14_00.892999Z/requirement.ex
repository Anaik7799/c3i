defmodule Intelitor.Compliance.Requirement do
  @moduledoc """
  Represents individual compliance requirements within frameworks.
  ...
  attributes do
    ...
    attribute :requirement_number, :string do
      public? true
    ...
    attribute :prerequisite_requirements, {:array, :uuid} do
      public? true
      default []
    end
    ...
    attribute :assessment_frequency, :atom do
      public? true
    ...
    attribute :required_resources, {:array, :string} do
      public? true
      default []
    end
  ...
  relationships do
    ...
    belongs_to :assigned_to_user, Intelitor.Accounts.User do
      source_attribute :assigned_to
      attribute_public? true
    end

    belongs_to :last_updated_by_user, Intelitor.Accounts.User do
      source_attribute :last_updated_by
      attribute_public? true
    end

    belongs_to :reviewed_by_user, Intelitor.Accounts.User do
      source_attribute :reviewed_by
      attribute_public? true
    end

    has_many :assessments, Intelitor.Compliance.Assessment
  ...
  actions do
    ...
    update :update_implementation_status do
      ...
      change fn changeset, _context ->
        ...
        |> (fn cs ->
              if percentage do
                Ash.Changeset.force_change_attribute(cs, :compliance_percentage, percentage)
              else
                cs
              end
            end).()
    ...
    update :record_assessment do
      ...
      change fn changeset, _context ->
        ...
        frequency = Ash.Changeset.get_attribute(changeset, :assessment_frequency)
    ...
    update :request_exception do
      ...
      change fn changeset, _context ->
        ...
        new_exception = %{
          ...
          "requested_by" => changeset.arguments.requested_by,
          "request_date" => Date.utc_today(),
      ...
      calculation fn records, _context ->
        ...
        values =
          Enum.map(
            records,
            fn requirement ->
              requirement.implementation_deadline &&
                Date.compare(
                  today,
                  requirement.implementation_deadline
                ) == :gt &&
                requirement.implementation_status not in [
                  :implemented,
                  :verified,
                  :compliant,
                  :not_applicable
      ...
      calculation fn records, _context ->
        values =
          Enum.map(
            ...
            fn requirement ->
              risk_weight =
                case requirement.risk_if_not_implemented do
                  :critical -> 4
                  :high -> 3
                  :medium -> 2
                  :low -> 1
              ...
              impact_weight =
                case requirement.business_impact do
                  :critical -> 4
                  :high -> 3
                  :medium -> 2
                  :low -> 1
              ...
              criticality_weight =
                case requirement.criticality do
                  :critical -> 4
                  :high -> 3
                  :medium -> 2
                  :low -> 1
              ...
              compliance_factor =
                case requirement.compliance_status do
                  :compliant -> 0.1
      ...
      calculation fn records, _context ->
        values =
          Enum.map(
            records,
            fn requirement ->
              case requirement.implementation_status do
                :not_started -> "Not Started (0%)"
                :planning -> "Planning (10%)"
                :in_progress -> "In Progress (#{requirement.compliance_percentage || 50}%)"
                :implemented -> "Implemented (#{requirement.compliance_percentage || 100}%)"
  ...
  policies do
    ...
    policy action(:read) do
      ...
      # Assigned users can read their requirements
    ...
    policy action([:assign, :update_implementation_status, :record_assessment]) do
      ...
      authorize_if actor_attribute_equals(:role, "compliance_officer")
      authorize_if actor_attribute_equals(:role, "auditor")
      # Assigned users can update their requirements
      authorize_if expr(assigned_to == ^actor(:id))
    end
  ...
  postgres do
    ...
    custom_indexes do
      index [:tenant_id, :framework_id, :requirement_id], unique: true
      index [:framework_id]
      ...
      index [:priority]
      index [:criticality]
      index [:mandatory?], name: "requirements_mandatory_index", where: "mandatory? = true"
      ...
      index [:training_required?],
        name: "requirements_training_required_index",
        where: "training_required? = true"

      index [:monitoring_required?],
        name: "requirements_monitoring_required_index",
        where: "monitoring_required? = true"

      index [:automation_possible?],
        name: "requirements_automation_possible_index",
        where: "automation_possible? = true"

      index [:review_required?],
        name: "requirements_review_required_index",
