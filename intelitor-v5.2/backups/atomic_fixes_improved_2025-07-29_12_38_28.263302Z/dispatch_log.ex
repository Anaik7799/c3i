defmodule Intelitor.Alarms.DispatchLog do
  @moduledoc """
  ...
  relationships do
    ...
    belongs_to :dispatcher, Intelitor.Accounts.User do
      allow_nil? false
      attribute_public? true
    end

    belongs_to :acknowledged_by, Intelitor.Accounts.User do
      attribute_public? true
    end

    # Future relationships
    # belongs_to :dispatch_team, Intelitor.Dispatch.Team
    # belongs_to :zone, Intelitor.Sites.Zone
  end
  ...
  actions do
    ...
    update :add_handoff do
      ...
      change fn changeset, _context ->
        ...
        |> Ash.Changeset.force_change_attribute(:next_team_id, changeset.arguments.next_team_id)
    ...
    update :mark_departure do
      ...
      validate fn changeset, _context ->
        ...
        departure = Ash.Changeset.get_attribute(changeset, :actual_departure_time)

        if arrival && departure && DateTime.compare(departure, arrival) == :lt do
  ...
  calculations do
    ...
    calculate :requires_acknowledgment?, :boolean do
      calculation fn records, _context ->
        values =
          Enum.map(records, fn log ->
            !log.acknowledged? &&
