defmodule Intelitor.Billing.UsageRecord do
  @moduledoc """
  ...
  actions do
    ...
    update :apply_discount do
      ...
      change fn changeset, _context ->
        ...
        |> Ash.Changeset.force_change_attribute(:discount_percentage, discount_percentage)
        |> Ash.Changeset.force_change_attribute(:discount_amount, discount_amount)
    ...
    update :aggregate_usage do
      ...
      change fn changeset, _context ->
        ...
        changeset
        |> Ash.Changeset.force_change_attribute(:aggregation_level, aggregation_level)
        |> Ash.Changeset.force_change_attribute(:aggregated_from_records, child_ids)
        |> Ash.Changeset.force_change_attribute(:child_records_count, length(child_ids))
      end
    end
          ...
          Enum.map(
            ...
            fn usage ->
              if usage.usage_start do
                Date.to_string(DateTime.to_date(usage.usage_start))
                # YYYY-MM format
  ...
  defp generate_record_number(changeset) do
    # Generate record number like USG-20251206-001
  ...
  defp calculate_usage_cost(changeset) do
    ...
    overage_rate = Ash.Changeset.get_argument_or_attribute(changeset, :overage_rate) || unit_price
