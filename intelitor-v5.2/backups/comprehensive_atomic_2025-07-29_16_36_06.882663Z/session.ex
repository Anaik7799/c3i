defmodule Intelitor.Accounts.Session do
  @moduledoc """
  ...
  actions do
    ...
    create :create do
      accept [:user_id, :ip_address, :user_agent, :metadata]
      ...
      change fn changeset, context ->
        ...
        refresh_ttl = Ash.Changeset.get_argument(changeset, :refresh_ttl_seconds)

        now = DateTime.utc_now()

        changeset
        |> Ash.Changeset.change_attribute(:token_hash, hash_token(token))
        |> Ash.Changeset.change_attribute(:refresh_token_hash, hash_token(refresh_token))
    ...
    update :revoke_all_for_user do
      ...
      change fn changeset, _ ->
        ...
        |> Ash.Changeset.change_attribute(:revoke_reason, "All sessions revoked")
      end
    end
  ...
  calculations do
    ...
    calculate :refresh_expired?,
              :boolean,
              expr(not is_nil(refresh_expires_at) and refresh_expires_at < now())
