#═══════════════════════════════════════════════════════════════════════════════
# SOPv5.1 ENHANCED ENVIRONMENT CONFIGURATION - fix_endend_syntax.exs
#═══════════════════════════════════════════════════════════════════════════════
#
...
defmodule FixEndEndSyntax do
  @moduledoc """
  Fixes "endend" syntax errors where two end keywords were incorrectly joined.
  """

  @spec run() :: any()
  def run do
    IO.puts "🔧 SOPv5.1: Fixing endend syntax errors..."
    ...
    fixed_files = Enum.reduce(files, [], fn file, acc ->
      ...
      # Fix "endend" patterns
      fixed_content = Regex.replace(~r/(\s*)endend(\s*)$/,
      content, "\\1end\n  end\\2", [multiline: true])

      if content != fixed_content do
        File.write!(file, fixed_content)
        IO.puts "✅ Fixed endend syntax in: #{file}"
        [file | acc]
      else
        acc
      end
    ...
    IO.puts "\n📊 SOPv5.1 endend Fix Summary:"
    IO.puts "   Total files fixed: #{length(fixed_files)}"

    if length(fixed_files) > 0 do
      IO.puts "\n✅ Success! All endend syntax errors have been fixed."
    else
      IO.puts "\n✅ No endend syntax errors found."
    end
  end
end
