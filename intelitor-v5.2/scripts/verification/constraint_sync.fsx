...
// =============================================================================
// Constraint Synchronization Engine (F# Script)
// =============================================================================
...
//
// STAMP:    SC-SYNC-DOC-001 to SC-SYNC-DOC-010
// AOR:      AOR-SYNC-DOC-001 to AOR-SYNC-DOC-010
//
// Usage:
//   dotnet fsi scripts/verification/constraint_sync.fsx                  # Dashboard
//   dotnet fsi scripts/verification/constraint_sync.fsx --json           # JSON output
//   dotnet fsi scripts/verification/constraint_sync.fsx --gaps           # Show undocumented families
//   dotnet fsi scripts/verification/constraint_sync.fsx --gaps --json    # Gaps as JSON
//   dotnet fsi scripts/verification/constraint_sync.fsx --reconcile      # Reconciliation plan
//   dotnet fsi scripts/verification/constraint_sync.fsx --inventory      # .claude/ inventory
//   dotnet fsi scripts/verification/constraint_sync.fsx --full           # All of the above
//   dotnet fsi scripts/verification/constraint_sync.fsx --record         # Record sync timestamp
//   dotnet fsi scripts/verification/constraint_sync.fsx --analysis       # Full analysis (info theory, FMEA, STAMP, criticality)
//   dotnet fsi scripts/verification/constraint_sync.fsx --cached         # Read cached last-run results (fast, <100ms)
//
// MANDATE:  Claude agents MUST use this script (and ONLY this script) for
...
let extractConstraints (filePath: string) : (string * string) list =
    try
...
let runCensus (constraintType: string) : CensusResult =
    ...
    let codeHits =
        codeFiles
        |> List.collect extractConstraints
    ...
    let codeByFamily =
        codeHits
        |> List.groupBy (fun (id, _) -> extractFamily id)
        |> List.map (fun (fam, hits) -> fam, hits |> List.map fst |> Set.ofList)
        |> Map.ofList

    let codeExamples =
        codeHits
        |> List.groupBy (fun (id, _) -> extractFamily id)
        |> List.map (fun (fam, hits) ->
            let exFile = hits |> List.head |> snd
            let relPath =
...
let printReconciliationPlan (plan: ReconciliationItem list) =
    ...
    printfn "    5. Run this script with --record to update sync timestamp"
...
let computeInfoTheory (scCensus: CensusResult) (aorCensus: CensusResult) : InfoTheoryMetrics =
    ...
    { CodeEntropy = Math.Round(hCode, 4)
      ...
      MutualInformation = Math.Round(max 0.0 (hCode + hDocs - hCross), 4) // I(X;Y) = max(0, H(X) + H(Y) - H(P,Q))
...
let computeFmea (scCensus: CensusResult) (aorCensus: CensusResult) : FmeaEntry list =
    ...
    |> List.map (fun f ->
        ...
        // Occurrence: based on ID count (more IDs = more likely to hit undocumented constraint)
        let occurrence =
        ...
        // Detection: undocumented constraints are hard to detect
        // P0/P1 are harder because they affect safety/core (might not have obvious symptoms)
...
type StampAnalysis = {
    ControlActions: int              // Total SC-* (control actions documented)
    FeedbackLoops: int               // Total AOR-* (feedback/operating rules)
    UnsafeControlActions: int        // Undocumented SC-* (unsafe because unverified)
    MissingFeedback: int             // Undocumented AOR-* (missing operational feedback)
    ControlStructureCompleteness: float  // documented / total
    SafetyGapByPriority: Map<Priority, int * int>  // priority -> (undoc families, undoc IDs)
    TopUnsafeControlActions: string list  // Top 10 riskiest undocumented families
}
...
type CriticalityAnalysis = {
    OverallRiskScore: float          // 0-100, weighted composite
    CriticalFamiliesAtRisk: int      // P0 undocumented families
    HighFamiliesAtRisk: int          // P1 undocumented families
    MeanRPN: float                   // Average FMEA RPN
    MaxRPN: int                      // Highest single RPN
    P0Coverage: float                // % of P0 families documented
    P1Coverage: float                // % of P1 families documented
    EstimatedRemediationEffort: string  // Human-readable estimate
...
let computeCriticality (scCensus: CensusResult) (aorCensus: CensusResult)
    ...
    // Risk score: composite of coverage gaps weighted by priority
    let riskScore =
        (1.0 - p0Cov) * 40.0 +       // P0 coverage gap (40% weight)
        (1.0 - p1Cov) * 30.0 +       // P1 coverage gap (30% weight)
        (float metrics.ScGapPct / 100.0) * 20.0 +  // Overall SC gap (20% weight)
        (float metrics.AorGapPct / 100.0) * 10.0    // Overall AOR gap (10% weight)

    // Trend: check history file
    let trend =
...
let buildAnalysisJson (sb: System.Text.StringBuilder) (info: InfoTheoryMetrics) (fmea: FmeaEntry list) (stamp: StampAnalysis) (crit: CriticalityAnalysis) =
...
/// Write full analysis results to cache file for fast retrieval
...
/// Read cached analysis (for fast startup)
...
/// Check if reconciliation is due (weekly gate)
...
let buildDashboardJson (sb: System.Text.StringBuilder) (m: SyncMetrics) =
...
let buildGapsJson (sb: System.Text.StringBuilder) (scCensus: CensusResult) (aorCensus: CensusResult) =
    let allUndoc = scCensus.UndocumentedFamilies @ aorCensus.UndocumentedFamilies
...
let buildInventoryJson (sb: System.Text.StringBuilder) (inv: ClaudeInventory) =
...
let buildReconcileJson (sb: System.Text.StringBuilder) (plan: ReconciliationItem list) (skipped: bool) (lastDate: string) =
...
/// Emit a unified JSON object from a StringBuilder, removing trailing commas
let emitUnifiedJson (sb: System.Text.StringBuilder) =
    // Remove trailing comma+whitespace before closing brace
...
if helpMode then
    ...
    printfn ""
    printfn "Usage: dotnet fsi %s [FLAGS]" (Path.GetFileName(__SOURCE_FILE__))
    printfn ""
    printfn "FLAGS:"
    ...
    printfn ""
    printfn "Examples:"
    printfn "  dotnet fsi %s                  # Quick dashboard" (Path.GetFileName(__SOURCE_FILE__))
    printfn "  dotnet fsi %s --json           # Dashboard as JSON" (Path.GetFileName(__SOURCE_FILE__))
    printfn "  dotnet fsi %s --gaps           # Show gaps by priority" (Path.GetFileName(__SOURCE_FILE__))
    printfn "  dotnet fsi %s --analysis       # Deep analysis (writes cache)" (Path.GetFileName(__SOURCE_FILE__))
    printfn "  dotnet fsi %s --cached         # Read cached analysis (fast)" (Path.GetFileName(__SOURCE_FILE__))
    printfn "  dotnet fsi %s --full --json    # Everything as single JSON" (Path.GetFileName(__SOURCE_FILE__))
    exit 0
...
if cachedMode && not fullMode && not analysisMode then
    if readAnalysisCache () then exit 0
    else
...
// Record-only fast path (still needs census for metrics)
// (census is lightweight enough that we always run it when needed)

// Run census for all modes that need it
let scCensus = if needsCensus || recordMode then runCensus "SC" else { CodeConstraints = Map.empty; DocsConstraints = Map.empty; CodeExamples = Map.empty; UndocumentedFamilies = []; DocOnlyFamilies = []; AllFamilies = [] }
let aorCensus = if needsCensus || recordMode then runCensus "AOR" else { CodeConstraints = Map.empty; DocsConstraints = Map.empty; CodeExamples = Map.empty; UndocumentedFamilies = []; DocOnlyFamilies = []; AllFamilies = [] }
let metrics = if needsCensus || recordMode then computeMetrics scCensus aorCensus else { Date = ""; ScCodeCount = 0; ScCodeFamilies = 0; ScDocsCount = 0; ScDocsFamilies = 0; ScGap = 0; ScGapPct = 0.0; ScRatio = 0.0; AorCodeCount = 0; AorCodeFamilies = 0; AorDocsCount = 0; AorDocsFamilies = 0; AorGap = 0; AorGapPct = 0.0; AorRatio = 0.0; Health = "UNKNOWN"; LastSync = ""; RulesCount = 0; AgentsCount = 0; CommandsCount = 0; HooksCount = 0 }
...
if jsonMode then
    ...
    if dashboardMode || fullMode then
        ...
        sb.AppendLine(",") |> ignore  // separator for next section
    ...
    // Reconciliation plan (WEEKLY GATE: only runs if ≥7 days since last reconcile)
    if reconcileMode || fullMode then
        if isReconciliationDue () then
            let plan = generateReconciliationPlan scCensus aorCensus
    ...
    // Analysis (information theory, FMEA, STAMP, criticality) — auto-caches results
    if analysisMode || fullMode then
        let infoMetrics = computeInfoTheory scCensus aorCensus
        let fmeaEntries = computeFmea scCensus aorCensus
        let stampAnalysis = computeStamp scCensus aorCensus metrics
        ...
        // Auto-cache for fast retrieval via --cached
        writeAnalysisCache metrics infoMetrics fmeaEntries stampAnalysis critAnalysis
...
let exitCode =
    ...
    | "HEALTHY" -> 0
    | "DEGRADED" -> 0  // Warning but not failure
    | _ -> 0           // Don't block sessions on CRITICAL (informational)
