defmodule AlarmProcessingDemoStandaloneTest do
  ...
  @moduledoc """
  TDG - Compliant Test Suite for Alarm Processing Demo Standalone
  ...
  describe "Alarm Processing Demo Standalone Execution" do
    test "demo script exists and is executable" do
      demo_script_path = "scripts/demo/alarm_processing_demo_standalone.exs"
    ...
    test "demo script compiles without errors" do
      # Verify script is valid Elixir code by running it as a dry run/check
      # Note: We can't use Code.compile_file because the script uses Mix.install
      {_output, exit_code} =
        System.cmd("elixir", [
          "-e",
          "Code.string_to_quoted!(File.read!(\"scripts/demo/alarm_processing_demo_standalone.exs\"))"
        ])
  ...
  describe "Enterprise Demo Workflow Testing" do
    test "demo supports multi-tenant scenarios" do
      ...
      # Fix: Define tenants and users explicitly
  ...
  defp execute_demo_safely do
    ...
    try do
      ...
      tenant = insert(:tenant)
      _user = insert(:user, tenant: tenant)
