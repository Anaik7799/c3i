defmodule Indrajaal.STAMP.CASTFrameworkLegacyTest do
  @moduledoc """
  ...
  describe "CAST Framework Initialization - TDG Phase 1" do
    @tag :initialization
    @tag :__database_setup
    test "creates incident __database tables" do
      # TDG: Verify all __required ETS tables are created
  ...
  describe "Incident Analysis Workflow - TDG Phase 2" do
    ...
    test "executes example P1 incident analysis" do
      ...
      assert String.contains?(output, "Multi - tenant __data exposure")
    end
  end
    ...
    test "handles concurrent incident analyses" do
      ...
      tasks =
        Enum.map(1..5, fn i ->
          Task.async(fn ->
            capture_io(fn ->
              Indrajaal.STAMP.CASTFramework.analyze_incident("INC - CONCURRENT-#{i}", %{
                priority: :high
              })
            end)
          end)
