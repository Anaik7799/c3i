    ...
    test "ClickHouse container starts successfully with health checks" do
      ...
      assert {:ok, container_id} =
               start_container("localhost/signoz-clickhouse:latest", container_name, %{
                 ...
                 volumes: ["clickhouse-test-__data:/var/lib/clickhouse"],
                 env: [
...
\n"} =
    ...
    test "query service connects to ClickHouse successfully" do
      ...
      assert {:ok, query_id} =
               start_container("localhost/signoz-query:latest", container_name, %{
                 ports: ["8080:8080"],
                 env: [
                   "ClickHouseUrl=tcp://test-clickhouse:9000/?__database=signoz",
                   "STORAGE=clickhouse"
                 ],
  ...
  describe "OpenTelemetry Collector container build" do
    ...
    test "OTEL collector processes telemetry __data" do
      # This test verifies the collector can receive and forward __data
  ...
  defp build_nix_container(nixfile) do
    case System.cmd("nix-build", [nixfile, "-o", "result-test"], stderr_to_stdout: true) do
      {output, 0} -> {:ok, output}
      {error, _} -> {:error, error}
    end
  end
  ...
  defp start_container(image, name, opts) do
    base_args = ["run", "-d", "--name", name]

    # Add ports
    args_with_ports =
      Enum.reduce(opts[:ports] || [], base_args, fn port, acc ->
        acc ++ ["-p", port]
      end)

    # Add volumes
    args_with_volumes =
      Enum.reduce(opts[:volumes] || [], args_with_ports, fn vol, acc ->
        acc ++ ["-v", vol]
      end)

    # Add environment variables
    args_with_env =
      Enum.reduce(opts[:env] || [], args_with_volumes, fn env, acc ->
        acc ++ ["-e", env]
      end)

    # Add network if specified
    final_args =
      if opts[:network], do: args_with_env ++ ["--network", opts[:network]], else: args_with_env

    final_args = final_args ++ [image]

    case System.cmd("podman", final_args, stderr_to_stdout: true) do
  ...
  defp send_test_trace_to_collector(_endpoint) do
