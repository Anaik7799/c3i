defmodule Indrajaal.Authorization.PolicyTest do
  @moduledoc """
  TDG-compliant test suite for Indrajaal.Authorization.Policy.

  Tests the Policy Ash resource that manages authorization policies
  ...
  alias Indrajaal.Authorization.Policy
  ...
  describe "Module Structure" do
    ...
    test "module is an Ash resource" do
      assert function_exported?(Policy, :spark_is, 0)
    end

    test "has proper moduledoc" do
      case Code.fetch_docs(Policy) do
  ...
  describe "Ash Resource Structure" do
    test "has attributes defined" do
      info = Policy.__info__(:functions)
      assert is_list(info)
    end

    test "uses Indrajaal.BaseResource" do
      source_path = "lib/indrajaal/authorization/policy.ex"
    ...
    test "has postgres table configured" do
      ...
      assert content =~ ~s(table "authorization_policy")
  ...
  describe "Source Code Validation" do
    test "source file exists" do
      assert File.exists?("lib/indrajaal/authorization/policy.ex")
    end

    test "source file is valid Elixir" do
      source = File.read!("lib/indrajaal/authorization/policy.ex")
      {:ok, _ast} = Code.string_to_quoted(source)
    end

    test "has proper defmodule structure" do
      source = File.read!("lib/indrajaal/authorization/policy.ex")
      assert source =~ "defmodule Indrajaal.Authorization.Policy"
    end
  ...
  describe "Edge Cases" do
    test "module info returns expected structure" do
      ...
      assert info == Indrajaal.Authorization.Policy
    end

    test "handles introspection without errors" do
      _ = Policy.__info__(:functions)
      _ = Policy.__info__(:macros)
      _ = Policy.__info__(:attributes)
