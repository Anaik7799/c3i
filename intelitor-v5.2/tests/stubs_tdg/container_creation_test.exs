defmodule Intelitor.TDG.ContainerCreationTest do
  @moduledoc """
  ...
  alias Intelitor.ContainerSetup

  @ssl_env_script "/tmp/ssl_env.sh"
  @log_dir "./data/tmp"
  @container_images %{
    app: "localhost/intelitor-app:nixos-devenv",
    timescaledb: "localhost/intelitor-timescaledb:nixos-devenv",
    redis: "localhost/intelitor-redis:demo-ready"
  }

  describe "TDG: Environment Validation (Test-First)" do
    test "validate_environment/0 returns success when pre_requisites met" do
      # Agent-friendly comment: This test validates the environment checking
      # functionality that ensures all pre_requisites are available before
    ...
    test "validate_environment/0 fails gracefully with missing pre_requisites" do
      ...
      # Mock missing pre_requisites by temporarily renaming commands
  ...
  describe "TDG: Container Image Management (Test-First)" do
      ...
      case ContainerSetup.build_all_images() do
        {:ok, built_images} ->
          ...
          Enum.each(built_images, fn image ->
            assert String.starts_with?(image, "localhost/intelitor-"),
    ...
    test "validate_image_tags/1 ensures localhost registry compliance" do
      ...
      test_images = [
        ...
        "localhost/intelitor-timescaledb:nixos-devenv"
      ]
      ...
      case ContainerSetup.start_infrastructure_containers() do
        :ok ->
          ...
          case System.cmd("podman", ["ps", "--format", "{{.Names}}"], stderr_to_stdout: true) do
            {output, 0} ->
              container_names = String.trim(output) |> String.split("\n")

              expected_containers = ["intelitor-timescaledb-demo", "intelitor-redis-demo"]
    ...
    test "start_application_containers/0 starts app services after infrastructure" do
      ...
      case ContainerSetup.start_application_containers() do
        :ok ->
          ...
          case System.cmd("podman", ["ps", "--filter", "ancestor=localhost/intelitor-app"],
  ...
  describe "TDG: Property-Based Container Testing" do
    property "container names follow naming convention" do
      ...
      forall container_name <- container_name_generator() do
        String.starts_with?(container_name, "intelitor-") and
    ...
    property "container images use localhost registry" do
      ...
      forall container_image <- container_image_generator() do
        String.starts_with?(container_image, "localhost/intelitor-")
      end
    end
  ...
  defp container_image_generator do
    let {service, tag} <- {
      ...
      "localhost/intelitor-#{service}:#{tag}"
    end
  end
