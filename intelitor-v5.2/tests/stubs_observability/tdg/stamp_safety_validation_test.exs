defmodule Observability.TDG.StampSafetyValidationTest do
  ...
  enforced and that unsafe control actions (UCAs) are pr_evented.
  """
  use ExUnit.Case, async: false
  use Intelitor.Ultimate.TestConsolidation
  @tag :tdg_required
  @tag :stamp_safety
  describe "SC1: Telemetry data loss pr_evention" do
    test "collector buffers data during temporary outages" do
      ...
      # Send telemetry data during outage
      test_traces =
        for i <- 1..100 do
      ...
      # Verify data is buffered (not lost)
      ...
      # Verify all data was __eventually delivered
    ...
    test "collector applies backpressure when buffer is full" do
      ...
      # Verify rejected data is not lost (client can retry)
      assert {:ok, :retriable} = check_rejection_type(results)
    end

    test "disk persistence prevents data loss on collector restart" do
      ...
      # Verify all data recovered from disk
  ...
  describe "SC2: Query authorization and tenant isolation" do
    test "cross - tenant data access is pr_evented" do
      tenant_a = "company - a"
      tenant_b = "company - b"
      ...
      # Attempt to query tenant B data as tenant A
    ...
    test "tenant isolation works in aggregation queries" do
      ...
      # Verify only tenant - 1 data included
      assert stats["total_traces"] == 10
      assert stats["tenant_id"] == "tenant - 1"

      # Verify no data leak in response
    ...
    test "admin queries require proper authorization" do
      ...
      regular_auth = %{role: "user", tenant: "tenant - 1"}
  ...
  describe "SC3: Storage capacity management" do
    test "storage limits pr_event disk overflow" do
      ...
      # Try to insert data exceeding limit
  ...
  describe "SC4: Alert delivery timeliness" do
    ...
    test "alert suppression prevents notification storms" do
  ...
  @tag :tdg_required
  @tag :stamp_safety
  describe "UCA1: Query - induced ClickHouse OOM pr_evention" do
    ...
    test "query timeout prevents runaway queries" do
  ...
  defp check_clickhouse_health, do: {:ok, :healthy}

  defp check_clickhouse_health(_clickhouse) do
    {:ok, :healthy}
  end
