...
- **Module**: `Indrajaal.Observability.ObservabilityHelpers`
- **Behaviors**: [GenServer]
- **Functions**: 0 exported functions
- **Generated**: 2026-03-20 08:11:02.296115Z
...
```elixir
# Start the process
{:ok, pid} = Indrajaal.Observability.ObservabilityHelpers.start_link([])

# Basic operation example
{:ok, result} = Indrajaal.Observability.ObservabilityHelpers.basic_operation(%{
  param1: "value1",
  param2: "value2"
})
...
case Indrajaal.Observability.ObservabilityHelpers.advanced_operation(config) do
  {:ok, __data} ->
...
defmodule MyApp.Integration do
  ...
  def setup_observability do
    ...
    with {:ok, pid} <- Indrajaal.Observability.ObservabilityHelpers.start_link(config),
         {:ok, result} <- Indrajaal.Observability.ObservabilityHelpers.configure(config) do
