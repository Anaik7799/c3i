...
pub fn main() {
  ...
  let endpoints = [
    ...
    #("plan", "plan_get_pref", json.object([#("key", json.string("telegram_token"))])),
    #("plan", "workspace_get_auth_url", json.object([#("client_id", json.string("test_client"))])),
    #("plan", "gmail_list_unread", json.object([])),
    #("plan", "browser_screenshot", json.object([#("url", json.string("https://example.com"))])),
    #("plan", "exec", json.object([#("command", json.string("echo 'hello from exec'"))])),
    #("plan", "read_file", json.object([#("path", json.string("PROJECT_TODOLIST.md"))])),
