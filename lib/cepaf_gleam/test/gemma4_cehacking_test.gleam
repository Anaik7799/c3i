...
fn test_semantic_memory(state: moz.MoZClientState) {
  ...
  let prompt = "Using the EventLog in Smriti.db, identify the last 3 tasks I performed in Wave 1 and summarize their outcomes."
  let params = json.object([#("prompt", json.string(prompt)), #("model", json.string("gemma4"))])
  
  let #(_, result) = moz.send_request(state, "plan", "inference_generate", params)
  case result {
...
fn test_mcp_mapping(state: moz.MoZClientState) {
  ...
  let prompt = "I want to see my system health and send it to Abhi on GChat. Output the MCP tool calls as JSON-RPC."
  let params = json.object([#("prompt", json.string(prompt)), #("model", json.string("gemma4"))])
...
fn test_concurrent_load(state: moz.MoZClientState) {
  ...
  list.each(prompts, fn(p) {
    let params = json.object([#("prompt", json.string(p)), #("model", json.string("gemma4"))])
    let #(_, result) = moz.send_request(state, "plan", "inference_generate", params)
    case result {
