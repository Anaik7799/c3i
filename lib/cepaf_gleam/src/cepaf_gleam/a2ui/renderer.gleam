...
pub fn component_count(proposal: ComponentProposal) -> Int {
  1 + list.fold(proposal.children, 0, fn(acc, child) {
...
fn render_html(proposal: ComponentProposal) -> String {
  ...
  case proposal.component_type {
    ...
    "split_pane" | "grid_layout" | "responsive_columns" ->
      "<div class=\"card-grid\" role=\"group\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "tab_strip" | "sidebar_nav" | "layer_accordion" ->
      "<nav role=\"tablist\"" <> id_attr <> aria_label <> ">" <> children_html <> "</nav>"
    "collapsible_panel" | "dashboard_tile" ->
      "<details" <> id_attr <> aria_label <> "><summary>" <> proposal.id <> "</summary>" <> children_html <> "</details>"
    "fractal_breadcrumb" ->
      "<nav aria-label=\"breadcrumb\"" <> id_attr <> ">" <> children_html <> "</nav>"
    "scroll_viewport" ->
      "<div style=\"max-height:400px;overflow-y:auto\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "modal_overlay" ->
      "<div class=\"modal-overlay\" role=\"presentation\"" <> id_attr <> ">" <> children_html <> "</div>"
    "sticky_footer" ->
      "<footer role=\"contentinfo\"" <> id_attr <> aria_label <> ">" <> children_html <> "</footer>"
    "header_bar" ->
      "<header role=\"banner\"" <> id_attr <> aria_label <> ">" <> children_html <> "</header>"
    "empty_state" ->
      "<div class=\"empty-state\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    // Data components
    ...
    "json_tree" | "state_inspector" ->
      "<pre role=\"document\"" <> id_attr <> aria_label <> ">" <> children_html <> "</pre>"
    "triple_row" | "version_vector_row" | "resource_usage_row" ->
      "<tr role=\"row\"" <> id_attr <> aria_label <> ">" <> children_html <> "</tr>"
    "diff_viewer" | "reconciliation_diff" ->
      "<div class=\"diff\" role=\"document\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "metric_counter" | "latency_gauge" | "entropy_score" ->
      "<div class=\"card\" role=\"status\"" <> id_attr <> aria_label <> "><div class=\"card-value\">" <> children_html <> "</div></div>"
    "histogram_bar" | "coverage_gauge_ring" ->
      "<div class=\"bar-wrap\" role=\"meter\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "hash_display" | "hash_chain_strip" | "constitutional_hash_chain" ->
      "<code class=\"proof-chain\"" <> id_attr <> aria_label <> ">" <> children_html <> "</code>"
    "event_payload_card" | "proof_token_card" | "agent_run_card" ->
      "<article class=\"card\" role=\"article\"" <> id_attr <> aria_label <> ">" <> children_html <> "</article>"
    "task_detail_pane" ->
      "<article class=\"card\" role=\"article\"" <> id_attr <> aria_label <> ">" <> children_html <> "</article>"
    // Status components
    "health_indicator" | "container_status_dot" | "agent_heartbeat" ->
      "<span class=\"badge\" role=\"status\"" <> id_attr <> aria_label <> ">" <> children_html <> "</span>"
    "connection_status" | "sse_connection_indicator" | "circuit_breaker_status" | "sync_status_icon" ->
      "<span class=\"badge\" role=\"status\"" <> id_attr <> aria_label <> ">" <> children_html <> "</span>"
    "cockpit_mode_badge" | "mesh_mode_indicator" | "sil_compliance_badge" | "dag_integrity_badge" ->
      "<span class=\"badge\" role=\"status\"" <> id_attr <> aria_label <> ">" <> children_html <> "</span>"
    "quorum_indicator" | "threat_level_bar" | "mara_status" | "cognitive_load_meter" ->
      "<div class=\"card\" role=\"status\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "boot_phase_tracker" ->
      "<div class=\"ooda-phases\" role=\"progressbar\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "psi_invariant_row" ->
      "<tr role=\"row\"" <> id_attr <> aria_label <> ">" <> children_html <> "</tr>"
    "test_suite_status" ->
      "<div class=\"card\" role=\"status\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    ...
    "filter_bar" | "bulk_action_bar" ->
      "<div role=\"toolbar\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "search_input" ->
      "<input type=\"search\" role=\"searchbox\" placeholder=\"Search...\"" <> id_attr <> aria_label <> ">"
    "confirm_dialog" | "two_key_release" ->
      "<dialog aria-modal=\"true\"" <> id_attr <> aria_label <> ">" <> children_html <> "</dialog>"
    "toggle_switch" ->
      "<label class=\"toggle\"" <> id_attr <> aria_label <> "><input type=\"checkbox\" role=\"switch\">" <> children_html <> "</label>"
    "dropdown_select" ->
      "<select" <> id_attr <> aria_label <> ">" <> children_html <> "</select>"
    "command_palette" ->
      "<div class=\"command-palette\" role=\"combobox\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "threshold_slider" ->
      "<input type=\"range\" role=\"slider\"" <> id_attr <> aria_label <> ">"
    "topic_subscribe_btn" | "refresh_button" | "copy_button" | "chaos_inject_btn" ->
      "<button tabindex=\"0\"" <> id_attr <> aria_label <> ">" <> children_html <> "</button>"
    "pagination_controls" ->
      "<nav aria-label=\"pagination\"" <> id_attr <> ">" <> children_html <> "</nav>"
    "sort_header" ->
      "<th role=\"columnheader\" tabindex=\"0\"" <> id_attr <> aria_label <> ">" <> children_html <> "</th>"
    "time_range_picker" ->
      "<div class=\"time-range\" role=\"group\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    // Visualization components
    "container_grid_16" ->
      "<div class=\"genome-grid\" role=\"grid\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "ooda_waterfall" | "span_gantt_chart" | "metric_time_series" | "pid_control_plot" ->
      "<figure role=\"img\"" <> id_attr <> aria_label <> ">" <> children_html <> "</figure>"
    ...
    "antibody_list" | "hitl_pending_queue" | "message_thread" ->
      "<ul role=\"list\"" <> id_attr <> aria_label <> ">" <> children_html <> "</ul>"
    "attack_timeline" | "event_frequency_heatmap" ->
      "<div class=\"timeline\" role=\"img\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "knowledge_graph_mini" | "dependency_dag" ->
      "<figure role=\"img\"" <> id_attr <> aria_label <> ">" <> children_html <> "</figure>"
    "task_kanban_board" ->
      "<div class=\"card-grid-wide\" role=\"grid\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    // Agent components
    "tool_call_panel" | "reasoning_stream" ->
      "<div role=\"log\" aria-live=\"polite\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "agent_hierarchy_tree" ->
      "<ul role=\"tree\"" <> id_attr <> aria_label <> ">" <> children_html <> "</ul>"
    "agent_capability_badges" ->
      "<div class=\"ooda-phases\" role=\"group\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    // Safety components
    "guardian_approval_panel" ->
      "<div class=\"alert alert-warning\" role=\"alertdialog\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    "psi_invariant_dashboard" | "sil6_compliance_matrix" ->
      "<table role=\"table\"" <> id_attr <> aria_label <> ">" <> children_html <> "</table>"
    "emergency_banner" ->
      "<div class=\"alert alert-critical\" role=\"alert\" aria-live=\"assertive\"" <> id_attr <> ">" <> children_html <> "</div>"
    "audit_trail_log" ->
      "<div role=\"log\" aria-live=\"off\"" <> id_attr <> aria_label <> ">" <> children_html <> "</div>"
    _ ->
      "<div data-a2ui-type=\""
...
fn render_ansi(proposal: ComponentProposal) -> String {
  ...
  let component_text = case proposal.component_type {
    ...
    "button" | "action_button" ->
      ...
      <> visuals.with_color("┃", "dim")
    "emergency_stop" ->
      visuals.render_badge("EMERGENCY STOP", "critical")
    "alert" ->
      ...
      <> proposal.id
    "progress" ->
      proposal.id <> " " <> visuals.render_progress_bar(0.6, 20)
    "sparkline" ->
      proposal.id
      <> " "
    ...
    "topology" ->
      ...
      <> proposal.id
    "reasoning" ->
      visuals.with_color("💭 ", "magenta") <> proposal.id
    "data_table" ->
    ...
    "modal" ->
      ...
      <> visuals.with_color("└─────────────────────────────┘", "white")
    // --- LAYOUT ---
    "split_pane" -> visuals.with_color("╟", "dim") <> " " <> proposal.id <> " " <> visuals.with_color("╢", "dim")
    "tab_strip" -> visuals.with_color("┌─┐┌─┐┌─┐", "cyan") <> " " <> proposal.id
    "collapsible_panel" -> visuals.with_color("▸ ", "cyan") <> proposal.id
    "fractal_breadcrumb" -> visuals.with_color("L0 › L3 › L5", "dim") <> " " <> proposal.id
    "grid_layout" | "responsive_columns" -> visuals.with_color("┌┬┐", "dim") <> " " <> proposal.id
    ...
    "sticky_footer" -> visuals.with_color("─── ", "dim") <> proposal.id <> visuals.with_color(" ───", "dim")
    "layer_accordion" -> visuals.with_color("▼ L0-L7", "cyan")
    "dashboard_tile" -> visuals.with_color("┌─────┐", "cyan") <> "\n" <> visuals.with_color("│", "cyan") <> " " <> proposal.id
    ...
    "triple_row" -> visuals.with_color("(S)─(P)→(O)", "magenta") <> " " <> proposal.id
    "diff_viewer" -> visuals.with_color("+", "green") <> visuals.with_color("-", "red") <> " " <> proposal.id
    "metric_counter" -> visuals.with_color("▲", "green") <> " " <> proposal.id
    "histogram_bar" -> visuals.render_progress_bar(0.7, 16) <> " " <> proposal.id
    "version_vector_row" -> visuals.with_color("⊕", "blue") <> " " <> proposal.id
    "hash_display" -> visuals.with_color("#", "yellow") <> proposal.id
    "container_log_tail" -> visuals.with_color("⏎ ", "dim") <> proposal.id
    "sparql_result_grid" -> visuals.with_color("?s ?p ?o", "magenta") <> " " <> proposal.id
    "event_payload_card" -> visuals.render_badge(proposal.id, "info")
    "latency_gauge" -> visuals.with_color("⏱", "yellow") <> " " <> proposal.id
    "resource_usage_row" -> visuals.render_progress_bar(0.5, 12) <> " " <> proposal.id
    "task_detail_pane" -> visuals.with_color("📋", "cyan") <> " " <> proposal.id
    "proof_token_card" -> visuals.render_badge("PROOF", "healthy") <> " " <> proposal.id
    // --- STATUS ---
    ...
    "boot_phase_tracker" -> visuals.with_color("①②③④⑤⑥⑦", "cyan") <> " " <> proposal.id
    "threat_level_bar" -> visuals.render_badge("NOMINAL", "healthy")
    "container_status_dot" -> visuals.with_color("●", "green") <> " " <> proposal.id
    "psi_invariant_row" -> visuals.with_color("[✓]", "green") <> " " <> proposal.id
    ...
    "test_suite_status" -> visuals.with_color("✓", "green") <> " " <> proposal.id
    "cognitive_load_meter" -> visuals.render_progress_bar(0.3, 10) <> " cognitive"
    ...
    "confirm_dialog" -> visuals.with_color("[Approve]", "green") <> " " <> visuals.with_color("[Reject]", "red")
    ...
    "bulk_action_bar" -> visuals.with_color("[Start All] [Stop All] [Restart]", "yellow")
    ...
    "two_key_release" -> visuals.with_color("🔑", "yellow") <> visuals.with_color("🔑", "yellow") <> " bicameral"
    ...
    "ooda_waterfall" -> visuals.render_sparkline([0.03, 0.08, 0.02, 0.05, 0.04, 0.03]) <> " OODA cycles"
    "trace_flamegraph" -> visuals.with_color("▓▓▓▒▒░░", "yellow") <> " flamegraph"
    "span_gantt_chart" -> visuals.with_color("━━━━  ━━━  ━━━━━━", "cyan") <> " gantt"
    ...
    "pid_control_plot" -> visuals.render_sparkline([0.8, 0.9, 0.95, 0.98, 1.0, 0.99]) <> " PID"
    "version_clock_ring" -> visuals.with_color("◎ ", "blue") <> "version clocks"
    "event_frequency_heatmap" -> visuals.with_color("▓▒░▒▓", "yellow") <> " events/s"
    "task_kanban_board" -> visuals.with_color("│TODO│WIP│DONE│", "cyan")
    "dependency_dag" -> visuals.with_color("⊙→⊙→⊙", "dim") <> " DAG"
    "reconciliation_diff" -> visuals.with_color("+3 -1 ~2", "yellow") <> " reconcile"
    "router_topology_mini" -> visuals.with_color("◆─◆\n ╲╱\n  ◆", "green")
    "metric_time_series" -> visuals.render_sparkline([0.5, 0.6, 0.7, 0.8, 0.75, 0.9])
    ...
    // --- AGENT ---
    "agent_run_card" -> visuals.with_color("▶ ", "green") <> "run:" <> proposal.id
    ...
    "agent_hierarchy_tree" -> visuals.with_color("├─ cortex\n│ ├─ intel×4\n│ └─ worker×20", "cyan")
    "hitl_pending_queue" -> visuals.with_color("⏳", "yellow") <> " HITL pending"
    "activity_feed" -> visuals.with_color("≡ ", "dim") <> proposal.id
    "state_inspector" -> visuals.with_color("{state}", "cyan") <> " " <> proposal.id
    "message_thread" -> visuals.with_color("💬 ", "blue") <> proposal.id
    "agent_capability_badges" -> visuals.render_badge("EMIT", "info") <> visuals.render_badge("HITL", "warning")
    // --- SAFETY ---
    "guardian_approval_panel" -> visuals.render_badge("GUARDIAN", "warning") <> " " <> proposal.id
    "psi_invariant_dashboard" -> visuals.with_color("Ψ₀✓ Ψ₁✓ Ψ₂✓ Ψ₃✓ Ψ₄✓ Ψ₅✓ Ω₀✓", "green")
    "emergency_banner" -> visuals.render_badge("EMERGENCY", "critical") <> " " <> proposal.id
    "constitutional_hash_chain" -> visuals.with_color("⊕→⊕→⊕→⊕", "green") <> " constitution"
