...
(function() {
  ...
  // ═══════════════════════════════════════════════════════════════
  // SSE EventSource — Real-time task status push (replaces polling)
  // ═══════════════════════════════════════════════════════════════
  ...
  function initSSE() {
    if (typeof EventSource === "undefined") return; // Fallback to polling
    try {
      sseSource = new EventSource(API_BASE + "/api/v1/plan/stream");
      ...
      sseSource.addEventListener("status", function(e) {
        try {
          ...
          updateHeaderFromSSE(total, completed, pending, active, blocked);
          logChange("refresh", "SSE push: " + active + " active, " + blocked + " blocked, " + total + " total");
        } catch(ex) {}
      });

      sseSource.addEventListener("active", function(e) {
        try {
          var data = JSON.parse(e.data);
          if (grids.active && Array.isArray(data)) {
            data.forEach(function(t) { t._layer = classifyFractalLayer(t); });
            var newSnap = snapshotData(data);
            var changed = findChangedIds(prevSnapshots.active, newSnap);
            if (changed.length > 0) {
      ...
      sseSource.onerror = function() {
        sseConnected = false;
        updateStatusBar("SSE disconnected \u2014 falling back to polling", "error");
        // Reconnect handled by EventSource built-in (retry: 3000ms from server)
      };

      sseSource.onopen = function() {
        sseConnected = true;
        logChange("refresh", "SSE connected \u2014 real-time push active");
      };
    } catch(ex) {
  ...
  function updateHeaderFromSSE(total, completed, pending, active, blocked) {
  ...
  function initAIChat() {
    ...
    container.innerHTML =
      ...
      "<div style='color:#7a8fa6;text-align:center;padding:20px;font-size:0.78rem'>Ask Gemma 4 about your tasks...<br>Try: \"What are the blocked P0 tasks?\" or \"Summarize active work\"</div>" +
      "</div>" +
      "<div style='display:flex;gap:6px;padding:8px;border-top:1px solid rgba(30,42,58,0.4)'>" +
      "<input id='ai-chat-input' type='text' placeholder='Ask Gemma 4...' style='flex:1;background:rgba(10,14,23,0.6);border:1px solid rgba(30,42,58,0.5);color:var(--text,#e0e6ed);padding:10px 14px;border-radius:8px;font-size:0.85rem;outline:none;min-height:44px'>" +
  ...
  function sendAIMessage(query) {
    ...
    messagesDiv.innerHTML += "<div id='" + typingId + "' style='display:flex;align-items:center;gap:8px;padding:8px 0'><span style='color:#00d4aa;font-weight:600;font-size:0.72rem'>Gemma 4</span><span style='color:#7a8fa6;font-size:0.78rem;animation:shimmer 1.5s infinite;background:linear-gradient(90deg,#7a8fa633,#7a8fa688,#7a8fa633);background-size:200%;border-radius:4px;display:inline-block;width:80px;height:14px'></span></div>";
    messagesDiv.scrollTop = messagesDiv.scrollHeight;

    // Call Gemma 4 via Ollama directly
    callGemma4(query).then(function(response) {
      ...
      fetch(API_BASE + "/api/v1/ai/chat?q=" + encodeURIComponent(query))
        ...
        .catch(function() {
          appendChatMessage(messagesDiv, "assistant", "AI agent unavailable. Check Ollama status on port 11435.");
        });
    });
  }

  function callGemma4(query) {
    var statusData = null;
    ...
    return fetch(API_BASE + "/api/v1/plan/status")
      ...
      .then(function(status) {
        statusData = status;
        // Build Gemma 4 prompt with task context
        var systemPrompt = "You are the C3I Planning AI assistant for the Indrajaal distributed mesh system. " +
          "Current task status: " + JSON.stringify(status) + ". " +
          "The system has " + status.total + " total tasks, " + status.active + " active, " +
          status.blocked + " blocked, " + status.completed + " completed. " +
          "You help operators analyze tasks, suggest priorities, assess risks, and provide insights. " +
          "Be concise (2-3 sentences max). Use specific numbers from the data.";
        ...
        return fetch("http://localhost:11435/api/generate", {
          method: "POST",
      ...
      .then(function(r) {
        if (!r.ok) throw new Error("Gemma 4 HTTP " + r.status);
        return r.json();
      })
      .then(function(data) {
        return data.response || "No response from Gemma 4";
      });
  }
  ...
  setTimeout(function() {
    ...
    initSSE();
    startRefreshTimers();
    // Live header refresh every 5s (fallback if SSE disconnects)
    setInterval(function() {
      if (!sseConnected) refreshHeaderStatus();
    }, 5000);
