...
4. **Fractal Alignment**: Ensure the design and code align perfectly with the L0-L7 fractal layers, components, and runtime operations. Any new UI must be Agentic (Penta-Stack: Lustre/Wisp/TUI/Canvas/Realtime).
5. **Task Authority**: ALL interactions with the planning and task system must go exclusively through the Rust `./sa-plan` tool (`add`, `update`, `sync`). Never edit `PROJECT_TODOLIST.md` directly.
6. **Documentation & Persistence**: After each distinct feature addition, update the active plan, update the technical specs, and create a detailed journal entry in `docs/journal/`. Commit and push the features atomically using `git`.
7. **Sprint Closure**: Upon fully meeting the sprint goal, perform a final `./sa-plan sync`, write a sprint closure journal entry summarizing the mathematical proofs and TDG coverage, and push all final artifacts to the root repository."
